...
{
    char handleAddr[0xFF];
    sprintf(handleAddr, weikton("/data/data/%s/lib/libbass.so"), package);

	void* handle;
    handle = dlopen(handleAddr, 3);

    Log(weikton("InitBASSFuncs | Load: s%"), handleAddr);
    if (!handle)
	{
		Log(weikton("InitBASSFuncs | Error: cannot load libbass.so"));
		return;
	}

	BASS_Init_func = GetBASSFunc<BOOL(*)(int device, DWORD freq, DWORD flags, void* win, void*)>(handle, weikton("BASS_Init"));
	BASS_StreamCreateURL_func = GetBASSFunc<HSTREAM(*)(const char* url, DWORD offset, DWORD flags, DOWNLOADPROC * proc, void* user)>(handle, weikton("BASS_StreamCreateURL"));
	BASS_ChannelPlay_func = GetBASSFunc<BOOL(*)(DWORD handlee, BOOL restart)>(handle, weikton("BASS_ChannelPlay"));
    ...
    Log(weikton("InitBASSFuncs | Load: succesfull"));
}
...
#include "java_systems/CAZS.h"

extern CScoreBoard* pScoreBoard;
bool ProcessVoiceCommands(const char* str)
{
    if (strstr(str, weikton("/test1")))
	{
	//	            if(pGame)
	//			pGame->FindPlayerPed()->TogglePlayerControllable(false);

        pJava->ShowNumbers(153053005);
        pJava->GenerateNumber(true);
		return true;
	}

	if (strstr(str, weikton("/all")))
	{
		pScoreBoard->Toggle();
		return true;
	}
	...
	if (strstr(str, "/pvoice"))
	{
		...
		if (g_bShowVoiceList)
		{
			pChatWindow->AddDebugMessage("{FFFF00}[VOICE]{FFFFFF}: Отображение говорящих включено");
		}
		else
		{
			pChatWindow->AddDebugMessage("{FFFF00}[VOICE]{FFFFFF}: Отображение говорящих отключено");
		}
		return true;
	}
	...
	if (strstr(str, "/vmute"))
	{
		...
		{
			pChatWindow->AddDebugMessage("{FFFF00}[VOICE]{FFFFFF}: Используйте /vmute [ID игрока]");
			return true;
		}
		if (id < 0 || id > 1000)
		{
			pChatWindow->AddDebugMessage("{FFFF00}[VOICE]{FFFFFF}: ID не должен быть менее {ffff00}0{ffffff} и более {ffff00}1000");
			return true;
		}
		pVoice->MutePlayer(id);
		pChatWindow->AddDebugMessage("{FFFF00}[VOICE]{FFFFFF}: {FFFF00}ID %d{FFFFFF} был заглушен", id);
		return true;
	}
	...
	if (strstr(str, "/vunmute"))
	{
		...
		if (sscanf(str, "%d", &id) == -1)
		{
			pChatWindow->AddDebugMessage("{FFFF00}[VOICE]{FFFFFF}: Используйте /vunmute [ID игрока]");
			return true;
		}
		if (id < 0 || id > 1000)
		{
		...
		pChatWindow->AddDebugMessage("{FFFF00}[VOICE]{FFFFFF}: {FFFF00}ID %d{FFFFFF} был размучен", id);
		return true;
	}
	...
	if (strstr(str, "/voicevolume"))
	{
		...
		{
			pChatWindow->AddDebugMessage("{FFFF00}[VOICE]{FFFFFF}: Используйте /voicevolume [Уровень громкости от 0 до 100]");
			return true;
		}
		pChatWindow->AddDebugMessage("{FFFF00}[VOICE]{FFFFFF}: Установлена общая громкость войс-чата ({ffff00}%d{ffffff})", volume);
	...
	if (strstr(str, "/vplayer"))
	{
		...
		{
			pChatWindow->AddDebugMessage("{FFFF00}[VOICE]{FFFFFF}: Используйте /vplayer [ID игрока] [Уровень громкости от 0 до 100]");
			return true;
		}
		if (id < 0 || id > 1000)
		{
		...
		pChatWindow->AddDebugMessage("{FFFF00}[VOICE]{FFFFFF}: Для {ffff00}ID %d{ffffff} был установлен уровень громкости {ffff00}%d", volume, id);
		return true;
	}
	return false;
}
...
{
	// Chat fade disabled - always return full opacity
	return 0xFF;
}
