...
void CKeyBoard::Render()
{
    ...
    ImVec2 pos{m_Pos.x + pGUI->ScaleX(50.0f), m_Pos.y - pGUI->GetFontSize() * 1.5f};

    std::string notificationUTF8 = "Чтобы вставить текст, нажмите дважды на поле ввода";

    char notification[128];

    cp1251_to_utf8(notification, notificationUTF8.c_str());

    pGUI->RenderText(pos, IM_COL32_WHITE, true, notification);
    ...
    // paste text by weikton
    // ImVec2 verpos = ImVec2(pGUI->ScaleX(8), pGUI->ScaleY(8));
    // pGUI->RenderText(verpos, ImColor(0xFFFFFFFF), true, "tapy.me/weikton");

    // background
    ImGui::GetOverlayDrawList()->AddRectFilled(
            m_Pos,
            ImVec2(m_Size.x, io.DisplaySize.y),
            IM_COL32(25, 25, 25, 205));
            ...
            switch (key.type)
            {
                ...
                case KEY_SHIFT:
                    ImGui::GetOverlayDrawList()->AddTriangleFilled(
                            ...
                            m_iCase == LOWER_CASE ? IM_COL32(115, 115, 115, 200) : 0xFFFF0000);
                    ImGui::GetOverlayDrawList()->AddRectFilled(
                ...
                case KEY_SPACE:
                    ImGui::GetOverlayDrawList()->AddRectFilled(
                            ...
                            key.id == m_iPushedKey && !IsHidden() ? 0xFFFF0000 : 0xFF8A8886);
                    break;
...
bool CKeyBoard::OnTouchEvent(int type, bool multi, int x, int y)
{
    ...
    if (pJava)
    {
        if (type == TOUCH_PUSH && x >= leftCorner.x && y >= leftCorner.y && x <= rightCorner.x && y <= rightCorner.y)
        {
            ...
            {
                std::string msg = pJava->GetClipboardString();
