...
void CSettings::Save(int iIgnoreCategory)
{
	...
	ini_table_create_entry(config, "client", "host", m_Settings.szAddress);
	ini_table_create_entry_as_int(config, "client", "port", m_Settings.szPort);
...
void CSettings::LoadSettings(const char* szNickName, int iChatLines)
{
	...
	const char* szAddress = ini_table_get_entry(config, "client", "host");
	...
	if (szAddress)
	{
		strcpy(m_Settings.szAddress, szAddress);
	}
	if (szPassword)
	{
	...
	m_Settings.szPort = ini_table_get_entry_as_int(config, "client", "port", 7777);
