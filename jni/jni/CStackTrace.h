...
public:
	...
	{
		CrashLog(weikton("------------ START BACKTRACE ------------"));
		...
		CrashLog(weikton("------------ END BACKTRACE ------------"));
	}
...
private:
	static void printAddressBacktrace(const unsigned address, void* pc, void* lr)
	{
		...
		sprintf(filename, weikton("/proc/%d/maps"), getpid());

		auto* const fp = fopen(filename, weikton("rt"));
		if (fp == nullptr)
		{
			Log(weikton("ERROR: can't open file %s"), filename);
			return;
		}
