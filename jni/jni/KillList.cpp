...
void KillList::MakeRecord(const char *playername, unsigned int playercolor, unsigned short playerid, const char *killername, unsigned int killercolor, unsigned short killerid, uint8_t reason)
{
...
{
	if (!pGame->IsToggledHUDElement(HUD_ELEMENT_KILL_LIST)) return;
        ...
        for(auto& playerkill : m_pKillList)
        {
            if(playerkill)
			{
				...
				else
				{
					...
					PlayerColor[0] = 256 + ((playerkill->playercolor & 0xFF000000) >> 24);
					PlayerColor[1] = (playerkill->playercolor & 0x00ff0000) >> 16;
					PlayerColor[2] = (playerkill->playercolor & 0x0000ff00) >> 8;
					
					unsigned char KillerColor[3];
					KillerColor[0] = 256 + ((playerkill->killercolor & 0xFF000000) >> 24);
					KillerColor[1] = (playerkill->killercolor & 0x00ff0000) >> 16;
					KillerColor[2] = (playerkill->killercolor & 0x0000ff00) >> 8;
