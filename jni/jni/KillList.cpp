...
void KillList::UpdateValuesRender(){
    render_posX = pSettings->GetReadOnly().fKillListPosX;
    render_posY = pSettings->GetReadOnly().fKillListPosY;
    render_scale = (float) pSettings->GetReadOnly().fKillListScale/100.0f;
    if(render_scale == 0){
    render_scale = 0.01f;
    }
}
...
void KillList::Render()
{
    ...
    {
        float fPosSprite = pGUI->ScaleX(render_posX);

        ImVec2 vecPos;
        vecPos.y = render_posY;
        ...
        for(auto& playerkill : m_pKillList)
        {
            if(playerkill)
			{
				if (playerkill->reason == 255)
				{
					...
					pGUI->RenderTextDeathMessage(vecPos, 0xFFFFFFFF, true, "  ", nullptr, pGUI->GetFontSize() * render_scale);
						
					vecPos.x = fPosSprite;
					pGUI->RenderTextDeathMessage(vecPos, 0xFFFFFFFF, false, "  ", nullptr, pGUI->GetFontSize() * render_scale, pGUI->GetWeaponFont());
				...
				else
				{
					...
					pGUI->RenderTextDeathMessage(vecPos, ImColor(KillerColor[0], KillerColor[1], KillerColor[2]), true, killerName,
                                                 nullptr, pGUI->GetFontSize() * render_scale);
						
					vecPos.x = fPosSprite;
					pGUI->RenderTextDeathMessage(vecPos, 0xFF000000, false, "G", nullptr, pGUI->GetFontSize() * render_scale, pGUI->GetWeaponFont());
					pGUI->RenderTextDeathMessage(vecPos, 0xFFFFFFFF, false, SpriteIDForWeapon(playerkill->reason), nullptr, pGUI->GetFontSize() * render_scale, pGUI->GetWeaponFont());
