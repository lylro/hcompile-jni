...
#ifndef BKUZN_CSTACKTRACE_H
#define BKUZN_CSTACKTRACE_H
...
#define PRINT_CRASH_STATES(context) \
    ...
    FLog("1: libmultiplayer.so + 0x%X", context->uc_mcontext.arm_pc - g_libSAMP); \
    FLog("2: libmultiplayer.so + 0x%X", context->uc_mcontext.arm_lr - g_libSAMP);
#else
#define PRINT_CRASH_STATES(context) \
    ...
    FLog("1: libmultiplayer.so + 0x%llx", context->uc_mcontext.pc - g_libSAMP); \
    FLog("2: libmultiplayer.so + 0x%llx", context->uc_mcontext.regs[30] - g_libSAMP);
#endif
