...
- Привет недокодерам Vernetto, Рокинов, Codevsky
...
RwTexture* CCustomPlateManager::ProcessRuPlate(SCustomPlate& plate)
{
	...
	pText->Draw(22.0f, 10.0f, 160.0f, 40.0f, white); // x y width height ТЕКСТ
	pRegion->Draw(206.0f, 14.0f, 38.0f, 20.0f, white); // x y width height РЕГИОН
	RwTexture* pTexture = m_pRenderTarget->End();
