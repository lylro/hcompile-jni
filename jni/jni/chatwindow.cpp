...
void InitBASSFuncs()
{
	...
	void* handle;
	//handle = dlopen(OBFUSCATE("/data/data/com.dynamicmobilenew.game/lib/libbass.so"), 3);
	handle = dlopen(OBFUSCATE("libbass.so"), 3);

	if (!handle)
	{
		//Log("Cannot load zalupa: 1");
		return;
	} else //Log("Loaded zalupa!");
...
//	}
	...
	if (strstr(str, OBFUSCATE("/cameditgui")))
	{
		if(camedit){
			...
			camedit=false;
				...
				camedit=true;
				if(chat3){pChatWindow->showingchat=true;}
	...
	if (strstr(str, OBFUSCATE("/testpon")))
	{
		...
		pVehicle->m_pVehicle->pHandling->m_nFrontLights = eVehicleLightsSize::LIGHTS_LONG;
		
		return true;
	}
	if (strstr(str, OBFUSCATE("/lightset1")))
	{
		CVehicle *pVehicle = nullptr;
		CVehiclePool *pVehiclePool = pNetGame->GetVehiclePool();
		CPlayerPed *pPlayerPed = pGame->FindPlayerPed();
		VEHICLEID id = pVehiclePool->FindIDFromGtaPtr(pPlayerPed->GetGtaVehicle());
		pVehicle = pVehiclePool->GetAt(id);
		pVehicle->m_pVehicle->pHandling->m_nFrontLights = eVehicleLightsSize::LIGHTS_BIG;
			
		
		return true;
	}
	if (strstr(str, OBFUSCATE("/lightset2")))
	{
		CVehicle *pVehicle = nullptr;
		CVehiclePool *pVehiclePool = pNetGame->GetVehiclePool();
		CPlayerPed *pPlayerPed = pGame->FindPlayerPed();
		VEHICLEID id = pVehiclePool->FindIDFromGtaPtr(pPlayerPed->GetGtaVehicle());
		pVehicle = pVehiclePool->GetAt(id);
		pVehicle->m_pVehicle->pHandling->m_nFrontLights = eVehicleLightsSize::LIGHTS_SMALL;
			
		
		return true;
	}
	if (strstr(str, OBFUSCATE("/lightset0")))
	{
		CVehicle *pVehicle = nullptr;
		CVehiclePool *pVehiclePool = pNetGame->GetVehiclePool();
		CPlayerPed *pPlayerPed = pGame->FindPlayerPed();
		VEHICLEID id = pVehiclePool->FindIDFromGtaPtr(pPlayerPed->GetGtaVehicle());
		pVehicle = pVehiclePool->GetAt(id);
		pVehicle->m_pVehicle->pHandling->m_nFrontLights = eVehicleLightsSize::LIGHTS_TALL;
			
		
		return true;
	}
                  if (strstr(str, OBFUSCATE("/tab")))
	{
		if(pNetGame->GetGameState() == GAMESTATE_CONNECTED) pScoreBoard->Toggle();
		return true;
	}

	if (strstr(str, OBFUSCATE("/online")))
	{
		if(pNetGame->GetGameState() == GAMESTATE_CONNECTED) pScoreBoard->Toggle();
		return true;
	}
	...
	if (strstr(str, "/debug"))
	{
		/*if(pNetGame->GetGameState() == GAMESTATE_CONNECTED) */CDebugInfo::ToggleDebugDraw();
		pGUI->bShowTexDebugger = !pGUI->bShowTexDebugger;
		return true;
	}
	...
	if (strstr(str, "/vconnect"))
	{
		if (pVoice) pVoice->SetNetworkState(VOICECHAT_WAIT_CONNECT);
                                    pChatWindow->AddDebugMessage("Voice connecting");
		return true;
	}
	if (strstr(str, "/vmute"))
	{
...
void CChatWindow::CloseKeyboard()
{
			if (pKeyBoard)
			{
				pKeyBoard->Close();
...
void CChatWindow::RenderText(const char* u8Str, float posX, float posY, uint32_t dwColor, float alphaNewMessage)
{
	...
	ImVec4 col;
		...
		ImVec2 b(posCur.x + 1150, posCur.y + ImGui::GetFontSize());

	const ImU32 startColor = IM_COL32(0, 0, 40, 60); 
	const ImU32 endColor = IM_COL32(0, 0, 0, 0); 
	const ImU32 toc = IM_COL32(255, 255, 255, 255); 
