...
#ifdef FLIN
	handle = dlopen("/data/data/com.flin.online/lib/libbass.so", 3);
#elif JUST
	handle = dlopen("/data/data/com.brilliant.cr/lib/libbass.so", 3);
#else
	handle = dlopen("/data/data/com.brilliant.cr/lib/libbass.so", 3);
#endif
	if (!handle)
...
RwObject* Tessssstttt(RwObject* object, void* data){
...
bool ProcessVoiceCommands(const char* str)
{
    if (strcmp(str, "/q") == 0)
    {
        *(uint8_t*)(g_libGTASA + 0x63E094) = 0;
        ...
        if (pNetGame)
        {
            if (pNetGame->GetRakClient()) pNetGame->GetRakClient()->Disconnect(500, 0);
        } 
        g_pJavaWrapper->CallLauncherActivity(14555);
        return true;
    }
	    if(strcmp(str, "/cleartd") == 0){
        pSnapShotHelper->ClearCacheTexture();

        pChatWindow->AddDebugMessage("You have successfully cleared the cache TextDraw!");

        return true;
    }
    if(strcmp(str, "/donatesite") == 0){
	...
	if (strstr(str, "/cjwhite"))
	{
		g_pMasterCheat->swTest(true);
		return true;
	}
