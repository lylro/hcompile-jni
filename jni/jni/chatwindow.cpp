...
void InitBASSFuncs()
{
	...
	handle = dlopen("/data/data/com.mobilerussia.game/lib/libbass.so", 3);
#else
	handle = dlopen("/data/data/com.mobilerussia.game/lib/libbass.so", 3);
#endif
	if (!handle)
...
bool ProcessVoiceCommands(const char* str)
{
	if (strstr(str, "/dl"))
	{
		pGUI->m_bMemDebugOn = !pGUI->m_bMemDebugOn;
		return true;
	}//Rickk was here by @rkkdev
	if (strstr(str, "/fpsinfo") || strstr(str, "/debug"))
	{
