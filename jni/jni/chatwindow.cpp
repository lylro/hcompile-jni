...
extern "C"
{
	JNIEXPORT void JNICALL Java_com_byreytiz_game_gui_NewChat_showCppKeyboard(JNIEnv* pEnv, jobject thiz)
	{
		if (pKeyBoard)
			{
				pKeyBoard->Open(&ChatWindowInputHandler);
...
void CChatWindow::AddChatMessage(char* szNick, uint32_t dwNickColor, char* szMessage)
{
	...
	g_pJavaWrapper->AddMessageToJavaChat(szMessage);
}
...
void CChatWindow::AddInfoMessage(char* szFormat, ...)
{
	...
	g_pJavaWrapper->AddMessageToJavaChat(tmp_buf);
...
void CChatWindow::AddDebugMessageNonFormatted(char* szStr)
{
	...
	g_pJavaWrapper->AddMessageToJavaChat(szStr);
