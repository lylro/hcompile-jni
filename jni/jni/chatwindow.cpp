...
void InitBASSFuncs()
{
	...
	void* handle;
	handle = dlopen(OBFUSCATE("/data/data/fussrussia.online/lib/libbass.so"), 3);
...
//	}
	...
	if (strstr(str, OBFUSCATE("/hud")))
	{
		if(pNetGame->GetGameState() == GAMESTATE_CONNECTED) g_pJavaWrapper->ShowClientSettings();
		return true;
	}

	if (strstr(str, OBFUSCATE("/test")))
	{
		g_pJavaWrapper->toggleDrivingSchool(true);
		return true;
	}
	
	if (strstr(str, OBFUSCATE("/client")))
	{
		if(pNetGame->GetGameState() == GAMESTATE_CONNECTED) g_pJavaWrapper->ShowClientSettings();
		return true;
	}
