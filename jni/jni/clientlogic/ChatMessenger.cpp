...
char CLocalisation::m_szMessages[E_MSG::MSG_COUNT][MAX_LOCALISATION_LENGTH] = {
	"{bfbeba}Соединение с ELITE RUSSIA",
	"{bfbeba}Соединено. Подготовка к игре...",
	"{bfbeba}Сервер закрыл соединение. Перезайдите",
	"{ffffff}Клиент {ff0000}ELITE RUSSIA {ffffff}запущен",
	"{bfbeba}unused",
	"{bfbeba}Вы были заблокированы на сервере",
	"{bfbeba}Потеряно соединение с сервером. Переподключение через 15 секунд...",
	"{bfbeba}Отсутствует соединение. Попробуйте перезагрузить роутер, сменить интернет или воспользоваться VPN",
