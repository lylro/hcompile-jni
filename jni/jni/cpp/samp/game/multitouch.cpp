...
void MultiTouch::initialize()
{
    ...
    // 3 touch end
#if VER_x32
    CHook::WriteMemory2(g_libGTASA + 0x0026B03C, (uintptr_t)"\x03\x20", 2); // OS_PointerGetNumber
    CHook::WriteMemory2(g_libGTASA + 0x0026B046, (uintptr_t) "\x03\x28", 2); // OS_PointerGetType
    CHook::WriteMemory2(g_libGTASA + 0x002700AC, (uintptr_t) "\x03\x28", 2); // LIB_PointerGetCoordinates
    CHook::WriteMemory2(g_libGTASA + 0x00270118, (uintptr_t) "\x03\x28", 2); // LIB_PointerGetWheel
    CHook::WriteMemory2(g_libGTASA + 0x00270164, (uintptr_t) "\x03\x28", 2); // LIB_PointerDoubleClicked
    CHook::WriteMemory2(g_libGTASA + 0x002700F2, (uintptr_t) "\x03\x2A", 2); // LIB_PointerGetButton
#else
    CHook::WriteMemory2(g_libGTASA + 0x320844, (uintptr_t)"\x60\x00\x80\x52", 4); // OS_PointerGetNumber
    CHook::WriteMemory2(g_libGTASA + 0x320850, (uintptr_t) "\x1F\x0C\x00\x71", 4); // OS_PointerGetType
    CHook::WriteMemory2(g_libGTASA + 0x326FEC, (uintptr_t) "\x1F\x0C\x00\x71", 4); // LIB_PointerGetCoordinates
    CHook::WriteMemory2(g_libGTASA + 0x32706C, (uintptr_t) "\x1F\x0C\x00\x71", 4); // LIB_PointerGetWheel
    CHook::WriteMemory2(g_libGTASA + 0x3270AC, (uintptr_t) "\x1F\x0C\x00\x71", 4); // LIB_PointerDoubleClicked
    CHook::WriteMemory2(g_libGTASA + 0x327040, (uintptr_t) "\x1F\x0D\x00\x71", 4); // LIB_PointerGetButton
    /*CHook::WriteMemory(g_libGTASA + 0x320844, (uintptr_t)0x52800060, 4); // OS_PointerGetNumber
    CHook::WriteMemory(g_libGTASA + 0x320850, (uintptr_t) 0x71000F1F, 4); // OS_PointerGetType
	...
	// NVEventGetNextEvent
    CHook::HookFunc("_Z19NVEventGetNextEventP7NVEventi", NVEventGetNextEvent_hook, &NVEventGetNextEvent_hooked);
}