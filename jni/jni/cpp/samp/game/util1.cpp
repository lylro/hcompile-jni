...
CPhysicalGta* CUtil::GamePool_Object_GetAt(int slot)
{
    CPhysicalGta* (*GetPoolObj)(int slot);
    *(void **) (&GetPoolObj) = (void*)(g_libGTASA + (VER_x32 ? 0x00483DD2 + 1 : 0x575D30));
    return (GetPoolObj)(slot);
}
...
struct RwRaster* GetRWRasterFromBitmapPalette(uint8_t* pBitmap, size_t dwStride, size_t dwX, size_t dwY, uint8_t r, uint8_t g, uint8_t b, uint8_t a)
{
	...
	CHook::WriteMemory2(g_libGTASA + 0x0020A404, "\x03\x20", 2); // rwSTREAMMEMORY
    ...
    CHook::WriteMemory2(g_libGTASA + 0x0020A404, "\x02\x20", 2); // rwSTREAMFILENAME
...
{
    ((void (*)(RwMatrix*, RwMatrix*))(g_libGTASA+0x001E3420+1))(matIn, matOut);
}

void RwMatrixInvert(RwMatrix *matOut, RwMatrix *matIn)
{
    ((void (*)(RwMatrix*, RwMatrix*))(g_libGTASA+0x001E39A8+1))(matOut, matIn);
}
...
RwTexture* LoadTexture(const char *texname)
{
    static char *texdb[] = { "samp", "mobile", "playerhi", "txd", "menu", "gta3", "gta_int", "player", "flame" };
    ...
    for (int i = 0; i < 9; i++)
    {
        auto texture = CUtil::LoadTextureFromDB(texdb[i], str.c_str());
        if (texture != nullptr)
        {
...
void CUtil::drawText(const char* fmt, ...)
{
    char tempStr[4096];
    ...
    while(textCur < (tempStr + sizeof(tempStr)) && *textCur != '\0')
    {
        if (*textCur == '{')
        {
            ...
            ImVec4 textColor;
            if (CUtil::ProcessInlineHexColor(colorStart, textCur, textColor))
            {
...
void CUtil::drawText(ImVec2 pos, ImColor col, const char* szStr, const float font_size)
{
    char tempStr[4096];
...
bool CUtil::ProcessInlineHexColor(const char* start, const char* end, ImVec4& color)
{
...
void CUtil::drawText(ImVec2 pos, char* utf8string, uint32_t dwColor)
{
...
ImVec2 CUtil::CalcTextSizeWithoutTags(char* szStr, float font_size)
{
...
std::string CUtil::removeColorTags(std::string line)
{
    std::string string;
...
uintptr_t GetModelInfoByID(int iModelID)
{
    ...
    return reinterpret_cast<uintptr_t>(CModelInfo::ms_modelInfoPtrs[iModelID]);
}
void GetModelColSphereVecCenter(int iModel, CVector* vec)
{
    ...
    if (modelInfo)
    {
        ...
        if (colModel != 0) {
            CVector* v = (CVector*)(colModel + 0x18);
...
void DestroyAtomicOrClump(uintptr_t rwObject)
{
    if (rwObject)
    {
        ...
        {
            // RpAtomicDestroy
            ((void(*)(uintptr_t))(g_libGTASA + (VER_x32 ? 0x21416C + 1 : 0x2BA534)))(rwObject);

            uintptr_t parent = *(uintptr_t*)(rwObject + 4);
            if (parent)
            {
                // RwFrameDestroy
                ((void(*)(uintptr_t))(g_libGTASA + (VER_x32 ? 0x1D83EC + 1 : 0x26FD08)))(parent);
            }
        ...
        {
            // RpClumpDestroy
            ((void(*)(uintptr_t))(g_libGTASA + (VER_x32 ? 0x21458C + 1 : 0x2BAAB0)))(rwObject);
        }
    }
}