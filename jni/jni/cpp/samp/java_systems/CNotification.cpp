...
void CNetGame::packetNotification(Packet* p)
{
    ...
    Log("NOTIF: %s", utf_str);
    Log("%d | %d | %s | %d | %d", type, len, utf_str, time, actionId);
...
Java_com_criminal_moscow_gui_Notification_sendClick(JNIEnv *env, jobject thiz, jint action_id,
                                                  jint button_id) {
...
extern "C"
JNIEXPORT void JNICALL
Java_com_criminal_moscow_gui_Notification_init(JNIEnv *env, jobject thiz) {
