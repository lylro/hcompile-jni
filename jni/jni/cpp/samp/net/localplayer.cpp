    ...
    if(m_bIsActive && m_pPlayerPed) {
        ...
        if (m_pPlayerPed->m_pPed->bInVehicle)
        {
            ...
            if ((dwThisTick - m_dwLastSendTick) > (unsigned int) GetOptimumOnFootSendRate()) {
                ...
                CTask* task = CTaskManager::FindActiveTaskByType(&m_pPlayerPed->m_pPed->pPedIntelligence->m_TaskMgr, 0x2BD);
                if(task) {
                    CVehicleGta* gtaveh = (CVehicleGta *) (VER_x32 ? task->field_C : task->field_C);
                    if (gtaveh) {
                        uint16_t id = pNetGame->GetVehiclePool()->findSampIdFromGtaPtr(gtaveh);
                        Log("%d", id);
                        if (id != 65535) {
                            SendEnterVehicleNotification(id, 0);
                        }
                    }
                }
            ...
            if (!IS_TARGETING(wKeys) && !IS_FIRING(wKeys)) {
                ...
                // Targeting only. Just synced for toggle really, so use a slower rate
...
#else
    *(uint8_t*)(g_libGTASA + 0x00982111) = 0;
#endif
    m_pPlayerPed->SetInterior(0, false);
...
void CLocalPlayer::ProcessSpectating()
{
    ...
    else if(m_byteSpectateType == SPECTATE_TYPE_PLAYER)
    {
        if(pPlayerPool->m_pPlayers[m_SpectateID])
        {
            ...
            {
#if !VER_x32
                auto* TheCamera = (uintptr_t*)(g_libGTASA + 0xBBA8D0);
#else
...
void CLocalPlayer::MaybeSendExitVehicle(){
    CTask* task = CTaskManager::FindActiveTaskByType(&m_pPlayerPed->m_pPed->pPedIntelligence->m_TaskMgr, 0x2C0);
    if(task){
        CPlayerPed *pPed = GetPlayerPed();
        if(!pPed) return;

        uint16_t VehicleID = pPed->GetCurrentSampVehicleID();
        if(VehicleID == INVALID_VEHICLE_ID) return;
...
void CLocalPlayer::ProcessGuiVisible(){
    ...
    if(CDialog::bIsShow || pGame->isCasinoDiceActive || CTab::bIsShow || pGame->isAutoShopActive
       ...
       CMedic::bIsShow || bFirstSpawn || CEditobject::bIsToggle || CChip::bIsShow
       || CAucContainer::bIsShow || CAdminRecon::bIsToggle || CHUD::bIsCamEditGui || CDailyReward::bIsShow ||
       CTechInspect::bIsShow || CBaccarat::bIsShow || CAuthrization::bIsShow || CRegistration::bIsShow || CGameActionMenu::bIsShow || !CJavaGui::drawHud)
    {
        needDrawableHud = false;
    }

    if(!CJavaGui::drawChat) {
