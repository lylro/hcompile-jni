...
void CNetGame::Process()
{
	...
	{
		CChatWindow::AddDebugMessageNonFormatted("{bbbbbb}Подключение к серверу...{ffffff}");
	...
	while(pkt = m_pRakClient->Receive())
	{
		...
		switch(packetIdentifier)
		{
			...
			case ID_INVALID_PASSWORD:
				CChatWindow::AddDebugMessage("Не верный пароль!");
				m_pRakClient->Disconnect(0);
				break;
