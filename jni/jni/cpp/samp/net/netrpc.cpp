...
void EditAttachedObject(RPCParameters *rpcParams) {
    ...
    Log(OBFUSCATE("RPC: EditAttachedObject %d"), index);
}
void InitGame(RPCParameters *rpcParams)
{
    ...
    bsInitGame.ReadCompressed(pNetGame->m_bShowPlayerTags);						// native ShowNameTags(toggle); +
...
void Weather(RPCParameters *rpcParams)
{
    ...
    Log(OBFUSCATE("Weather: %d"), byteWeather);
...
{
    Log(OBFUSCATE("WorldPlayerRemove"));
    unsigned char * Data = reinterpret_cast<unsigned char *>(rpcParams->input);
    int iBitLength = rpcParams->numberOfBitsOfData;
...
{
    Log(OBFUSCATE("WorldVehicleRemove"));
    auto Data = reinterpret_cast<unsigned char *>(rpcParams->input);
    int iBitLength = rpcParams->numberOfBitsOfData;
...
void EnterVehicle(RPCParameters *rpcParams)
{
    ...
    if(pPlayerPool)
    {
        ...
        if(pPlayer) {
            Log(OBFUSCATE("RPC_EnterVehicle VehicleID = %d, playerId = %d,  bytePassenger = %d"),
...
{
    Log(OBFUSCATE("GameModeRestart"));
    // CChatWindow::AddInfoMessage("The server is restarting..");
    CChatWindow::AddInfoMessage(OBFUSCATE("{ff0000}?????? ???????????????..."));
...
{
    Log(OBFUSCATE("ConnectionRejected"));
    unsigned char * Data = reinterpret_cast<unsigned char *>(rpcParams->input);
    int iBitLength = rpcParams->numberOfBitsOfData;
    ...
    if(byteRejectReason == REJECT_REASON_BAD_VERSION)
        CChatWindow::AddInfoMessage("?????????? ?????????. ?????? ?? ???????? ??????????.");
    else if(byteRejectReason == REJECT_REASON_BAD_NICKNAME)
    {
        CChatWindow::AddInfoMessage("?????????? ?????????. ???????????? ???!");
        CChatWindow::AddInfoMessage("??????????, ???????? ?????? ??? ?? 5-20 ????????");
        CChatWindow::AddInfoMessage("?????????? ?????? A-Z a-z 0-9 [] ??? _");
        CChatWindow::AddInfoMessage("??????????? /quit ??? ?????? ??? ??????? ESC ? ???????? ????? ?? ????");
    }
...
{
    Log(OBFUSCATE("Create3DTextLabel"));
    unsigned char * Data = reinterpret_cast<unsigned char *>(rpcParams->input);
    int iBitLength = rpcParams->numberOfBitsOfData;
...
{
    Log(OBFUSCATE("Delete3DTextLabel"));
    unsigned char * Data = reinterpret_cast<unsigned char *>(rpcParams->input);
    int iBitLength = rpcParams->numberOfBitsOfData;
...
{
    Log(OBFUSCATE("ProcessIncommingEvent"));
    uint32_t v;
    int iVehicleID;
    int iPaintJob;
    int iComponent;
...
//		pGame->RequestModel(iComponent, STREAMING_GAME_REQUIRED);
//		pGame->LoadRequestedModels();
            ...
            if (!iWait)
            {
                CChatWindow::AddDebugMessage(OBFUSCATE("Timeout on car component."));
                break;
            }
...
{
#if !VER_x32
    return (void*) * (uintptr_t*)(g_libGTASA + 0x00BC3BD0);
#else
...
{
#if !VER_x32
    return (void*) * (uintptr_t*)(g_libGTASA + 0x00BC3BC8);
#else
...
{
#if !VER_x32
    return (void*) * (uintptr_t*)(g_libGTASA + 0x00BC3BD8);
#else
...
{
#if !VER_x32
    uintptr_t* numOccluders = (uintptr_t*)(g_libGTASA + 0x00CE8538);
#else
    ...
    {
#if !VER_x32
        char* v5 = (char*)(g_libGTASA + 0x00CE8538 + 8);
#else
...
{
    Log(OBFUSCATE("SetPlayerChatBubble"));
    uint8_t* Data = reinterpret_cast<uint8_t*>(rpcParams->input);
    int iBitLength = rpcParams->numberOfBitsOfData;
    //PlayerID sender = rpcParams->sender;
...
{
    Log(OBFUSCATE("WorldPlayerDeath"));
    uint8_t* Data = reinterpret_cast<uint8_t*>(rpcParams->input);
    int iBitLength = rpcParams->numberOfBitsOfData;
...
{
    Log(OBFUSCATE("WorldActorRemove"));
...
void SetActorPos(RPCParameters* rpcParams)
{
    ...
    CActorPool* pActorPool = pNetGame->GetActorPool();
#ifdef _CDEBUG
    CChatWindow::AddDebugMessage("Set actor to %d %f %f %f", actorId, to.x, to.y, to.z);
#endif
    if (pActorPool)
    {
        if (pActorPool->GetAt(actorId))
...
{
    auto Data = reinterpret_cast<uint8_t*>(rpcParams->input);
    int iBitLength = rpcParams->numberOfBitsOfData;
    ...
    CRemoveBuildings::addObjectToList(modelId, pos, radius);
}
