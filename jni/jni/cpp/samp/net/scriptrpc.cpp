...
void ScrClearPlayerAnimations(RPCParameters *rpcParams)
{
    ...
    if(pPlayerPool) {
        ...
        if(pPlayerPed)
        {
            ...
            //pPlayerPed->TeleportTo(mat.to.x, mat.to.y, mat.to.z);
        }
    }
}
...
//	Log("1| id: %d model: %d x: %f y: %f z: %f", wObjectID, ModelID, vecPos.x, vecPos.y, vecPos.z);
    iTotalObjects++;
    ...
    ReadObjectMaterialInfo(iMaterialCount, pObject, bsData);
}
...
void ScrSetPlayerAttachedObject(RPCParameters* rpcParams)
{
    ...
    Log("ScrSetPlayerAttachedObject %d %f %f %f", slot, info.vecOffset.x, info.vecOffset.y, info.vecOffset.z);
}
...
uintptr_t GetTextureFromTXDStore(const char* szTxd, const char* szTexture)
{
...
#if !VER_x32
    int iTXD = ((int(*)(const char*))(g_libGTASA + 0x006F8EA0))(szTxd); //FindTxDSlot
	if (iTXD == -1)
	{
		Log("no txd");
		return 0;
	}
	((int(*)())(g_libGTASA + 0x006F92A8))(); // pushCurrentTXD
	((int(*)(int, int))(g_libGTASA + 0x006F91EC))(iTXD, 0); // setCurrentTXD
	uintptr_t tex = ((uintptr_t(*)(const char*, const char*))(g_libGTASA + 0x002742C8))(szTexture, 0); // rwTextureRead
	((int(*)())(g_libGTASA + 0x006F92D4))(); // popCurrentTXD
	return tex;
    ...
    ((int(*)(int, int))(g_libGTASA + 0x005D4144 + 1))(iTXD, 0); // setCurrentTXD
...
void ScrSetObjectMaterial(RPCParameters* rpcParams)
{
    ...
    if (byteMaterialType == 1)
    {
        ...
        if (wModelID < 0 || wModelID > 20000)
        {
            wModelID = 18631;
        }
        if (!strncmp(texname, "wall8", sizeof("wall8"))) strcpy(texname, "samp");

        if (!CGame::IsModelLoaded(wModelID))
        {
            CStreaming::RequestModel((int)wModelID, STREAMING_KEEP_IN_MEMORY);
            pGame->LoadRequestedModels();

            int trying = 0;
            while (!CGame::IsModelLoaded(wModelID)) {
                trying++;
                if(trying > 10) {
                    Log("WARNING! NO LOAD OBJ MODEL = %d", wModelID);
                    break;
                    // mb assert?
                }
                usleep(200);
...
void RegisterScriptRPCs(RakClientInterface* pRakClient)
{
    ...
    pRakClient->RegisterAsRemoteProcedureCall(&RPC_ScrSetPlayerObjectMaterial, ScrSetObjectMaterial);
