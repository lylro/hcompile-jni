...
extern "C"
JNIEXPORT void JNICALL
Java_com_criminal_moscow_gui_Menu_sendRPC(JNIEnv *env, jobject thiz, jint type, jbyteArray str,
										jint action) {
	switch(type) {
		case 1:
			switch(action) {
...
extern "C"
JNIEXPORT void JNICALL
Java_com_criminal_moscow_SnapShot_00024Companion_nativeEntitySnapAddToQueue(JNIEnv *env, jobject thiz,
                                                                      jint id, jobject image_view,
                                                                      jint type, jint modelid,
                                                                      jint color1, jint color2,
                                                                      jfloat zoom, jfloat off_set_x,
                                                                      jfloat off_set_y,
                                                                      jfloat off_set_z, jfloat offset_x,
                                                                      jfloat offset_y,
                                                                      jfloat offset_z) {
    // TODO: implement nativeEntitySnapAddToQueue()

    Log("StartProcessng modelId %d", modelid);
    CSnapShotWrapper::queueMutex.lock();
    auto *queues = new CSnapShotWrapper::QueueItem();
    //CSnapShotWrapper::queue.front();
    queues->jid = id;
    queues->type = type;
    queues->id = modelid;
    queues->ImageView = env->NewGlobalRef(image_view);
    queues->color1 = color1;
    queues->color2 = color2;
    queues->zoom = zoom;
    queues->vecRot.x = off_set_x;
    queues->vecRot.y = off_set_y;
    queues->vecRot.z = off_set_z;
    queues->vecOffset = CVector{ offset_x, offset_y, offset_z };
    CSnapShotWrapper::isProcessing = true;
    CSnapShotWrapper::list.push(queues);
    // CSnapShotWrapper::queue.pop();
