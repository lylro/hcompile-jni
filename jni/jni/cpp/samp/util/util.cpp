...
RwTexture* CUtil::GetTexture(const char* name)
{
    ...
    if (!tex)
    {
        auto dw_handle = TextureDatabaseRuntime::GetDatabase("gta3");
        if(dw_handle) {
            TextureDatabaseRuntime::Register(dw_handle);

            tex = CUtil::GetTexture("oak2");
            if ( !tex )
                Log("Error: Texture (%s) not found in database (%s)", "gta3", "oak2");
        }else{
            Log("Error: Database not found! (%s)", "gta3");
            tex = nullptr;
        }
