...
uint32_t lasttickmicro = GetTickCount();
void Plugin::MainLoop()
{
    ...
    Plugin::RenderVoiceButton();
    if(!Plugin::muteStatus)
    {
        if(Plugin::recordStatus)
        {
            if(GetTickCount() - lasttickmicro >= 20000) {
                LogVoice("[sv:dbg:plugin] : activation key(%hhu) released timeout", defaultKey);
                ...
                if (!Network::SendControlPacket(SV::ControlPacketType::releaseKey,
                                                &releaseKeyPacket, sizeof(releaseKeyPacket)))
                    LogVoice("[sv:err:main:HookWndProc] : failed to send ReleaseKey packet");
                lasttickmicro = GetTickCount();
            }
        }
    }
    ...
    switch(controlPacket.packet)
    {
        ...
        case SV::ControlPacketType::createLPStream:
        {
            ...
            LogVoice("[sv:dbg:plugin:createlpstream] : "
                "stream(%p), dist(%.2f), to(%.2f;%.2f;%.2f), color(0x%x), name(%s)",
        ...
        case SV::ControlPacketType::updateLPStreamPosition:
        {
            ...
            LogVoice("[sv:dbg:plugin:updatelpstreamcoords] : stream(%p), to(%.2f;%.2f;%.2f)",
...
void Plugin::RenderVoiceButton(){
    ...
    if(CHUD::bIsShow) {
        ...
        ImGui::PushStyleColor(ImGuiCol_ButtonHovered,
                              (ImVec4) ImColor(0x00, 0x00, 0x00, 0x00).Value);
        ...
        auto *widget = CGtaWidgets::m_pWidgets[8];

        float v3 = widget->m_RectScreen.left;//widget[8];
        float v4 = widget->m_RectScreen.right - v3;//widget[10] - v3;

        ImVec2 pos2 = ImVec2((float) v3 , widget->m_RectScreen.bottom - 7.0f);
        ImVec2 size2 = ImVec2(v4, v4);

        ImGui::Begin("VoiceButton", nullptr,
        ...
        if (!Plugin::muteStatus) {
            if (!Plugin::recordStatus) {
                if (ImGui::ImageButton((ImTextureID) MicroIcon::tPassiveIcon->raster, size2)) {
                    lasttickmicro = GetTickCount();
                    ...
                    if (!Network::SendControlPacket(SV::ControlPacketType::pressKey,
                                                    &pressKeyPacket, sizeof(pressKeyPacket)))
                        LogVoice("[sv:err:main:HookWndProc] : failed to send PressKey packet");
                }
            } else {
                if (ImGui::ImageButton((ImTextureID) MicroIcon::tActiveIcon->raster, size2)) {
            ...
            if (ImGui::ImageButton((ImTextureID) MicroIcon::tMutedIcon->raster, size2)) {
