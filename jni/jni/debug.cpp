...
void CDebugInfo::Draw()
{
	...
	{ // wеiktоn in your heart
		...
		if (pGame->FindPlayerPed()->m_pPed)
		{
			...
			snprintf(&szStrPos[0], 256, OBFUSCATE("%.4f, %.4f, %.4f"), matFromPlayer.pos.X, matFromPlayer.pos.Y, matFromPlayer.pos.Z);
			pos = ImVec2(pGUI->ScaleX(40.0f), pGUI->ScaleY(1080.0f - pGUI->GetFontSize() * 8));
			pGUI->RenderText(pos, (ImU32)0xFFFFFFFF, true, &szStrPos[0]);

	                                    snprintf(&szStr[0], 256, OBFUSCATE("Peds: %d, Vehicles: %d"), uiStreamedPeds, uiStreamedVehicles);
		                  pos = ImVec2(pGUI->ScaleX(40.0f), pGUI->ScaleY(1080.0f - pGUI->GetFontSize() * 12));
	                                    pGUI->RenderText(pos, (ImU32)0xFFFFFFFF, true, &szStr[0]);
		}
	} else *(uint8_t*)(g_libGTASA + 0x8ED875) = 0;
}

void CDebugInfo::ApplyPatches()
{
                  // Draw distance
//                  UnFuck(g_libGTASA +  0x3BDDF4);
  //                *(uint32_t *)(g_libGTASA + 0x3BDDF4) = 0x41C80000; // 0x41C80000;

//                  UnFuck(g_libGTASA +  0x3BE2D0);
  //               *(uint32_t *)(g_libGTASA + 0x3BE2D0) = 0x40A00000; // 0x40A00000;
}
...
void CDebugInfo::ProcessSpeedMode(VECTOR* pVecSpeed)
{
	...
	if (speed >= 1.0f)
	{
		if (!m_dwSpeedStart)
		{
			...
			m_dwState = 0;
			pChatWindow->AddDebugMessage(OBFUSCATE("Start"));
		}
		if ((speed >= 119.0f) && (speed <= 121.0f) && (m_dwState == 0))
		{
			pChatWindow->AddDebugMessage(OBFUSCATE("1 to 100: %d"), GetTickCount() - m_dwSpeedStart);
			m_dwSpeedStart = GetTickCount();
			m_dwState = 1;
		}
		if ((speed >= 230.0f) && (speed <= 235.0f) && (m_dwState == 1))
		{
			pChatWindow->AddDebugMessage(OBFUSCATE("100 to 200: %d"), GetTickCount() - m_dwSpeedStart);
			m_dwSpeedStart = 0;
			m_dwState = 0;
		}
