...
void CDialogWindow::RenderTabList(int dStyle)
{
	...
	ImVec2 winSize(vecWinSize.x, pGUI->ScaleY(vecWinSize.y + dStyle == DIALOG_STYLE_TABLIST_HEADERS ? pGUI->ScaleY(187.5f) / 2 : 0));
	ImGui::SetWindowSize(winSize);
	ImGui::SetWindowPos(ImVec2(((io.DisplaySize.x - winSize.x) / 2), ((io.DisplaySize.y - winSize.y) / 2)));
	//

	/*if (pGUI->ScaleX(ImGui::GetWindowWidth()) > 1800.0f)
		ImGui::SetWindowSize(ImVec2(pGUI->ScaleX(1800.0f), ImGui::GetWindowHeight()));*/
...
void CDialogWindow::Render()
{
	...
	ImGui::Begin(" ", nullptr,
		ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoScrollbar);
