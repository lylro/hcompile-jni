...
void CDialogWindow::SetInfo(char* szInfo, int length)
{
	...
	std::string sinf(szInfo);
	int i = sinf.find("\x0A\x0A");
	...
	Log("puutf8 - %s", m_putf8Info);
	Log("m_pszInfo - %s", m_pszInfo);
}
...
void CDialogWindow::RenderTabList(int dStyle)
{
    ...
    ImGui::Begin(" ", nullptr , ImVec2( pGUI->ScaleX(vecWinSize.x), pGUI->ScaleY(vecWinSize.y) ) , 0.9f, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar ); //
	m_bRendered = true;
	...
	ImGui::BeginChild( "###tablist", ImVec2(-1, pGUI->ScaleY(vecWinSize.y)-pGUI->ScaleY(95.0f*2) ), true, ImGuiWindowFlags_NoScrollbar);
