...
EGLBoolean hook_eglSwapBuffers(EGLDisplay dpy, EGLSurface surface)
{
    ...
    // --- Рендерим обычные окна ---
    CApp::Render();

    // --- Добавляем свою кнопку ImGui ---
    ImGui::Begin("Custom GUI"); // Можно скрыть окно, если нужно
    ImGui::PushID(2); // Уникальный ID = 2
    if (ImGui::Button("btn_donat_store")) {
        // Лог в чат
        CChat::AddDebugMessage(xorstr("Кнопка btn_donat_store нажата! ID = 2"));
        // Или FLog("btn_donat_store pressed!");
    }
    ImGui::PopID();
    ImGui::End();
