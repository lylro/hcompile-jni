...
struct CSprite2d* CCustomPlateManager::m_pTransitSprite = nullptr;
...
RwTexture* CCustomPlateManager::ProcessBuPlate(SCustomPlate& plate)
{
	...
	pText->Draw(40.0f, 2.0f, 214.0f, 66.0f, white);
	RwTexture* pTexture = m_pRenderTarget->End();
...
RwTexture* CCustomPlateManager::ProcessKzPlate(SCustomPlate& plate)
{
	...
	pText->m_pRwTexture = ((struct RwTexture* (*)(struct RwRaster*))(SA_ADDR(0x1B1B4C + 1)))(pTextRaster); // RwTextureCreate
	pRegion->m_pRwTexture = ((struct RwTexture* (*)(struct RwRaster*))(SA_ADDR(0x1B1B4C + 1)))(pRegionRaster); // RwTextureCreate
	...
	pText->Draw(48.0f, 8.0f, 154.0f, 48.0f, white);
...
RwTexture* CCustomPlateManager::ProcessRuPlate(SCustomPlate& plate)
{
	...
	pText->Draw(22.0f, 12.0f, 160.0f, 44.0f, white); // x y width height ÒÅÊÑÒ
	pRegion->Draw(198.0f, 14.0f, 38.0f, 24.0f, white); // x y width height ÐÅÃÈÎÍ
	RwTexture* pTexture = m_pRenderTarget->End();
...
RwTexture* CCustomPlateManager::ProcessTransitPlate(SCustomPlate& plate)
{
	if (!m_pTransitSprite)
	{
		return nullptr;
	}
	if (!m_pTransitSprite->m_pRwTexture)
	{
		return nullptr;
	}
	...
	m_pRenderTarget->Begin();
	m_pTransitSprite->Draw(0.0f, 0.0f, 256.0f, 64.0f, white);
	RwTexture* pTexture = m_pRenderTarget->End();
...
void CCustomPlateManager::Initialise()
{
	...
	m_pTransitSprite = new CSprite2d();
	...
	m_pTransitSprite->m_pRwTexture = (RwTexture*)LoadTextureFromDB("samp", "plate_transit");
	...
	for (auto& i : m_Plates)
	{
		...
		if (pPlate)
		{
			if (pNetGame->GetVehiclePool())
			{
				...
				{
					pNetGame->GetVehiclePool()->GetAt(i.vehicleId)->ApplyTexture("remap_plate_c", pPlate);
