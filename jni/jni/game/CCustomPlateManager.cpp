...
{
	if(strlen(plate.szNumber) < 2){return ProcessNoPlatePlate(plate);}else{
	if (!m_pRuSprite)
	{
		return nullptr;
	}
...
void CCustomPlateManager::Initialise()
{
	...
	m_pRenderTarget = new CRenderTarget(256, 64, true, 000000);
		...
		if (pPlate)
		{
			if (pNetGame->GetVehiclePool())
			{
				if (pNetGame->GetVehiclePool()->GetAt(i.vehicleId))
				{
					...
					pNetGame->GetVehiclePool()->GetAt(i.vehicleId)->ApplyTexture("platem_remap", pPlate);
                                                                                          ...
                                                                                          //pNetGame->GetVehiclePool()->GetAt(i.vehicleId)->ApplyTexture("remaptoner", "bleft"); //remapvehicles
                                                                                          //pNetGame->GetVehiclePool()->GetAt(i.vehicleId)->ApplyTexture("remapvehicles", "remapbody14"); //remapvehicles
				}
			}
		}
	}
