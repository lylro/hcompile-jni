...
void CInfoBarText::DrawHealth(int fHP)
{
	if (!m_bIsEnabled || !pGUI) return;
	if (!pGame->IsToggledHUDElement(HUD_ELEMENT_HUD) || pDialogWindow->m_bIsActive) return;
	...
	CFont::SetScale((float)CAdjustableHudScale::GetElementScale(E_HUD_ELEMENT::HUD_HP_TEXT).X / 100.0f);
...
void CInfoBarText::DrawArmour(int fArmour)
{
	if (!m_bIsEnabled || !pGUI) return;
	if (!pGame->IsToggledHUDElement(HUD_ELEMENT_HUD) || pDialogWindow->m_bIsActive) return;

	CRGBA color = CAdjustableHudColors::GetHudColor(E_HUD_ELEMENT::HUD_ARMOR_TEXT);
	CRGBA black;
	black.A = 0;
	black.R = 0;
	black.G = 0;
	...
	CFont::SetScale((float)CAdjustableHudScale::GetElementScale(E_HUD_ELEMENT::HUD_ARMOR_TEXT).X / 100.0f);
