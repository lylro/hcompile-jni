...
{
	m_pDiscTexture = (RwTexture*)LoadTextureFromDB(OBFUSCATE("txd"), OBFUSCATE("radardisc"));
	m_pRectTexture = (RwTexture*)LoadTextureFromDB(OBFUSCATE("txd"), OBFUSCATE("radardisc_r"));
}
...
void CRadarRect::ChangeTextures(uint8_t RadarType, const char* TextureName)
{
	...
	{
		SetEnabled(true);
		m_pRectTexture = nullptr;
		m_pRectTexture = (RwTexture*)LoadTextureFromDB(CMain::GetCustomStorage(), TextureName);
		if (!m_pRectTexture) m_pRectTexture = (RwTexture*)LoadTextureFromDB("txd", "radardisc_r");
	}
	else 
	{
		SetEnabled(false);
		m_pDiscTexture = nullptr;
		m_pDiscTexture = (RwTexture*)LoadTextureFromDB(CMain::GetCustomStorage(), TextureName);
		if (!m_pDiscTexture) m_pDiscTexture = (RwTexture*)LoadTextureFromDB("txd", "radardisc");
	}
	return;
}
