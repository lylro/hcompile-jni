...
void CRenderTarget::ProcessCamera()
{
	...
	((struct RwCamera* (*)(struct RwCamera*, uint32_t*, int))(g_libGTASA + 0x1AD8A0 + 1))(m_pCamera, &m_color, 3);
	...
	else
	{
		...
		RwRenderStateSet(rwRENDERSTATEFOGCOLOR, (void*)&m_color);
...
CRenderTarget::CRenderTarget(uint32_t dwResultSizeX, uint32_t dwResultSizeY, bool b2D, uint32_t color)
{
...
RwTexture* CRenderTarget::End()
{
	...
	struct RwTexture* pTexture = ((struct RwTexture*(*)(struct RwRaster*))(g_libGTASA + 0x1B1B4C + 1))(*(RwRaster**)((uintptr_t)m_pCamera + 0x60));
