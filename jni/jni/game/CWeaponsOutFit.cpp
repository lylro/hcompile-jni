...
uint32_t CWeaponsOutFit::m_bLastRealeseModel[10];
...
void CWeaponsOutFit::OnWeaponAdded(CPlayerPed* pPed, int iWeaponID)
{
	...
	/*if (GetTickCount() - m_bLastRealeseModel[iSlot] > 5000)
	{
		m_bLastRealeseModel[iSlot] = GetTickCount();
...
void CWeaponsOutFit::ParseDatFile()
{
	...
	snprintf(buff, sizeof(buff), OBFUSCATE("%sSAMP/outfit.dat"), g_pszStorage);

	FILE* pFile = fopen(buff, OBFUSCATE("r"));
...
void CWeaponsOutFit::ProcessLocalPlayer(CPlayerPed* pPed)
{
	...
	for (int i = 0; i < 13; i++)
	{
		...
		WEAPON_SLOT_TYPE* iCurrentWeapon = pPed->GetCurrentWeaponSlot();
		if (iWeapon && iWeapon != iCurrentWeapon->dwType)
