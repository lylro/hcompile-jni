...
public:
    // Initializes pool
    CPool(uint32_t size, const char* name) {
    ...
    void Init(uint32_t size, void* objects, void* infos) {
    ...
    */
    ...
    uint32_t GetRef(const A* obj) {
        const auto idx = GetIndex(obj);
        return (idx << 8) + m_byteMap[idx].IntValue();
    }

    // Returns pointer to object by SCM handle (ref)
    A* GetAtRef(uint32_t ref) {
        uint32_t idx = ref >> 8; // It is possible the ref is invalid here, thats why we check for the idx is valid below (And also why GetIndexFromRef isn't used, it would assert)
        return IsIndexInBounds(idx) && m_byteMap[idx].IntValue() == (ref & 0xFF)
               ? reinterpret_cast<A*>(&m_pObjects[idx])
               : nullptr;
    }

    A* GetAtRefNoChecks(uint32_t ref) {
    ...
    */
    ...
    [[nodiscard]] bool IsIndexInBounds(uint32_t idx) const {
    ...
    uint32_t GetIndexFromRef(uint32_t ref) {
