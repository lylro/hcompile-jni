...
uint32 CGeneral::GetNodeHeadingFromVector(float x, float y) {
    ...
    angle = TWO_PI - angle + CUtil::DegreesToRadians(22.5f);

    if (angle >= TWO_PI)
        angle -= TWO_PI;

    return (uint32)floor(angle / CUtil::DegreesToRadians(45.0f));
}
...
uint32_t CGeneral::GetRandomNumber() { // Why does this return `uint16`? Should be `int32` (same as retval of `rand()`)
