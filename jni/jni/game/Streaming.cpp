...
bool CStreaming::TryLoadModel(int modelId) {
    if (modelId == 1082) {
        //pUI->chat()->addDebugMessage("{ff0000} Модель %d заблокирована!", modelId);
        return false;
    }

    auto& info = CStreaming::GetInfo(modelId);
    if (!info.IsLoaded()) {
        //pUI->chat()->addDebugMessage("{00ff00} Запрашивающая модель %d...", modelId);
        ...
        while (!info.IsLoaded()) {
            ...
            if (count > 30) {
                //pUI->chat()->addDebugMessage("{ff0000} Ошибка: модель %d не загрузилась вовремя!", modelId);
                return false;
            }
        }
        //pUI->chat()->addDebugMessage("{00ff00} Модель %d успешно загружена!", modelId);
    }
...
void CStreaming::LoadAllRequestedModels(bool bOnlyPriorityRequests) {
    ...
    CHook::CallFunction<void>("_ZN10CStreaming22LoadAllRequestedModelsEb", bOnlyPriorityRequests);
}
