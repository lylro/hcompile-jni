...
CWidgetGta* CGtaWidgets::m_pWidgets[190] = { nullptr };
void CGtaWidgets::setPositionAndScale(CWidgetGta* widget, float originX, float originY, float scaleX, float scaleY) {
    if(!widget) return;
    widget->m_fOriginX = originX;
    widget->m_fOriginY = originY;
    widget->m_fScaleX = scaleX;
    widget->m_fScaleY = scaleY;
}