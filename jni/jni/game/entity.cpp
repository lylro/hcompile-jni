...
void CEntity::Remove()
{
	...
	if (m_pEntity->dwUnkModelRel) WorldRemoveEntity((uintptr_t)m_pEntity);
}

void CEntity::RotateMatrix(PMATRIX4X4 Matrix, VECTOR vec)
{
	if (!m_pEntity || !m_pEntity->mat) return;
	
	float f1 = vec.X * 0.0174533;
	float f2 = vec.Y * 0.0174533;
	float f3 = vec.Z * 0.0174533;
	...
	Matrix->right.X = f3cos * f2cos - (f3sin * f1sin) * f2sin;
	Matrix->right.Y = (f3cos * f1sin) * f2sin + f3sin * f2cos;
	Matrix->right.Z = -(f2sin * f1cos);

	Matrix->up.X = -(f3sin * f1cos);
	Matrix->up.Y = f3cos * f1cos;
	Matrix->up.Z = f1sin;
	
	Matrix->at.X = (f3sin * f1sin) * f2cos + f3cos * f2sin;
	Matrix->at.Y = f3sin * f2sin - (f3cos * f1sin) * f2cos;
	Matrix->at.Z = f2cos * f1cos;
}
...
bool CEntity::SetModelIndex(unsigned int uiModel)
{
	...
	{
		Log("SetModelIndex (uiModel): %d", uiModel);
		...
		while(!pGame->IsModelLoaded(uiModel))
		{
			...
			if(iTryCount > 20)
			{
				...
				Log(OBFUSCATE("Warning: Model %u wouldn't load in time!"), uiModel);
				return false;
			}
...
void CEntity::SetSuperLowLOD(int bCheck)
{
	if(!m_pEntity) {
		return;
	}
	...
	if (m_pEntity->nEntityFlags.m_bDisplayedSuperLowLOD == bCheck) m_pEntity->nEntityFlags.m_bDisplayedSuperLowLOD = !bCheck;
	if (m_pEntity->nEntityFlags.m_bCollisionProcessed == !bCheck) m_pEntity->nEntityFlags.m_bCollisionProcessed = bCheck;
}