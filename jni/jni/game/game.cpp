...
bool CGame::IssetModelInFiles(int32_t modelid){
	return *(uint32_t*)(g_libGTASA + 0x6706D8 + 0x14 * modelid + 0xC) != 0;
}
...
void CGame::InitInGame()
{
	...
	m_bUseNotFoundModel = IssetModelInFiles(18631);
}
...
void CGame::SetCheckpoint(VECTOR *pos, VECTOR *extent)
{
	...
	ScriptCommand(&create_sphere, pos->X, pos->Y, pos->Z, extent->X, &m_checkpointData.m_dwSphereId);
...
void CGame::DisableCheckpoint()
{
	if (m_checkpointData.m_bIsActive)
	{
		...
		if(m_checkpointData.m_dwSphereId){
		    ScriptCommand(&remove_sphere, &m_checkpointData.m_dwSphereId);
		}
...
uint32_t m_dwLastUpdateTime = GetTickCount();
void CGame::SetWorldTime(int iHour, int iMinute)
{
	CPlayerPed *pPlayerPed = this->FindPlayerPed();
	if (pPlayerPed && pPlayerPed->GetUpdatePlayer())
	{
		if (GetTickCount() - m_dwLastUpdateTime > 4000)
			m_dwLastUpdateTime = GetTickCount();
		else
			return;
	}
	else if (iHour == currentHour) return;
...
/*
// 0.3.7
bool CGame::RequestModel(unsigned int &iModelID, int iLoadingStream) 
{
	bool issetModel = IssetModelInFiles(iModelID);
	if(!issetModel) {
		pChatWindow->AddDebugMessage(OBFUSCATE("Warning: not found model id:%d"), iModelID);
		if(m_bUseNotFoundModel) iModelID = 18631;
		else return false;
	}
...
/*
const int MAX_MODEL_IDS = 200000;
static int isCheckedModel[MAX_MODEL_IDS] = { -1 }; 

bool CGame::RequestModel(unsigned int& iModelID, int iLoadingStream)
{
    bool issetModel = true; 
    if (iModelID >= MAX_MODEL_IDS) 
    {
        pChatWindow->AddDebugMessage(OBFUSCATE("Error: model id:%d is out of range"), iModelID);
        return false;
    }
    if (isCheckedModel[iModelID] == -1)
    {
        issetModel = IssetModelInFiles(iModelID);
        if (!issetModel)
        {
            isCheckedModel[iModelID] = 0;
            pChatWindow->AddDebugMessage(OBFUSCATE("Warning: not found model id:%d"), iModelID);
        }
        else isCheckedModel[iModelID] = 1;
    }

    if(isCheckedModel[iModelID] == 0) 
    {
        issetModel = false;
        pChatWindow->AddDebugMessage(OBFUSCATE("Warning 2: not found model id:%d"), iModelID);
        if (m_bUseNotFoundModel)
            iModelID = 18631;
        else
            return false;
    }
...
bool CGame::RequestModel(uint16_t &iModelID, int iLoadingStream) 
{
	uint32_t modelx32 = (uint32_t)iModelID;
	bool res = RequestModel(modelx32, iLoadingStream);
	iModelID = modelx32;
	
	return res;
}

bool CGame::RequestModel(int &iModelID, int iLoadingStream) 
{
	uint32_t modelx32 = (uint32_t)iModelID;
	bool res = RequestModel(modelx32, iLoadingStream);
	iModelID = modelx32;
...
uint8_t CGame::IsModelLoaded(unsigned int iModelID)
{
	...
	Log("[IsModelLoaded] iModelID: %d", iModelID);
	...
	{
		Log("IsModelLoaded true!");
		return 1;
	}
	Log("IsModelLoaded false!");
	return 0;
