...
#include "CClothesTextureManager.h"
...
public:
	...
	//От малого
	// 0.3.7
	//bool RequestModel(unsigned int &iModelID, int iLoadingStream = 2);
	
	// 0.3.7
	//bool RequestModel(uint16_t &iModelID, int iLoadingStream = 2);
	
	// 0.3.7
	//bool RequestModel(int &iModelID, int iLoadingStream = 2);
	
	// 0.3.7
	void LoadRequestedModels();
	// 0.3.7
	uint8_t IsModelLoaded(unsigned int iModelID);
	void RequestVehicleUpgrade(uint16_t iModelId, uint8_t iLoadingStream = 0x2);
	uint8_t IsModelLoadedScript(unsigned int iModelID);
	// 0.3.7
	...
	// NOTSA
	bool IssetModelInFiles(int32_t modelid);
