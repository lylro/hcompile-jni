...
{	
	NvUtilInit();
	*(char**)(g_libGTASA + 0x5D1608) = OBFUSCATE("/storage/emulated/0/BlackRussia/");
}
uintptr_t* (*CCustomRoadsignMgr_RenderRoadsignAtomic)(uintptr_t*, CVector2D const&);
uintptr_t* CCustomRoadsignMgr_RenderRoadsignAtomic_hook(uintptr_t* atomic, CVector2D const& a2)
{
    return atomic;
}
void SetUpGLHooks();
...
//                 WriteMemory(g_libGTASA + 0x52DD38, (uintptr_t)"\x00\x20\x70\x47", 4); // CCoronas::RenderReflections
  //               NOP(g_libGTASA + 0x39AD14, 1); //render clouds, sunrefl, raineffect 
	...
	if (pAudioStreamManager) pAudioStreamManager->Process();
...
void DrawRadarTexture(float x, float y, float size1, float size2)
{
    ...
    m_pSprite->Draw(x, y, size1, size2, color_white);
}
...
void RenderEffects_hook()
{
    ...
    ((void(*)())(g_libGTASA + 0x54C330 + 1))();
    ((void(*)())(g_libGTASA + 0x54D738 + 1))();
}
...
int *(*CMissionCleanup__Process)(uintptr_t *thiz, unsigned int a2);
int CMissionCleanup__Process_hook(uintptr_t *thiz, unsigned int a2)
{
...
void InstallHooks()
{
    ...
    CHook::InlineHook(g_libGTASA, 0x2CE788, &CObject__ProcessGarageDoorBehaviour_hook, &CObject__ProcessGarageDoorBehaviour);
    ...
    //SetUpHook(g_libGTASA + 0x39AE28, (uintptr_t)RenderEffects_hook, (uintptr_t*)&RenderEffects);

    SetUpHook(g_libGTASA + 0x005311D0, (uintptr_t)CDraw__SetFOV_hook, (uintptr_t*)&CDraw__SetFOV);
    SetUpHook(g_libGTASA + 0x002E41C0, (uintptr_t)CMissionCleanup__Process_hook, (uintptr_t*)&CMissionCleanup__Process);
