...
{	
	...
	*(char**)(g_libGTASA + 0x5D1608) = СAndroid::GetBaseStorage();	
}
...
void InstallSpecialHooks()
{
	...
	SetUpHook(g_libGTASA + 0x241D94, (uintptr_t)NvUtilInit_hook, (uintptr_t*)&NvUtilInit);/* UpDate Android 11 */
			...
			if (pVoice)
			{
				...
				if (pWidget->GetState() == 2 && GetTickCount() - lastTick >= 250)
				{
					...
					if (pVoice->IsRecording())
					{
						...
						{
							pChatWindow->AddDebugMessage("Âû îòêëþ÷åíû îò ãîëîñîâîãî ÷àòà");
							pVoice->DisableInput();
						}
					}
...
RwFrame* CClumpModelInfo_GetFrameFromId_Post(RwFrame* pFrameResult, RpClump* pClump, int id)
{
	...
	// íå ïðîâåðÿåì åñëè íîëü ýòî íîðìà
...
void CMatrix__SetScale_hook(void* thiz, float x, float y, float z)
{
	if (g_pLastProcessedVehicleMatrix && g_iLastProcessedWheelVehicle != -1)
	{
		if (g_iLastProcessedWheelVehicle >= 2 || g_iLastProcessedWheelVehicle <= 7)
		{
			...
			{
				y *= g_pLastProcessedVehicleMatrix->m_fWheelSize * 1.3f; // åñëè óæå ïðèìåí¸í scale òî ïèçäåö
