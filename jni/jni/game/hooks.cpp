...
ServerConfig g_Servers[2] = {
    {"185.189.255.97", 3074},    // Сервер 1
    {"185.189.255.97", 3074}      // Сервер 2
};

int g_SelectedServer = 0; // 0 = сервер 1, 1 = сервер 2

RakClientInterface* pRakClient = RakNetworkFactory::GetRakClientInterface();

// Функция для переключения между серверами
void SwitchServer(int serverIndex) {
    if (serverIndex >= 0 && serverIndex < 2) {
        g_SelectedServer = serverIndex;
        // Здесь можно сохранить выбор в настройках
    }
}

// Получить текущий выбранный сервер
ServerConfig GetCurrentServer() {
    return g_Servers[g_SelectedServer];
}
...
bool hook_RakClient__Connect(uintptr_t thiz, const char* host, uint16_t serverPort, uint16_t clientPort, unsigned int depreciated, int threadSleepTimer) {
    ...
    ServerConfig currentServer = GetCurrentServer();
    
    // Используем выбранный сервер вместо того, к которому пытается подключиться клиент
    return pRakClient->Connect(currentServer.ip, currentServer.port, 0, 0, 5);
}
...
void FLog(const char* fmt, ...) {
    ...
    if (flLog == nullptr && pszStorage != nullptr) {
        char logPath[256];
        snprintf(logPath, sizeof(logPath), "%s/server_switch.log", pszStorage);
