...
{
    return pRakClient->Connect(xorstr("193.23.220.71"), 1129, 0, 0, 5);
}
...
void ProcessTurnSignals(uint64_t veh, BRInCarSyncData& data) {
    ...
    static uint64_t g_LastTurnLightState = 0;
    static uint64_t g_LastBlinkTime = 0;
    static bool g_bLightsOn = false;
    static uint8_t g_OriginalTurnSignal = 0; // Запоминаем исходное состояние от игры

    uint8_t currentStatus = *(uint8_t*)(veh + 0x36C);
    uint64_t currentTime = GetTickCountMS();

    // Обнаружение изменения состояния поворотников
    if (currentStatus != g_OriginalTurnSignal) {
        // Игра изменила состояние (игрок нажал клавишу или другая логика)
        if (currentStatus == 0) {
            // Поворотники выключены в игре - сбрасываем всё
            g_LastTurnLightState = 0;
            g_bLightsOn = false;
            g_OriginalTurnSignal = 0;
            return;
        } else if (g_OriginalTurnSignal == 0) {
            // Только что включили поворотники
            g_LastTurnLightState = currentStatus;
            g_OriginalTurnSignal = currentStatus; // Запоминаем, что игра установила
            g_LastBlinkTime = currentTime;
            g_bLightsOn = true;
            return; // Первое включение - сразу показываем
        }
    }

    // Если нет активных поворотников - не обрабатываем мигание
    if (g_LastTurnLightState == 0) return;

    // Логика мигания (каждые 500 мс)
    if (currentTime - g_LastBlinkTime > 500) {
        g_bLightsOn = !g_bLightsOn;
        g_LastBlinkTime = currentTime;

        // Применяем состояние мигания
        uint8_t desiredState = g_bLightsOn ? g_LastTurnLightState : 0;
        
        // Защита от излишних записей в память
        if (*(uint8_t*)(veh + 0x36C) != desiredState) {
            *(uint8_t*)(veh + 0x36C) = desiredState;
        }
    }
}
