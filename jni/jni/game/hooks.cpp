...
{
    return pRakClient->Connect("188.127.241.74", 1252, 0, 0, 5);
}
   ...
   //                 CChat::AddDebugMessage("Response not sended..");
    ...
    if (nlohmann::json::accept(buffGUI)) {
        ...
        return false;
            ...
            if (guiId == 33) { // Специальная обработка для инвентаря
                if (nlohmann::json::accept(CGUI::buffGUI)) {
                    nlohmann::json jsonObj = nlohmann::json::parse(CGUI::buffGUI);
                    nlohmann::json outputJson;

                    // Копируем основные данные
                    if (jsonObj.contains("it")) outputJson["it"] = jsonObj["it"];
                    if (jsonObj.contains("ai")) outputJson["ai"] = jsonObj["ai"];
                    if (jsonObj.contains("s")) outputJson["s"] = jsonObj["s"];
                    if (jsonObj.contains("actions")) outputJson["actions"] = jsonObj["actions"];
                    if (jsonObj.contains("i")) outputJson["i"] = jsonObj["i"];
                    if (jsonObj.contains("sl")) outputJson["sl"] = jsonObj["sl"];

                    std::string jsonString = outputJson.dump();
                    size_t jsonSize = jsonString.size();
                    FLog("Dumped JSON for GUI 33: %s, size: %zu", jsonString.c_str(), jsonSize);

                    RakNet::BitStream bsSend;
                    bsSend.Write((uint8_t)251); // Magic number
                    bsSend.Write((uint8_t)guiId);
                    bsSend.Write((uint16_t)jsonSize);
                    bsSend.Write(jsonString.c_str(), jsonSize);

                    bool result = pRakClient->Send(&bsSend, HIGH_PRIORITY, RELIABLE, 0);
                    if (result) FLog("GUI 33 data sent successfully!");
                    else FLog("Failed to send GUI 33 data!");
                    return true;
                } else {
                    FLog("Invalid JSON for GUI 33");
                }
            }
            // Обработка GUI 38
            else if (guiId == 38) {
                ...
                if (nlohmann::json::accept(buffGUI)) {
                    nlohmann::json jsonObj = nlohmann::json::parse(buffGUI);
                    nlohmann::json outputJson;
                    int8_t type = jsonObj.value<int8_t>("t", -1);
                    outputJson["t"] = type;

                    FLog("Received GUI 74: Type=%d", type);

                    switch (type) {
                        case 1: // Filter button pressed
                        {
                            int filterPressed = jsonObj.value<int>("fl", 0);
                            outputJson["fl"] = filterPressed;
                            FLog("Received GUI 74: Type=1, Filter=%d", filterPressed);
                            break;
                        }
                        case 2: // Request new items
                        {
                            int updateFlag = jsonObj.value<int>("u", 0);
                            outputJson["u"] = updateFlag;
                            FLog("Received GUI 74: Type=2, UpdateFlag=%d", updateFlag);
                            break;
                        }
                        case 3: // Cases button pressed
                        {
                            FLog("Received GUI 74: Type=3 (Cases button pressed)");
                            break;
                        }
                        case 4: // Item pressed
                        {
                            int id = jsonObj.value<int>("id", 0);
                            int buttonType = jsonObj.value<int>("s", 0);
                            outputJson["id"] = id;
                            outputJson["s"] = buttonType;
                            FLog("Received GUI 74: Type=4, ItemId=%d, ButtonType=%d", id, buttonType);
                            break;
                        }
                        case 5: // Update tanpin
                        {
                            FLog("Received GUI 74: Type=5 (Update tanpin)");
                            break;
                        }
                        default:
                            FLog("Unknown type %d for GUI 74", type);
                            return false;
                    }

                    if (jsonObj.contains("c") && jsonObj["c"] == 1) {
                        outputJson["c"] = 1;
                        FLog("Received GUI 74: Close screen requested");
                    }
                    ...
                    bsSend.Write((uint8_t)74);
                    bsSend.Write((uint16_t)jsonSize);
                    bsSend.Write(jsonString.c_str(), jsonSize);

                    bool result = pRakClient->Send(&bsSend, HIGH_PRIORITY, RELIABLE, 0);
                    if (!result) {
                        FLog("Failed to send GUI 74 data!");
                    } else {
                        FLog("Data sent successfully! Payload: '%s' (size: %zu bytes)", jsonString.c_str(), jsonSize);
                    }
                } else {
                    FLog("Failed to parse JSON for GUI 74");
                    return false;
                }
            }
        }
