...
{
    const char* allowedIPs[] = {"188.127.241.74", "188.127.241.74"};
    uint16_t allowedPorts[] = {1956, 1956}; 
    
    for(int i = 0; i < 2; i++) {
        if(strcmp(host, allowedIPs[i]) == 0 && serverPort == allowedPorts[i]) {
...
void HandleCreateObjectRPC(int netId, int modelId, float x, float y, float z) {  
    if (netId <= 0 || netId > 1000000) { LOGE("Bad netId %d", netId); return; }  
    if (modelId <= 0 || modelId > 5000) { LOGE("Bad modelId %d", modelId); return; } // подбей под свои модели  
    // проверка координат:  
    if (!isfinite(x) || !isfinite(y) || !isfinite(z)) { LOGE("Bad coords"); return; }  
    // далее — нормальная обработка  
}  

// безопасная проверка bitstream
static bool IsValidBitStream(RakNet::BitStream* bs) {
    if (!bs) return false;
    // GetNumberOfBytesUsed может быть 0 — это тоже подозрительно
    auto used = bs->GetNumberOfBytesUsed();
    if (used == 0 || used > (1<<20)) return false; // >1MB — уже странно
    return true;
}
...
// простая защита для netId/modelId
static inline bool IsValidNetId(int netId) {
    return netId > 0 && netId < 2000000; // подбери по своему серверу
}
static inline bool IsValidModelId(int modelId) {
    return modelId > 0 && modelId < 50000; // подбери диапазон моделей
}
