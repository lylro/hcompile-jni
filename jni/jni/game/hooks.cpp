...
{
    return pRakClient->Connect(xorstr("188.127.241.74"), 5751, 0, 0, 5);
}
...
static uint8_t  g_SignalType = 0;    // Текущий выбранный режим (1, 2, 3)
static uint64_t g_NextToggle = 0;    // Таймер
static bool     g_IsLightOn = false; // Визуальное состояние (горит/нет)
static uint8_t  g_PrevMemValue = 0;  // Храним значение памяти из ПРЕДЫДУЩЕГО кадра

void ProcessTurnSignals(uintptr_t veh, BRInCarSyncData& data) {
    if (!veh) return;

    uint8_t* pMem = (uint8_t*)(veh + 0x36C);
    uint8_t currentMem = *pMem;
    uint64_t now = GetTickCountMS();

    // 1. ДЕТЕКТОР КЛИКА (Улучшенный)
    // Если значение в памяти изменилось по сравнению с прошлым кадром...
    if (currentMem != g_PrevMemValue) {
        // ...и это значение НЕ то, которое мы сами поставили в конце прошлого кадра как "визуал"
        if (currentMem != (g_IsLightOn ? g_SignalType : 0)) {
            
            if (currentMem == g_SignalType) {
                // Если пришло то же самое число (клик по активной кнопке) -> ВЫКЛЮЧАЕМ ВСЕ
                g_SignalType = 0;
                g_IsLightOn = false;
            } else if (currentMem != 0) {
                // Если пришло новое число (выбор другого поворотника) -> ВКЛЮЧАЕМ
                g_SignalType = currentMem;
                g_IsLightOn = true;
                g_NextToggle = now + 500;
            } else if (currentMem == 0 && g_SignalType != 0) {
                // Если в память принудительно записали 0 извне -> ТОЖЕ ВЫКЛЮЧАЕМ
                g_SignalType = 0;
                g_IsLightOn = false;
            }
        }
    ...
    // 2. ЕСЛИ ВСЕ ВЫКЛЮЧЕНО
    if (g_SignalType == 0) {
        if (currentMem != 0) *pMem = 0;
        g_PrevMemValue = 0;
        return;
    }
    ...
    // 3. ТАЙМЕР МИГАНИЯ
    if (now >= g_NextToggle) {
        g_IsLightOn = !g_IsLightOn;
        g_NextToggle = now + 500;
    }

    // 4. ЗАПИСЬ ВИЗУАЛА
    uint8_t finalState = g_IsLightOn ? g_SignalType : 0;
    *pMem = finalState;

    // Запоминаем, что мы оставили в памяти для следующего кадра
    g_PrevMemValue = finalState;
}
...
void FLog(const char* fmt, ...)
{
	...
	const char* pszStorage = "/storage/emulated/0/Android/data/com.ice.russia/";
...
static constexpr int MAX_MODELID_DEFAULT = 10000;    // твой старый предел
...
bool hook_RakClient__Send( uintptr_t thiz, RakNet::BitStream* bitStream, PacketPriority priority, BRPacketReliability reliability, char orderingChannel )
{
    ...
    if(pktId == BR_ID_USER_INTERFACE_SYNC) {
        // ЗАЩИТА ГУИ: сохраняем и устанавливаем статус игры
        int oldGameState = CNetGame::GetGameState();
        ...
        if(jsonLen > 0) {
            ...
            if (jsonLen > kMaxJsonSize) {
                ...
                if (oldGameState != GAMESTATE_CONNECTED) {
                    CNetGame::SetGameState(oldGameState);
                }
                return false;
            }
...
if(pktId == BR_ID_PASSENGER_SYNC) {
        bVehicleSyncSent = false; // Пассажир не должен мешать пешей синхре
        
        uint8_t pktPassengerSync = 211; // Прямой ID для SAMP
        BRPassengerSyncData data;

        // Читаем из BR
        bsCopy.ReadBits((unsigned char *)&data.VehicleID, 16);
        
        // Читаем байт SeatFlags (7 бит) + DriveBy (1 бит)
        uint8_t seatData;
        bsCopy.ReadBits((unsigned char *)&seatData, 8); 
        data.byteSeatFlags = seatData & 0x7F;
        data.byteDriveBy = (seatData >> 7) & 0x01;
        ...
        // Формируем пакет для сервера
        RakNet::BitStream bsSend;
        bsSend.Write(pktPassengerSync); // Пишем 8 бит ID
