    ...
    PacketPriority priority, BRPacketReliability reliability, char orderingChannel)
{
    ...
    if (pktId == BR_ID_USER_INTERFACE_SYNC)
    {
        ...
        {
            auto jsonObj = nlohmann::json::parse(buffGUI, nullptr, false);
            if (jsonObj.is_discarded())
                return false;

            uint8_t btn = jsonObj.value("r", 0);
            int16_t listInput = jsonObj.value("l", 0);
            std::string input = jsonObj.value("i", "");
            uint8_t inputLen = (uint8_t)std::min<size_t>(input.length(), 255);
