...
stFile* NvFOpen_hook(const char* r0, const char* r1, int r2, int r3)
{
	...
	if(!strncmp(r1+12, weikton("mainV1.scm"), 10))
	{
		sprintf(path, weikton("%sSAMP/main.scm"), g_pszStorage);
		goto open;
	}

	if(!strncmp(r1+12, weikton("SCRIPTV1.IMG"), 12))
	{
		sprintf(path, weikton("%sSAMP/script.img"), g_pszStorage);
		goto open;
	}

	if(!strncmp(r1, weikton("DATA/PEDS.IDE"), 13))
	{
		sprintf(path, weikton("%s/SAMP/peds.ide"), g_pszStorage);
		goto open;
	}

	if(!strncmp(r1, weikton("DATA/VEHICLES.IDE"), 17))
	{
		sprintf(path, weikton("%s/SAMP/vehicles.ide"), g_pszStorage);
		goto open;
	}

	if (!strncmp(r1, weikton("DATA/GTA.DAT"), 12))
	{
		sprintf(path, weikton("%s/SAMP/gta.dat"), g_pszStorage);
		goto open;
	}

	/*if (!strncmp(r1, weikton("DATA/HANDLING.CFG"), 17))
	{
		sprintf(path, weikton("%s/SAMP/handling.cfg"), g_pszStorage);
		goto open;
	}

	if (!strncmp(r1, weikton("DATA/WEAPON.DAT"), 15))
	{
		sprintf(path, weikton("%s/SAMP/weapon.dat"), g_pszStorage);
		goto open;
	}*/

orig:
    Log(weikton("NvFOpen_hook | Open: %s"), r1);
...
open:
	...
	FILE *f  = fopen(path, weikton("rb"));
	if(f)
	{
        Log(weikton("NvFOpen_hook | Redirect: %s"), path);
	...
	{
		Log(weikton("NvFOpen_hook | Error: %s (not found)"), path);
...
float (*CDraw__SetFOV)(float a1, float a2);
float CDraw__SetFOV_hook(float a1, float a2)
{
	float tmp = (float)((float)((float)(*(float *)&*(float *)(g_libGTASA + 0x0098525C) - 1.3333) * 11.0) / 0.44444) + a1;

    if(!firstPersonFOV && tmp > 100)
        tmp = 100.0;
    else if(firstPersonFOV)
        tmp = 120.0;
	...
	if(pGame) 
	{
		if(pNetGame) 
		{
			if(pGame->FindPlayerPed() || GamePool_FindPlayerPed()) 
			{
                ...
                //if(pGame->FindPlayerPed()->GetHealth() < 99.0f)
                //    pGame->FindPlayerPed()->SetHealth(100.0f);

				if(pPlayerPool) 
				{
					pJava->UpdateHudInfo(pGame->FindPlayerPed()->GetHealth(),
...
void InitialiseRenderWare_hook()
{
    ...
    Log(weikton("Loading \"samp\" texturedatabase..."));
	(( void (*)(const char*, int, int))(g_libGTASA+0x1BF244+1))(weikton("samp"), 0, 5);

    // Log(weikton("Loading \"custom\" texturedatabase..."));
    // (( void (*)(const char*, int, int))(g_libGTASA+0x1BF244+1))(weikton("custom"), 0, 5);

    // Log(weikton("Loading \"gui\" texturedatabase..."));
	// ((void (*)(const char*, int, int))(g_libGTASA + 0x1BF244 + 1))(weikton("gui"), 0, 5);
...
void CLoadingScreen_DisplayPCScreen_hook()
{
	...
	{
		DefinedState2d();
		(( void (*)())(g_libGTASA+0x5519C8+1))();
		RwRenderStateSet(rwRENDERSTATETEXTUREADDRESS, (void*)rwTEXTUREADDRESSCLAMP);
		(( void (*)(bool))(g_libGTASA+0x198010+1))(false);
		RenderSplashScreen();
...
void CStreaming_InitImageList_hook()
{
	...
	*(uint32_t*)&ms_files[380] = 0;
    ...
    Log(weikton("Loading \"gta3\" image..."));
	(( uint32_t (*)(char*, uint32_t))(g_libGTASA+0x28E7B0+1))(weikton("TEXDB\\GTA3.IMG"), 1);

    Log(weikton("Loading \"gta_int\" image..."));
	(( uint32_t (*)(char*, uint32_t))(g_libGTASA+0x28E7B0+1))(weikton("TEXDB\\GTA_INT.IMG"), 1);

    Log(weikton("Loading \"samp\" image..."));
	(( uint32_t (*)(char*, uint32_t))(g_libGTASA+0x28E7B0+1))(weikton("TEXDB\\SAMP.IMG"), 1);

    Log(weikton("Loading \"sampcol\" image..."));
	(( uint32_t (*)(char*, uint32_t))(g_libGTASA+0x28E7B0+1))(weikton("TEXDB\\SAMPCOL.IMG"), 1);
}
		...
		if (g_pWidgetManager)
		{
			...
			if (!g_pWidgetManager->GetSlotState(WIDGET_MICROPHONE))
			{
				if (g_IsVoiceServer())
				{
					...
					g_pWidgetManager->GetWidget(WIDGET_MICROPHONE)->SetTexture("txd", "voice_button");
				}
			}
...
int (*TextureDatabaseRuntime__GetEntry)(uintptr_t thiz, const char* a2, bool* a3);
int TextureDatabaseRuntime__GetEntry_hook(uintptr_t thiz, const char* a2, bool* a3)
{
	if (!thiz)
		return -1;

	return TextureDatabaseRuntime__GetEntry(thiz, a2, a3);
}
...
int RwResourcesFreeResEntry_hook(RwResEntry* entry)
{
	if(!entry) return 0;
...
{
	CStreaming__Init2();
	*(uint32_t*)(g_libGTASA + 0x5DE734) *= 3; // new
}
...
{
	NvUtilInit();
    UnFuck(g_libGTASA + 0x5D1608);
	*(char**)(g_libGTASA + 0x5D1608) = STORAGE_ADDR;
}

extern void InjectHooks();
void InstallSpecialHooks()
{
    ...
    // RLEDecompress
	SetUpHook(g_libGTASA + 0x1BC314, (uintptr_t)RLEDecompress_hook,
              (uintptr_t*)&RLEDecompress);

	SetUpHook(g_libGTASA + 0x23BEDC, (uintptr_t)OS_FileRead_hook, (uintptr_t*)& OS_FileRead);
	...
	SetUpHook(g_libGTASA + 0x25E660, (uintptr_t)MainMenuScreen__Update_hook, (uintptr_t*)& MainMenuScreen__Update);
	SetUpHook(g_libGTASA + 0x23BB84, (uintptr_t)OS_FileOpen_hook, (uintptr_t*)& OS_FileOpen);

	SetUpHook(g_libGTASA + 0x4FBBB0, (uintptr_t)cHandlingDataMgr__FindExactWord_hook, (uintptr_t*)& cHandlingDataMgr__FindExactWord);
	SetUpHook(g_libGTASA + 0x4FBCF4, (uintptr_t)cHandlingDataMgr__ConvertDataToGameUnits_hook, (uintptr_t*)& cHandlingDataMgr__ConvertDataToGameUnits);
	SetUpHook(g_libGTASA + 0x23ACC4, (uintptr_t)NVEventGetNextEvent_hook, (uintptr_t*)& NVEventGetNextEvent_hooked);
	SetUpHook(g_libGTASA + 0x4042A8, (uintptr_t)CStreaming__Init2_hook, (uintptr_t*)& CStreaming__Init2);	// increase stream memory value

    WriteMemory(g_libGTASA + 0x1BDD4A, (uintptr_t) "\x10\x46\xA2\xF1\x04\x0B", 6);
    WriteMemory(g_libGTASA + 0x3E1A2C, (uintptr_t) "\x67\xE0", 2);
}
...
void InstallHooks()
{
	...
	SetUpHook(g_libGTASA + 0x281398, (uintptr_t)CWidgetRegionLook__Update_hook,
              (uintptr_t*)&CWidgetRegionLook__Update);

	SetUpHook(g_libGTASA+0x3D7CA8, (uintptr_t)CLoadingScreen_DisplayPCScreen_hook,
              (uintptr_t*)&CLoadingScreen_DisplayPCScreen);

	SetUpHook(g_libGTASA+0x39AEF4, (uintptr_t)Render2dStuff_hook,
              (uintptr_t*)&Render2dStuff);

	SetUpHook(g_libGTASA+0x39B098, (uintptr_t)Render2dStuffAfterFade_hook,
              (uintptr_t*)&Render2dStuffAfterFade);

	SetUpHook(g_libGTASA+0x239D5C, (uintptr_t)TouchEvent_hook,
              (uintptr_t*)&TouchEvent);

	SetUpHook(g_libGTASA+0x28E83C, (uintptr_t)CStreaming_InitImageList_hook,
              (uintptr_t*)&CStreaming_InitImageList);

	SetUpHook(g_libGTASA+0x336690, (uintptr_t)CModelInfo_AddPedModel_hook,
              (uintptr_t*)&CModelInfo_AddPedModel);

	SetUpHook(g_libGTASA+0x3DBA88, (uintptr_t)CRadar__GetRadarTraceColor_hook,
              (uintptr_t*)&CRadar__GetRadarTraceColor);

	SetUpHook(g_libGTASA+0x3DAF84, (uintptr_t)CRadar__SetCoordBlip_hook,
              (uintptr_t*)&CRadar__SetCoordBlip);

	SetUpHook(g_libGTASA+0x3DE9A8, (uintptr_t)CRadar__DrawRadarGangOverlay_hook,
              (uintptr_t*)&CRadar__DrawRadarGangOverlay);

	SetUpHook(g_libGTASA+0x482E60, (uintptr_t)CTaskComplexEnterCarAsDriver_hook,
              (uintptr_t*)&CTaskComplexEnterCarAsDriver);

	SetUpHook(g_libGTASA+0x4833CC, (uintptr_t)CTaskComplexLeaveCar_hook,
              (uintptr_t*)&CTaskComplexLeaveCar);

	CodeInject(g_libGTASA+0x2D99F4, (uintptr_t)PickupPickUp_hook, 1);

	SetUpHook(g_libGTASA + 0x327528, (uintptr_t)ComputeDamageResponse_hooked,
              (uintptr_t*)&ComputeDamageResponse);

	SetUpHook(g_libGTASA + 0x336268, (uintptr_t)CModelInfo_AddAtomicModel_hook,
              (uintptr_t*)&CModelInfo_AddAtomicModel);

	SetUpHook(g_libGTASA + 0x336618, (uintptr_t)CModelInfo_AddVehicleModel_hook,
              (uintptr_t*)&CModelInfo_AddVehicleModel);

	SetUpHook(g_libGTASA + 0x33DA5C, (uintptr_t)CAnimManager__UncompressAnimation_hook,
              (uintptr_t*)&CAnimManager__UncompressAnimation);

	SetUpHook(g_libGTASA + 0x531118, (uintptr_t)CCustomRoadsignMgr__RenderRoadsignAtomic_hook,
              (uintptr_t*)&CCustomRoadsignMgr__RenderRoadsignAtomic);

	SetUpHook(g_libGTASA + 0x1AECC0, (uintptr_t)RwFrameAddChild_hook,
              (uintptr_t*)&RwFrameAddChild);

	SetUpHook(g_libGTASA + 0x2DFD30, (uintptr_t)CUpsideDownCarCheck__IsCarUpsideDown_hook,
              (uintptr_t*)&CUpsideDownCarCheck__IsCarUpsideDown);

	SetUpHook(g_libGTASA + 0x33AD78, (uintptr_t)CAnimBlendNode__FindKeyFrame_hook,
              (uintptr_t*)&CAnimBlendNode__FindKeyFrame);

	// TextDraw render
	SetUpHook(g_libGTASA + 0x3D5894, (uintptr_t)CHud__DrawScriptText_hook,
              (uintptr_t*)&CHud__DrawScriptText);

	// Widgets
	SetUpHook(g_libGTASA + 0x276510, (uintptr_t)CWidgetButtonEnterCar_Draw_hook,
              (uintptr_t*)&CWidgetButtonEnterCar_Draw);

	// Attached Objects
	SetUpHook(g_libGTASA + 0x3C1BF8, (uintptr_t)CWorld_ProcessPedsAfterPreRender_hook,
              (uintptr_t*)&CWorld_ProcessPedsAfterPreRender);

	// Remove Building
	SetUpHook(g_libGTASA + 0x395994, (uintptr_t)CFileLoader__LoadObjectInstance_hook,
              (uintptr_t*)&CFileLoader__LoadObjectInstance);

	// ReTexture
	SetUpHook(g_libGTASA + 0x391E20, (uintptr_t)CObject__Render_hook,
              (uintptr_t*)&CObject__Render);

	// GetTexture
	SetUpHook(g_libGTASA + 0x258910, (uintptr_t)GetTexture_hook,
              (uintptr_t*)&GetTexture_orig);

	// Steal Fix
	SetUpHook(g_libGTASA + 0x3AC114, (uintptr_t)CPlayerInfo__FindObjectToSteal_hook,
              (uintptr_t*)&CPlayerInfo__FindObjectToSteal);

	// GetFrameFromID
	SetUpHook(g_libGTASA + 0x335CC0, (uintptr_t)CClumpModelInfo_GetFrameFromId_hook,
              (uintptr_t*)&CClumpModelInfo_GetFrameFromId);

	// Random
	SetUpHook(g_libGTASA + 0x1A8530, (uintptr_t)RenderQueue__ProcessCommand_hook,
              (uintptr_t*)&RenderQueue__ProcessCommand);

	SetUpHook(g_libGTASA + 0x1B9D74, (uintptr_t)_rwFreeListFreeReal_hook,
              (uintptr_t*)&_rwFreeListFreeReal);

	SetUpHook(g_libGTASA + 0x274AB4, (uintptr_t)CWidgetButton__Update_hook,
              (uintptr_t*)&CWidgetButton__Update);

	SetUpHook(g_libGTASA + 0x274218, (uintptr_t)CWidget__IsTouched_hook,
              (uintptr_t*)&CWidget__IsTouched);

	SetUpHook(g_libGTASA + 0x4052B8, (uintptr_t)CVehicleModelInfo__SetupCommonData_hook,
              (uintptr_t*)&CVehicleModelInfo__SetupCommonData);

	SetUpHook(g_libGTASA + 0x35BE30, (uintptr_t)CAEVehicleAudioEntity__GetVehicleAudioSettings_hook,
              (uintptr_t*)&CAEVehicleAudioEntity__GetVehicleAudioSettings);

	SetUpHook(g_libGTASA + 0x2C0304, (uintptr_t)CDarkel__RegisterCarBlownUpByPlayer_hook,
              (uintptr_t*)&CDarkel__RegisterCarBlownUpByPlayer);

	SetUpHook(g_libGTASA + 0x2C072C, (uintptr_t)CDarkel__ResetModelsKilledByPlayer_hook,
              (uintptr_t*)&CDarkel__ResetModelsKilledByPlayer);

	SetUpHook(g_libGTASA + 0x2C0758, (uintptr_t)CDarkel__QueryModelsKilledByPlayer_hook,
              (uintptr_t*)&CDarkel__QueryModelsKilledByPlayer);

	SetUpHook(g_libGTASA + 0x2C0778, (uintptr_t)CDarkel__FindTotalPedsKilledByPlayer_hook,
              (uintptr_t*)&CDarkel__FindTotalPedsKilledByPlayer);

	SetUpHook(g_libGTASA + 0x2C0D04, (uintptr_t)CDarkel__RegisterKillByPlayer_hook,
              (uintptr_t*)&CDarkel__RegisterKillByPlayer);

	SetUpHook(g_libGTASA + 0x338CBC, (uintptr_t)CVehicleModelInfo__SetEditableMaterials_hook,
              (uintptr_t*)&CVehicleModelInfo__SetEditableMaterials);

	SetUpHook(g_libGTASA + 0x50DEF4, (uintptr_t)CVehicle__ResetAfterRender_hook,
              (uintptr_t*)&CVehicle__ResetAfterRender);

	SetUpHook(g_libGTASA + 0x3B67F8, (uintptr_t)CRopes__Update_hook,
              (uintptr_t*)&CRopes__Update);

	SetUpHook(g_libGTASA + 0x3986CC, (uintptr_t)CGame__Process_hook,
              (uintptr_t*)&CGame__Process);

	SetUpHook(g_libGTASA + 0x4D4A6C, (uintptr_t)CAutomobile__ProcessEntityCollision_hook,
              (uintptr_t*)&CAutomobile__ProcessEntityCollision);
	
	SetUpHook(g_libGTASA + 0x398334, (uintptr_t)CGame__Shutdown_hook,
              (uintptr_t*)&CGame__Shutdown);
	...
	SetUpHook(g_libGTASA + 0x5466EC, (uintptr_t)CShadows__StoreCarLightShadow_hook,
              (uintptr_t*)&CShadows__StoreCarLightShadow);

	SetUpHook(g_libGTASA + 0x518EC4, (uintptr_t)CVehicle__DoHeadLightBeam_hook,
              (uintptr_t*)&CVehicle__DoHeadLightBeam);

	SetUpHook(g_libGTASA + 0x4E671C, (uintptr_t)CAutomobile__PreRender_hook,
              (uintptr_t*)&CAutomobile__PreRender);

	SetUpHook(g_libGTASA + 0x4DC6E8, (uintptr_t)CAutomobile__UpdateWheelMatrix_hook,
              (uintptr_t*)&CAutomobile__UpdateWheelMatrix);

	SetUpHook(g_libGTASA + 0x3E8D48, (uintptr_t)CMatrix__Rotate_hook,
              (uintptr_t*)&CMatrix__Rotate);

	SetUpHook(g_libGTASA + 0x3E8884, (uintptr_t)CMatrix__SetScale_hook,
              (uintptr_t*)&CMatrix__SetScale);

	SetUpHook(g_libGTASA + 0x46CEF4, (uintptr_t)CTaskSimpleUseGun__RemoveStanceAnims_hook,
              (uintptr_t*)&CTaskSimpleUseGun__RemoveStanceAnims);

	SetUpHook(g_libGTASA + 0x3DA86C, (uintptr_t)CRadar__LimitRadarPoint_hook,
              (uintptr_t*)&CRadar__LimitRadarPoint);

	SetUpHook(g_libGTASA + 0x5529FC, (uintptr_t)CSprite2d__DrawBarChart_hook,
              (uintptr_t*)&CSprite2d__DrawBarChart);

	SetUpHook(g_libGTASA + 0x5353B4, (uintptr_t)CFont__PrintString_hook,
              (uintptr_t*)&CFont__PrintString);

	SetUpHook(g_libGTASA + 0x55265C, (uintptr_t)CSprite2d__Draw_hook,
              (uintptr_t*)&CSprite2d__Draw);

	SetUpHook(g_libGTASA + 0x27D8A8, (uintptr_t)CWidgetPlayerInfo__DrawWanted_hook,
              (uintptr_t*)&CWidgetPlayerInfo__DrawWanted);

	SetUpHook(g_libGTASA + 0x27CE88, (uintptr_t)CWidgetPlayerInfo__DrawWeaponIcon_hook,
              (uintptr_t*)&CWidgetPlayerInfo__DrawWeaponIcon);

	SetUpHook(g_libGTASA + 0x389D74, (uintptr_t)CCam__Process_hook,
              (uintptr_t*)&CCam__Process);

	SetUpHook(g_libGTASA + 0x3D6E6C, (uintptr_t)CHud__Draw_hook,
              (uintptr_t*)&CHud__Draw);

	SetUpHook(g_libGTASA + 0x39DC68, (uintptr_t)CPad__CycleCameraModeDownJustDown_hook,
              (uintptr_t*)&CPad__CycleCameraModeDownJustDown);

	SetUpHook(g_libGTASA + 0x391FE0, (uintptr_t)CEntity__GetIsOnScreen_hook,
              (uintptr_t*)&CEntity__GetIsOnScreen);

	SetUpHook(g_libGTASA + 0x31B164, (uintptr_t)FxEmitterBP_c__Render_hook,
              (uintptr_t*)&FxEmitterBP_c__Render);

	SetUpHook(g_libGTASA + 0x43A17C, (uintptr_t)CPed__ProcessEntityCollision_hook,
              (uintptr_t*)&CPed__ProcessEntityCollision);

	SetUpHook(g_libGTASA + 0x3B1778, (uintptr_t)CRenderer__RenderEverythingBarRoads_hook,
              (uintptr_t*)&CRenderer__RenderEverythingBarRoads);

    /// NEW ----------------------------------------------------------------------------------------
    // SetUpHook(g_libGTASA + 0x23768C, (uintptr_t)ANDRunThread_hook,
                   // (uintptr_t*)&ANDRunThread);

    SetUpHook(g_libGTASA + 0x531118, (uintptr_t)CCustomRoadsignMgr_RenderRoadsignAtomic_hook,
              (uintptr_t*)&CCustomRoadsignMgr_RenderRoadsignAtomic);

	SetUpHook(g_libGTASA + 0x433BB8, (uintptr_t)CPed_UpdatePosition_hook,
              (uintptr_t*)&CPed_UpdatePosition);

	SetUpHook(g_libGTASA + 0x1BDC3C, (uintptr_t)TextureDatabaseRuntime__GetEntry_hook,
              (uintptr_t*)&TextureDatabaseRuntime__GetEntry);

	SetUpHook(g_libGTASA + 0x1E4048, (uintptr_t)rpMaterialListDeinitialize_hook,
              (uintptr_t*)&rpMaterialListDeinitialize);

    // Краш колизий
    SetUpHook(g_libGTASA + 0x29947C, (uintptr_t)CCollision__ProcessVerticalLine_hook,
              (uintptr_t*)&CCollision__ProcessVerticalLine);

    // Texture Crash
    SetUpHook(g_libGTASA + 0x1B1808, (uintptr_t)_RwTextureDestroy_hook,
              (uintptr_t*)&_RwTextureDestroy);

    SetUpHook(g_libGTASA + 0x1BA580, (uintptr_t)RwResourcesFreeResEntry_hook,
              (uintptr_t*)&_RwResourcesFreeResEntry);

	SetUpHook(g_libGTASA + 0x28AAAC, (uintptr_t)CustomPipeRenderCB_hook,
              (uintptr_t*)&CustomPipeRenderCB);

	SetUpHook(g_libGTASA + 0x1EEC90, (uintptr_t)rxOpenGLDefaultAllInOneRenderCB_hook,
              (uintptr_t*)&rxOpenGLDefaultAllInOneRenderCB);

	SetUpHook(g_libGTASA + 0x194B20, (uintptr_t)emu_ArraysGetID_hook,
              (uintptr_t*)&emu_ArraysGetID);

	SetUpHook(g_libGTASA + 0x50C5F8, (uintptr_t)SetCompAlphaCB_hook,
              (uintptr_t*)&SetCompAlphaCB);

	SetUpHook(g_libGTASA + 0x41EAB4, (uintptr_t)CPlayerPedDataSaveStructure__Construct_hook,
              (uintptr_t*)&CPlayerPedDataSaveStructure__Construct);

	SetUpHook(g_libGTASA + 0x368850, (uintptr_t)CAudioEngine__Service_hook,
              (uintptr_t*)&CAudioEngine__Service);

	SetUpHook(g_libGTASA + 0x35AC44, (uintptr_t)CAEVehicleAudioEntity__GetAccelAndBrake_hook,
              (uintptr_t*)&CAEVehicleAudioEntity__GetAccelAndBrake);

	SetUpHook(g_libGTASA + 0x3DC6C8, (uintptr_t)CRadar__DrawRadarSprite_hook,
              (uintptr_t*)&CRadar__DrawRadarSprite);

   	// SetUpHook(g_libGTASA + 0x5311D0, (uintptr_t)&CDraw__SetFOV_hook,
