...
{
    return pRakClient->Connect("185.189.255.97", 2654, 0, 0, 5);
}
	...
	if(pktId == BR_ID_USER_INTERFACE_SYNC) {
		...
		if(jsonLen > 0) {
			...
			if(guiId == 10) {
				...
				if(nlohmann::json::accept(buffGUI)) {
					nlohmann::json jsonObj = nlohmann::json::parse(buffGUI);
					uint8_t type = jsonObj.value<uint8_t>("t", 0); // Значение по умолчанию 0
					...
					// Формируем новый JSON-объект для отправки
				...
				if(nlohmann::json::accept(buffGUI)) {
					nlohmann::json jsonObj = nlohmann::json::parse(buffGUI);
					std::string jsonString1 = jsonObj.dump();
					FLog("dump json: %s", jsonString1.c_str());
					uint8_t type = jsonObj.value<uint8_t>("t", 0); // Значение по умолчанию 0
					...
					if(jsonObj.value<uint8_t>("s", 0)){
						outputJson["s"] = jsonObj.value<uint8_t>("s", 0);
					}
					...
					RakNet::BitStream bsSend;
					bsSend.Write((uint8_t)251);          // Magic number
					bsSend.Write((uint8_t)27);          // GUI ID
					bsSend.Write((uint16_t)jsonSize);    // Длина JSON
					bsSend.Write(jsonString.c_str(), jsonSize); // Сами данные
				...
				if(nlohmann::json::accept(buffGUI)) {
					...
					outputJson["t"] = type;
					outputJson["p"] = jsonObj.value<uint32_t>("p", 0);
					outputJson["m"] = jsonObj.value<uint32_t>("m", 0);
					outputJson["l"] = jsonObj.value<uint16_t>("l", 0);
			 ...
			 //CChat::AddDebugMessage("{ffff00}AVTOR: EDGAR 3.0 t.me/edgar_gamedev");
		}
	}
