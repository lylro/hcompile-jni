...
int ReadGraphicValue(const char* filePath, const char* table, const char* valueName, int defaultValue) {
	...
	if (file == nullptr) {
		//LogGrap("Не удалось открыть файл: %s", filePath);
		return defaultValue;
	}
...
#include "../CGraphic.h" // by t.me/rkkdev
#include "..//game/WaterShader.h"
void (*emu_glEndInternal)();
uintptr_t* g_WaterShaderClass = nullptr;
static int g_waterValue = -1;
static bool g_waterValueInitialized = false;
extern int ReadGraphicValue(const char* filePath, const char* table, const char* valueName, int defaultValue);
bool g_EnableWaterShaders = false;
...
{
	if (!g_waterValueInitialized) {
		g_waterValue = 0;
	}

	int waterValue = g_waterValue;
	...
	if (((*(uintptr_t*)(g_libGTASA + 0x61572C)) & 0x80000) != 0)
	{
		if (g_WaterShaderClass == nullptr)
		{
			...
			{
				WaterShader::BuildShadersSource1(g_WaterShaderClass); // Значение по умолчанию
			}
		}
...
{
	NvUtilInit();
	*(char**)(g_libGTASA + 0x5D1608) = "/storage/emulated/0/MobileRussia/";
}
...
{
	if (CHUD::IsEnabled()) DrawRadarTexture(CHUD::radarBgPos1.x1, CHUD::radarBgPos1.y1, CHUD::radarBgPos2.x1 * mult, CHUD::radarBgPos2.x1 * mult);
