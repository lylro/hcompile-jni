		...
		if(pNetGame) 
		{
			if(pGame->FindPlayerPed() || GamePool_FindPlayerPed()) 
			{
				...
				if(pPlayerPool) 
				{
					...
					m_pPlayerCount, 
					pGame->GetLocalMoney(), pGame->GetWantedLevel(), pPlayerPool->GetLocalPlayerScore());
				}
...
{	
	...
	*(char**)(SA_ADDR(0x5D1608)) = "/storage/emulated/0/Hard/";
}
...
void InstallHooks()
{
	...
	//installHook(SA_ADDR(0x3AC45C), (uintptr_t) PlayerSpeed_hook, (uintptr_t *) &PlayerSpeed);
