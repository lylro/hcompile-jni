...
static bool isLoadedSprint = false;
int *(*LoadFullTexture)(uintptr_t *thiz, unsigned int a2);
int *LoadFullTexture_hook(uintptr_t *thiz, unsigned int a2)
{
	if(!isLoadedSprint && strcmp(*(const char **)(*((DWORD *)thiz + 7) + 0x17 * a2), "hud_sprint") == 0) {
		isLoadedSprint = true;
		return LoadFullTexture(thiz, a2);
	} else if(isLoadedSprint && strcmp(*(const char **)(*((DWORD *)thiz + 7) + 0x17 * a2), "hud_sprint") == 0) {
		return 0;
	}
    return LoadFullTexture(thiz, a2);
}
...
{	
	NvUtilInit();
	*(char**)(g_libGTASA + 0x5D1608) = "/storage/emulated/0/HyonteBlackRussiaOld/";
}
