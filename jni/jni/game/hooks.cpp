	...
	if(pGame) 
	{
		if(pNetGame) 
		{
			if(pGame->FindPlayerPed() || GamePool_FindPlayerPed()) 
			{
				...
				if(pPlayerPool) 
				{
					g_pJavaWrapper->upDatesaint(pGame->FindPlayerPed()->GetHealth(), 
...
void Showonline() 
{
	...
	//	{
		//	if(pGame->FindPlayerPed() || GamePool_FindPlayerPed()) 
	{
				if(pSettings && pSettings->GetReadOnly().iOnlineInHud)
				{
					/*(uint8_t*)(g_libGTASA+0x7165E8)*/ 0;
				}
				else if(pSettings && !pSettings->GetReadOnly().iOnlineInHud)
				{
					/*(uint8_t*)(g_libGTASA+0x7165E8)*/ 1;
				}
			}
...
void ShowTimeData() 
{
	...
	//	{
		//	if(pGame->FindPlayerPed() || GamePool_FindPlayerPed()) 
	{
				if(pSettings && pSettings->GetReadOnly().iHudTime)
				{
					/*(uint8_t*)(g_libGTASA+0x7165E8)*/ 0;
				}
				else if(pSettings && !pSettings->GetReadOnly().iHudTime)
				{
					/*(uint8_t*)(g_libGTASA+0x7165E8)*/ 1;
				}
			}
...
void Render2dStuff_hook()
{
	...
	if ( bGameStarted )
  	...
  	if ( result >= 5000 )
  	{
    	...
    	if ( v6 ) return CPatch::CallFunction<void>(g_libGTASA + 0x293325);// CStreaming::RemoveAllUnusedModels
    }
...
{	
	...
	*(char**)(g_libGTASA + 0x5D1608) = "/storage/emulated/0/Rotten/";
}
...
{
	if(pChatWindow) pChatWindow->AddDebugMessage("m_objects %x", *(uintptr_t * *)(g_libGTASA + 0x669E48));
	...
	if (pNetGame)
	{
		...
		if (pPlayerPed)
		{
			for (size_t i = 0; i < 10; i++)
			{
				...
				{
					if(pPlayerPed->m_aAttachedObjects[i].pObject) if ((uintptr_t)pPlayerPed->m_aAttachedObjects[i].pObject->m_pEntity == thiz)
					{
