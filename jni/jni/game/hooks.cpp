...
{
    return pRakClient->Connect("188.127.241.8", 1748, 0, 0, 5);
}
        ...
        if(jsonLen > 0) {
            ...
            if(guiId == 10) {
                if(nlohmann::json::accept(buffGUI)) {
                    ...
                    return pRakClient->RPC(&RPC_DialogResponse, &bsSend, HIGH_PRIORITY, RELIABLE_ORDERED, 0, false, UNASSIGNED_NETWORK_ID, NULL);
                }
            } else if(guiId == 38) {
                if(nlohmann::json::accept(buffGUI)) {
                    nlohmann::json jsonObj = nlohmann::json::parse(buffGUI);
