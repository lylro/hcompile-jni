...
void CJavaWrapper::ShowReg(const std::string &jsonString, uint16_t rpcID)
{
    JNIEnv* env = nullptr;
            ...
            if (showCurrentGUIMethod != NULL) {
                ...
                if (jsonObjectClass != NULL) {
                    ...
                    if (constructor != NULL) {
                        ...
                        env->CallVoidMethod(guiManagerInstance, showCurrentGUIMethod, rpcID /* экран ID */, jsonObject);
...
void CJavaWrapper::ShowDonateDialog(const std::string &jsonString)
{
    JNIEnv* env = nullptr;
    javaVM->GetEnv((void**)& env, JNI_VERSION_1_6);
    if (!env) {
    ...
    if (guiManagerClass != NULL) {
        ...
        if (getInstanceMethod != NULL) {
            ...
            jmethodID showCurrentGUIMethod = env->GetMethodID(donateGui, "onPacketIncoming", "(Lorg/json/JSONObject;)V");
            if (showCurrentGUIMethod != NULL) {
                jclass jsonObjectClass = env->FindClass("org/json/JSONObject");
                if (jsonObjectClass != NULL) {
                    jmethodID constructor = env->GetMethodID(jsonObjectClass, "<init>", "(Ljava/lang/String;)V");
...
CJavaWrapper::CJavaWrapper(JNIEnv* env)
{
	...
	donateGui = env->FindClass("com/blackhub/bronline/game/gui/donate/GUIDonate");
