...
// material text by x1y2z
// if u delete this comm, i fuck ur mom
...
ImColor RenderTextAndGetLastColor(ImFont *font, const float font_size, uint8_t outline, ImVec2 pos, ImColor col, const char* szStr)
{
	...
	while(textCur < (tempStr + sizeof(tempStr)) && *textCur != '\0')
	{
		if(*textCur == '{')
		{
			...
			{
				pGUI->RenderTextForMaterialText(vecPos, col, outline, textStart, textCur, font_size, font);
...
void CMaterialText::SetUpScene()
{
	...
	// RpWorldAddCamera
	RpWorld* rpWorld = *(RpWorld**)(g_libGTASA+0x95B060); // Scene
	if(rpWorld)
