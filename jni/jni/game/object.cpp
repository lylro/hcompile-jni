...
{	 
	...
	{	
		...
		int del = (byteMaterialSize / 10) - 1;
		int iSizeX = sizes[del][0];
		int iSizeY = sizes[del][1];

		m_bMaterialsText = true;
		m_MaterialTextTexture[iMaterialIndex].pTex = pMaterialText->Generate(iSizeX, iSizeY, szFontName, byteFontSize, byteFontBold, dwFontColor, dwBackgroundColor, byteAlign, szText);
