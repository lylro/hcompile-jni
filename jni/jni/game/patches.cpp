...
void ApplyPatches()
{
	...
	CHook::WriteMemory(g_libGTASA + 0x005197EA, (uintptr_t)"\xFA", 1);

    NOP(g_libGTASA + 0x52C064, 2); // CClouds::VolumetricCloudsRender(CClouds *__hidden this)
    UnFuck(g_libGTASA + 0x52A648, 100); // CClouds::RenderSkyPolys(void)
    *(uint32_t *)((char *)g_libGTASA + 0x52A648) = 0x41200000;
    ...
    NOP(g_libGTASA + 0x56C342, 2); // CGame::Shutdown(void)
    NOP(g_libGTASA + 0x4BE6FE, 4); // CPad::ExitVehicleJustDown(bool,CVehicle *,bool,CVector const&)
    NOP(g_libGTASA + 0x339240, 7); // Handling Mods

    NOP(g_libGTASA + 0x39B1E4, 2); // –асширение экрана (пиксели)

    NOP(g_libGTASA + 0x4E55E2, 2); // CPhysical::ApplyTurnForce(CVector,CVector)
    NOP(g_libGTASA + 0x51013E, 1); // CStreaming::ms_aInfoForModel
    NOP(g_libGTASA + 0x510166, 1); // CPlayerPed::DoesPlayerWantNewWeapon(eWeaponType,bool)
    NOP(g_libGTASA + 0x4BEBE6, 2); // 4BEBE6
    NOP(g_libGTASA + 0x5A2650, 2); // SET_CHAR_OBJ_AIM_GUN_AT_CHAR
    NOP(g_libGTASA + 0x39AD84, 2); // RenderScene(bool)
    NOP(g_libGTASA + 0x2467AA, 1); // CAEMP3BankLoader::Service(void)

    *(uint8_t *)((char *)g_libGTASA + 0x5D097C) = 0x72F31C; // CDarkel::RegisteredKills
    *(uint8_t *)((char *)g_libGTASA + 0x63D4F0) = 0x739F00; // pointers
    *(uint8_t *)((char *)g_libGTASA + 0x5D1E8C) = 0x738F60; // Points_ptr

    NOP(g_libGTASA + 0x3983C6, 2); // CWorld::ShutDown(void)
	UnFuck(g_libGTASA + 0x52A648, 100); // CClouds::RenderSkyPolys(void)
    *(uint32_t *)((char *)g_libGTASA + 0x52A648) = 0x41200000;

    CHook::NOP(g_libGTASA + 0x0056CD4A, 2);
	...
	CHook::RET(g_libGTASA + 0x00443EF4); // CPedGroupDefaultTaskAllocatorRandom::AllocateDefaultTasks
	CHook::RET(g_libGTASA + 0x00448B38); // CPedGroups::Process()
	CHook::RET(g_libGTASA + 0x00448850); // CPedGroupIntelligence::Process
	CHook::RET(g_libGTASA + 0x00444B18); // CPedGroupMembership::SetLeader
	CHook::RET(g_libGTASA + 0x004B3156); // CPedGroupIntelligence::Flush

	CHook::RET(g_libGTASA + 0x0051DEC4); // WaterCreatureManager_c::Update(float)
    ...
    CHook::RET(g_libGTASA + 0x00457644); // CPlayerPed::AnnoyPlayerPed
...
void ApplyInGamePatches()
{
	...
	*(uint8_t*)(g_libGTASA + 0x4EE200) = 0x9B;
	...
	/// new
    *(float*)(g_libGTASA + 0x608558) = 0.4f; // Ёффект разгона авто
