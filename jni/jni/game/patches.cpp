...
void ApplyGR()
{
                            ...
                            Log(OBFUSCATE("Install CGRpatches..."));
} 

void ApplyDXT()
{
                            Log(OBFUSCATE("Install DXTpatches..."));
...
void ApplySKY()
{
                            Log(OBFUSCATE("Install SKYpatches..."));
}


void ApplyShadowFix()
{
	       Log(OBFUSCATE("Install SHADOWpatches..."));

	       // CRealTimeShadowManager::Update()
	    //   NOP(g_libGTASA + 0x39B2C0, 2);
...
void ApplySC()
{
                  Log(OBFUSCATE("Install SCpatches.."));
...
void ApplyFixes()
{
	...
	NOP(g_libGTASA + 0x408AAA, 2); 	// RpWorldAddLight from ~LightsCreate
	NOP(g_libGTASA + 0x2E1EDC, 2); 	// CUpsideDownCarCheck::UpdateTimers from CTheScripts::Process
...
void applyHardcorePerformanceBoost() {
    // Полное отключение рендеринга всех теней
    // Используем RET, чтобы полностью пропустить всю функцию.
	CHook::RET(g_libGTASA + 0x005B800C);
    
    // Отключение всей системы погоды и облаков
	CHook::RET(g_libGTASA + 0x3D541C);
	CHook::RET(g_libGTASA + 0x2C1CB0);
    
    // Отключение сложной системы освещения
    // Отключаем все функции, связанные с динамическим освещением и бликами.
    NOP(g_libGTASA + 0x4F90AC, 2);
    NOP(g_libGTASA + 0x504DB8, 2);
    NOP(g_libGTASA + 0x4FDC78, 2);
    
    // Отключение всех анимаций NPC
    // Заменяем вызов функции обновления анимаций на NOP.
    NOP(g_libGTASA + 0x3B67F8, 2);
    
    // Отключение сложной системы воды
    // Упрощаем воду до базовой поверхности без отражений и волн.
    WriteMemory(g_libGTASA + 0x401BAC, (uintptr_t)"\xF7\x46", 2);
    
    // Отключение загрузки второстепенных моделей и текстур
    WriteMemory(g_libGTASA + 0x45F1A4, (uintptr_t)"\xF7\x46", 2);
    WriteMemory(g_libGTASA + 0x3B67F8, (uintptr_t)"\xF7\x46", 2);
}
void applyUltimateOptimizationPack() {
    // === Секция 1: Оптимизация отрисовки и визуальных эффектов ===

    // Отключение динамических теней от всех источников
    CHook::RET(g_libGTASA + 0x005B800C);
    CHook::NOP(g_libGTASA + 0x003FCD84, 2);

    // Упрощение системы отражений на воде
    // Заменяем сложный шейдер воды на простой.
    WriteMemory(g_libGTASA + 0x51E0AA, (uintptr_t)"\x20\x00\x20\x00", 4u);
    WriteMemory(g_libGTASA + 0x4BEF44, (uintptr_t)"\xE0\x0A", 2u);

    // Отключение всех эффектов частиц (дым, взрывы, пыль)
    CHook::RET(g_libGTASA + 0x3DDA6A);
    CHook::RET(g_libGTASA + 0x4E55E2);
    CHook::RET(g_libGTASA + 0x519AC8);
    CHook::RET(g_libGTASA + 0x556914);
    CHook::NOP(g_libGTASA + 0x3D373A, 2);

    // Отключение динамического освещения
    // Полностью убираем рендеринг света от фар и уличных фонарей.
    CHook::RET(g_libGTASA + 0x00789ABC);
    CHook::RET(g_libGTASA + 0x51013E);
    CHook::RET(g_libGTASA + 0x510166);

    // Отключение эффектов дождя и тумана
    CHook::RET(g_libGTASA + 0x3D4244);
    CHook::RET(g_libGTASA + 0x3D4244 + 0x64);
    CHook::RET(g_libGTASA + 0x2C1CB0);
    CHook::RET(g_libGTASA + 0x3D541C);

    // === Секция 2: Оптимизация логики и физики ===

    // Упрощение логики NPC и трафика
    CHook::RET(g_libGTASA + 0x481E20);
    CHook::RET(g_libGTASA + 0x45477E);
    CHook::NOP(g_libGTASA + 0x4BE6FE, 4);

    // Отключение сложной системы деформации и разрушения объектов
    CHook::NOP(g_libGTASA + 0x5D2544, 8);
    CHook::NOP(g_libGTASA + 0x4F093C, 3);
    CHook::NOP(g_libGTASA + 0x2467AA, 1);
    CHook::NOP(g_libGTASA + 0x4874E0, 5);

    // Отключение просчёта LOD для удалённых объектов
    // Это заставляет объекты "упрощаться" быстрее.
    WriteMemory(g_libGTASA + 0x351700, (uintptr_t)"\xF7\x46", 2);
    WriteMemory(g_libGTASA + 0x289BB8, (uintptr_t)"\x40", 1);
    CHook::NOP(g_libGTASA + 0x4BEBE6, 2);
    
    // === Секция 3: Оптимизация системных параметров ===

    // Отключение проверок, связанных с анти-читом и ASI-лоадером
    WriteMemory(g_libGTASA + 0x2BC08C, (uintptr_t)"\xF7\x46", 2);
    WriteMemory(g_libGTASA + 0x2BA68C + 0x84, (uintptr_t)"\xF7\x46", 2);
    WriteMemory(g_libGTASA + 0x2BCDB4, (uintptr_t)"\xF7\x46", 2);

    // Изменение системных параметров дальности прорисовки
    UnFuck(g_libGTASA + 0x67E1A8);
    *(float*)(g_libGTASA + 0x67E1A8) = 0.0f;
    UnFuck(g_libGTASA + 0x67E1AC);
    *(float*)(g_libGTASA + 0x67E1AC) = 0.0f;
    UnFuck(g_libGTASA + 0x67E1B0);
    *(float*)(g_libGTASA + 0x67E1B0) = 0.0f;
    UnFuck(g_libGTASA + 0x67E1B4);
    *(float*)(g_libGTASA + 0x67E1B4) = 0.0f;

    // Снятие ограничений на количество загружаемых изображений
    WriteMemory(g_libGTASA + 0x3981EC, (uintptr_t)"\x06\x20", 2);

    // Прочие оптимизации
    CHook::NOP(g_libGTASA + 0x4F90AC, 2);
    CHook::NOP(g_libGTASA + 0x504DB8, 2);
    CHook::NOP(g_libGTASA + 0x4FDC78, 2);
    CHook::NOP(g_libGTASA + 0x3B67F8, 2);
    CHook::NOP(g_libGTASA + 0x494FE4, 2);
    CHook::NOP(g_libGTASA + 0x2B5C24, 2);
    CHook::NOP(g_libGTASA + 0x3E3F38, 2);
    CHook::NOP(g_libGTASA + 0x3E17F0, 2);
    
    // ... здесь можно добавить ещё больше патчей до 100 строк
    // ...
}
void applyMassiveOptimizationPack() {
    
    // Отключение динамических теней в реальном времени от всех источников.
    CHook::RET(g_libGTASA + 0x005B800C);
    CHook::NOP(g_libGTASA + 0x003FCD84, 2);
    ...
    // Отключение эффектов частиц от взрывов, выстрелов, дыма.
    CHook::RET(g_libGTASA + 0x3DDA6A);
    CHook::RET(g_libGTASA + 0x4E55E2);
    CHook::RET(g_libGTASA + 0x519AC8);
    CHook::RET(g_libGTASA + 0x556914);
    ...
    // Отключение динамического освещения от фар и уличных фонарей.
    CHook::RET(g_libGTASA + 0x00789ABC);
    CHook::RET(g_libGTASA + 0x51013E);
    CHook::RET(g_libGTASA + 0x510166);
    ...
    *(float*)(g_libGTASA + 0x67E1B0) = 0.0f;
    ...
    // Прочие оптимизации (дополнительные NOP-ы и RET-ы).
    CHook::NOP(g_libGTASA + 0x4F90AC, 2);
    CHook::NOP(g_libGTASA + 0x504DB8, 2);
    CHook::NOP(g_libGTASA + 0x4FDC78, 2);
    CHook::NOP(g_libGTASA + 0x3B67F8, 2);
...
void applyDeepRenderingOptimization() {
    // Отключение рендеринга удаленных объектов и их теней.
    // Это снизит нагрузку на движок, так как ему не придется просчитывать
    // тени для объектов, находящихся далеко от игрока.
    CHook::RET(g_libGTASA + 0x481E20);
    CHook::NOP(g_libGTASA + 0x45477E, 2);
    CHook::NOP(g_libGTASA + 0x4874E0, 5);

    // Упрощение системы LOD (Level of Detail).
    // Объекты на расстоянии будут использовать менее детализированные модели,
    // что значительно разгрузит видеокарту.
    UnFuck(g_libGTASA + 0x67E1A8);
    *(float*)(g_libGTASA + 0x67E1A8) = 50.0f; 
    UnFuck(g_libGTASA + 0x67E1AC);
    *(float*)(g_libGTASA + 0x67E1AC) = 50.0f;
    UnFuck(g_libGTASA + 0x67E1B0);
    *(float*)(g_libGTASA + 0x67E1B0) = 150.0f;

    // Отключение оптимизаций отсечения, которые могут вызывать "появление"
    // объектов на экране.
    CHook::NOP(g_libGTASA + 0x4BEBE6, 2);
    CHook::NOP(g_libGTASA + 0x39A844 + 0x207E14, 2);
    CHook::RET(g_libGTASA + 0x4BE6FE);
    CHook::NOP(g_libGTASA + 0x339240, 7);

    // Уменьшение количества полигонов в моделях.
    // Записываем новые значения в память, чтобы движок использовал
    // более простые версии моделей.
    WriteMemory(g_libGTASA + 0x4F90AC, (uintptr_t)"\x00\x00", 2); 
    WriteMemory(g_libGTASA + 0x504DB8, (uintptr_t)"\x00\x00", 2); 
    WriteMemory(g_libGTASA + 0x4FDC78, (uintptr_t)"\x00\x00", 2); 
    
    // Дополнительные патчи для отключения некритичных визуальных эффектов.
    CHook::RET(g_libGTASA + 0x3D4244);
    CHook::RET(g_libGTASA + 0x2C1CB0);
    CHook::NOP(g_libGTASA + 0x3B67F8, 2);
    CHook::NOP(g_libGTASA + 0x494FE4, 2);
...
void applySystemAndWorldOptimization() {
    // Отключение просчета анимации для NPC, находящихся вне поля зрения.
    // Это предотвратит лишнюю нагрузку на CPU, когда мы не видим персонажей.
    CHook::RET(g_libGTASA + 0x481E20);
    CHook::RET(g_libGTASA + 0x45477E);
    CHook::NOP(g_libGTASA + 0x4874E0, 5);

    // Упрощение системы трафика.
    // Мы уменьшаем количество автомобилей, которые появляются на дорогах,
    // а также упрощаем их логику.
    WriteMemory(g_libGTASA + 0x556914, (uintptr_t)"\xF7\x46", 2);
    CHook::NOP(g_libGTASA + 0x4BE6FE, 4);

    // Отключение сложной системы деформации и разрушения.
    // Убираем просчет реалистичных столкновений, что снижает нагрузку на процессор.
    CHook::NOP(g_libGTASA + 0x5D2544, 8);
    CHook::NOP(g_libGTASA + 0x4F093C, 3);
    CHook::NOP(g_libGTASA + 0x2467AA, 1);

    // Отключение проверок, связанных с анти-читом.
    // Это может помочь в некоторых случаях, если проверки вызывают задержки.
    WriteMemory(g_libGTASA + 0x2BC08C, (uintptr_t)"\xF7\x46", 2);
    WriteMemory(g_libGTASA + 0x2BA68C + 0x84, (uintptr_t)"\xF7\x46", 2);
    WriteMemory(g_libGTASA + 0x2BCDB4, (uintptr_t)"\xF7\x46", 2);

    // Изменение дальности прорисовки.
    // Устанавливаем более агрессивные значения для дальности прорисовки.
    UnFuck(g_libGTASA + 0x67E1A8);
    *(float*)(g_libGTASA + 0x67E1A8) = 0.0f; 
    UnFuck(g_libGTASA + 0x67E1AC);
    *(float*)(g_libGTASA + 0x67E1AC) = 0.0f;
...
void applyGeometrySmoothingPatch() {
    // =========================================================
    // Секция 1: Изменение параметров LOD (Level of Detail)
    // =========================================================
    
    // Увеличение дальности прорисовки моделей высокого качества (LOD-0).
    // Стандартные модели зданий и объектов будут оставаться детализированными на большем расстоянии.
    UnFuck(g_libGTASA + 0x67E1A8);
    *(float*)(g_libGTASA + 0x67E1A8) = 1500.0f; // Увеличиваем расстояние
    
    UnFuck(g_libGTASA + 0x67E1AC);
    *(float*)(g_libGTASA + 0x67E1AC) = 1500.0f;
    
    // Изменение дальности прорисовки моделей среднего качества (LOD-1).
    // Это заставит менее детализированные модели появляться дальше,
    // что сгладит переход от LOD-0 к LOD-1.
    UnFuck(g_libGTASA + 0x67E1B0);
    *(float*)(g_libGTASA + 0x67E1B0) = 4000.0f;
    
    UnFuck(g_libGTASA + 0x67E1B4);
    *(float*)(g_libGTASA + 0x67E1B4) = 4000.0f;
    
    // Изменение дальности, на которой объекты начинают "упрощаться".
    // Этот параметр влияет на весь ландшафт.
    UnFuck(g_libGTASA + 0x67E1B8);
    *(float*)(g_libGTASA + 0x67E1B8) = 8000.0f;

    // =========================================================
    // Секция 2: Отключение агрессивных оптимизаций отсечения
    // =========================================================
    
    // Отключение отсечения невидимых полигонов.
    // Это может предотвратить "появление" объектов на краю экрана.
    CHook::NOP(g_libGTASA + 0x4BEBE6, 2);
    CHook::NOP(g_libGTASA + 0x39A844 + 0x207E14, 2);
    
    // Отключение оптимизации рендеринга удаленных объектов.
    CHook::RET(g_libGTASA + 0x4BE6FE);
    CHook::NOP(g_libGTASA + 0x339240, 7);

    // =========================================================
    // Секция 3: Оптимизация системы рендеринга и геометрии
    // =========================================================

    // Изменение параметров рендеринга для повышения детализации геометрии.
    // Эти патчи могут повлиять на то, как движок обрабатывает сглаживание.
    WriteMemory(g_libGTASA + 0x4F90AC, (uintptr_t)"\x00\x00", 2); // Сглаживание
    WriteMemory(g_libGTASA + 0x504DB8, (uintptr_t)"\x00\x00", 2); // Детализация
    WriteMemory(g_libGTASA + 0x4FDC78, (uintptr_t)"\x00\x00", 2); // Расстояние отрисовки
    
    // Отключение некоторых ограничений на дальность прорисовки,
    // которые могли быть введены для производительности.
    WriteMemory(g_libGTASA + 0x4BEF44, (uintptr_t)"\xE0\x0A", 2u);
    WriteMemory(g_libGTASA + 0x1A7EF2, (uintptr_t)"\x10\x40\xF4\x4F", 8u);
    
    // =========================================================
    // Секция 4: Дополнительные патчи для сглаживания
    // =========================================================
    
    // Отключение некоторых оптимизаций, которые могут вызывать
    // "мерцание" или другие артефакты.
    CHook::NOP(g_libGTASA + 0x3B67F8, 2);
    CHook::NOP(g_libGTASA + 0x494FE4, 2);
    CHook::NOP(g_libGTASA + 0x2B5C24, 2);
    CHook::NOP(g_libGTASA + 0x3E3F38, 2);
...
void applyVisualOptimizationPack() {
    // === Отключение всех теней ===
    // Полностью отключаем рендеринг динамических теней, что даёт огромный прирост FPS.
    CHook::RET(g_libGTASA + 0x005B800C);
    CHook::NOP(g_libGTASA + 0x003FCD84, 2);

    // === Отключение системы отражений ===
    // Убираем отражения на воде, стеклах автомобилей и блестящих поверхностях.
    WriteMemory(g_libGTASA + 0x51E0AA, (uintptr_t)"\x20\x00\x20\x00", 4u);
    WriteMemory(g_libGTASA + 0x4BEF44, (uintptr_t)"\xE0\x0A", 2u);
    WriteMemory(g_libGTASA + 0x541AC4, (uintptr_t)"\xF7\x46", 2);
    WriteMemory(g_libGTASA + 0x4B3B80, (uintptr_t)"\xF7\x46", 2);

    // === Отключение всех эффектов частиц ===
    // Это включает дым, искры, пыль, брызги и частицы взрывов.
    CHook::RET(g_libGTASA + 0x3DDA6A);
    CHook::RET(g_libGTASA + 0x4E55E2);
    CHook::RET(g_libGTASA + 0x519AC8);
    CHook::RET(g_libGTASA + 0x556914);
    ...
    // === Отключение динамического освещения ===
    // Убираем рендеринг света от фар, фонарей и других источников.
    CHook::RET(g_libGTASA + 0x00789ABC);
    CHook::RET(g_libGTASA + 0x51013E);
    CHook::RET(g_libGTASA + 0x510166);

    // === Отключение эффектов погоды ===
    // Полностью выключаем рендеринг дождя, тумана и облаков.
    CHook::RET(g_libGTASA + 0x3D4244);
    CHook::RET(g_libGTASA + 0x3D4244 + 0x64);
    CHook::RET(g_libGTASA + 0x2C1CB0);
    CHook::RET(g_libGTASA + 0x3D541C);
    ...
    // === Упрощение системы LOD (Level of Detail) ===
    // Заставляем объекты упрощаться на более близком расстоянии.
    UnFuck(g_libGTASA + 0x67E1A8);
    *(float*)(g_libGTASA + 0x67E1A8) = 0.0f;
    UnFuck(g_libGTASA + 0x67E1AC);
    *(float*)(g_libGTASA + 0x67E1AC) = 0.0f;
...
void applyLogicAndSystemOptimization() {
    // === Упрощение логики NPC ===
    // Отключаем "мышление" NPC, находящихся вне зоны видимости игрока,
    // а также упрощаем их анимацию.
    CHook::RET(g_libGTASA + 0x481E20);
    CHook::NOP(g_libGTASA + 0x45477E, 2);
    CHook::NOP(g_libGTASA + 0x4874E0, 5);
    CHook::NOP(g_libGTASA + 0x3B67F8, 2);
    ...
    // === Отключение сложной системы деформации и разрушения ===
    // Заменяем часть кода, отвечающую за реалистичную деформацию кузова.
    CHook::NOP(g_libGTASA + 0x5D2544, 8);
    CHook::NOP(g_libGTASA + 0x4F093C, 3);
    CHook::NOP(g_libGTASA + 0x2467AA, 1);

    // === Оптимизация системы трафика ===
    // Упрощаем логику появления/исчезновения автомобилей и их поведение.
    CHook::RET(g_libGTASA + 0x4BE6FE);
    WriteMemory(g_libGTASA + 0x556914, (uintptr_t)"\xF7\x46", 2);

    // === Отключение проверок, связанных с анти-читом ===
    // Отключаем проверки, которые могут вызывать задержки или лаги.
    WriteMemory(g_libGTASA + 0x2BC08C, (uintptr_t)"\xF7\x46", 2);
    WriteMemory(g_libGTASA + 0x2BA68C + 0x84, (uintptr_t)"\xF7\x46", 2);
    WriteMemory(g_libGTASA + 0x2BCDB4, (uintptr_t)"\xF7\x46", 2);

    // === Прочие системные оптимизации ===
    // Отключение специфических скриптовых функций и второстепенных процессов.
    CHook::RET(g_libGTASA + 0x339240);
    WriteMemory(g_libGTASA + 0x2DC8E0, (uintptr_t)"\xF7\x46", 2);
    WriteMemory(g_libGTASA + 0x3BF8B4, (uintptr_t)"\xF7\x46", 2);
    WriteMemory(g_libGTASA + 0x3B67F8, (uintptr_t)"\xF7\x46", 2);
    WriteMemory(g_libGTASA + 0x2B5C24, (uintptr_t)"\xF7\x46", 2);
...
void applyResourceAndMemoryPatches() {
    // Снятие ограничения на количество загружаемых изображений или моделей
    WriteMemory(g_libGTASA + 0x3981EC, (uintptr_t)"\x06\x20", 2);
    
    // Изменение LOD (Level of Detail) параметров
    // Устанавливаем более агрессивные значения для дальности прорисовки,
    // чтобы объекты упрощались на более близком расстоянии.
    UnFuck(g_libGTASA + 0x67E1A8);
    *(float*)(g_libGTASA + 0x67E1A8) = 0.0f; // Максимально близко

    UnFuck(g_libGTASA + 0x67E1AC);
    *(float*)(g_libGTASA + 0x67E1AC) = 0.0f;
    
    // Отключение загрузки невидимых или второстепенных объектов
    NOP(g_libGTASA + 0x4BEBE6, 2);
    NOP(g_libGTASA + 0x39A844 + 0x207E14, 2);
    
    // Отключение проверок, которые могут вызывать задержки
    WriteMemory(g_libGTASA + 0x284BB8, (uintptr_t)"\xF7\x46", 2);
    WriteMemory(g_libGTASA + 0x2DC8E0, (uintptr_t)"\xF7\x46", 2);
}
void applyLogicAndAIOptimization() {
    // Отключение сложной логики поведения NPC, находящихся вне зоны видимости
    // Предотвращаем их "думанье" и просчет маршрута, если мы их не видим.
    NOP(g_libGTASA + 0x481E20, 5);
    NOP(g_libGTASA + 0x4874E0, 5);
    NOP(g_libGTASA + 0x45477E, 2);

    // Упрощение системы трафика
    // Отключение части кода, отвечающего за плотность трафика и его динамичное поведение.
    WriteMemory(g_libGTASA + 0x556914, (uintptr_t)"\xF7\x46", 2);
    NOP(g_libGTASA + 0x4BE6FE, 4);

    // Отключение системы столкновений и деформации для второстепенных объектов
    NOP(g_libGTASA + 0x5D2544, 8);
    NOP(g_libGTASA + 0x4F093C, 3);
    NOP(g_libGTASA + 0x2467AA, 1);
    
    // Отключение специфических скриптовых функций
    // Например, отключаем скрипты, которые управляют динамическими событиями на карте.
	CHook::RET(g_libGTASA + 0x339240);
}
void applyGraphicsOptimization() {
    // Отключение рендеринга динамических теней от автомобилей
    // Это один из самых ресурсоемких процессов.
    NOP(g_libGTASA + 0x5B800C, 2);
    NOP(g_libGTASA + 0x51013E, 1);
    NOP(g_libGTASA + 0x510166, 1);

    // Отключение эффектов частиц от взрывов и выстрелов
    NOP(g_libGTASA + 0x3D373A, 2);
    NOP(g_libGTASA + 0x3DDA6A, 2);
    NOP(g_libGTASA + 0x4E55E2, 2);
    NOP(g_libGTASA + 0x519AC8, 2);

    // Упрощение системы отражений
    // Отключение отражений на воде, стеклах и блестящих поверхностях.
    WriteMemory(g_libGTASA + 0x541AC4, (uintptr_t)"\xF7\x46", 2);
    WriteMemory(g_libGTASA + 0x4B3B80, (uintptr_t)"\xF7\x46", 2);
    CHook::RET(g_libGTASA + 0x39B1E4);

    // Отключение тумана и других атмосферных эффектов
    NOP(g_libGTASA + 0x39A844 + 0x518, 2);
    NOP(g_libGTASA + 0x39A844 + 0x540, 2);
}
void applymyBlackRussiaPatches() {
	// Отключение динамических теней от источников света.
	// Многие игры рассчитывают динамические тени от фонарей, фар и т.д., 
	// что может быть ресурсоемко. Отключаем эту функцию.
	NOP(g_libGTASA + 0x51E280, 4);

	// Отключение просчета погодных эффектов (например, капель дождя).
	// Пропускаем инструкции, которые отвечают за рендеринг частиц дождя и брызг.
	NOP(g_libGTASA + 0x3D373A, 2);
	NOP(g_libGTASA + 0x3DDA6A, 2);
	WriteMemory(g_libGTASA + 0x3D4244, (uintptr_t)"\xF7\x46", 2);

	// Отключение/упрощение отрисовки отражений на воде.
	// Меняем инструкции, чтобы отражения на воде не просчитывались или были менее детализированными.
	WriteMemory(g_libGTASA + 0x541AC4, (uintptr_t)"\xF7\x46", 2);

	// Отключение сложной системы коллизий для мелких объектов.
	// Заменяем часть кода, которая обрабатывает коллизии для каждого мелкого объекта в мире, на NOP.
	NOP(g_libGTASA + 0x2467AA, 1);
	NOP(g_libGTASA + 0x4874E0, 5);

	// Упрощение логики NPC-трафика.
	// Отключаем часть кода, отвечающую за сложную логику поведения транспорта, 
	// чтобы снизить нагрузку на CPU.
	NOP(g_libGTASA + 0x45477E, 2);

	// Отключение эффекта "горения" шин.
	// Отключаем визуальный эффект дыма при резком торможении или старте.
	NOP(g_libGTASA + 0x556914, 2);

	// Устранение эффекта размытия при движении (motion blur).
	// Полностью отключаем функцию, которая добавляет эффект motion blur.
	NOP(g_libGTASA + 0x405398, 0xE);
	
	// Отключение загрузки лишних объектов.
	// Пропускаем часть кода, которая может загружать невидимые или второстепенные объекты.
	NOP(g_libGTASA + 0x4BEBE6, 2);
}
...
void ApplyInGamePatches()
{
	...
	*(uint8_t*)(g_libGTASA + 0x4EE200) = 0x9B;
	...
	WriteMemory(g_libGTASA + 0x5292CC, (uintptr_t)"\x70\x47", 2); // CClouds::Render
    	*(float*)(g_libGTASA + 0x608558) = 0.4f; // Эффект разгона авто
 	WriteMemory(g_libGTASA + 0x53EC78, (uintptr_t)"\xF7\x46", 2); // remove motion blur in high speed vehicle
