...
/*void CPlayerPed::FireInstant()
{
    ...
    g_pCurrentBulletData = m_bHaveBulletData ? &m_bulletData : nullptr;
...
/*void CPlayerPed::FireInstant()
{
    ...
    if (pSlot)
    {
        const eWeaponType weaponType = static_cast<const eWeaponType>(GetCurrentWeapon());
...
void CPlayerPed::ProcessAttach()
{
    ...
    for (auto iter : m_aAttachedObject)
    {
        ...
        if (!pObject)
        {
            FLog("AttachObject: object is null");
            continue;
        }
        ...
        if (!m_pPed->IsAdded() || !m_pPed->m_pRwClump)
        {
            pObject->m_pEntity->SetPosn(0.0f, 0.0f, 0.0f);
            continue;
        }
        ...
        pObject->m_pEntity->SetMatrix((CMatrix&)outMat);
        pObject->m_pEntity->UpdateRW();
        pObject->m_pEntity->UpdateRwFrame();
        pObject->m_pEntity->Add();
