			...
			if (pNetGame)
			{
				...
				if (pPlayerPool)
				{
					...
					if (pPlayerPed)
					{
						...
						if (pPlayerPed->GetCurrentWeapon() != WEAPON_MOLOTOV || btData->pEntity)
						{
							if (btData->pEntity)
							{
...
void CPlayerPed::SetInterior(uint8_t byteID)
{
	...
	CStreaming::RemoveBuildingsNotInArea(byteID);

	ScriptCommand(&select_interior, byteID);
	ScriptCommand(&link_actor_to_interior, m_dwGTAId,static_cast<int>(byteID));
...
void CPlayerPed::TogglePlayerControllable(bool bToggle)
{
	...
	if(pChatWindow->showinghud){*(uint8_t*)(g_libGTASA+0x8EF36B) = 0;}
