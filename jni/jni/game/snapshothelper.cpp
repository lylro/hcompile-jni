...
CRenderTarget* CSnapShotHelper::m_pRenderTarget = nullptr;
void CSnapShotHelper::SetUpScene()
{
	...
	*(uintptr_t*)(m_camera + 0x64) = m_zBuffer;
	...
	m_pRenderTarget = new CRenderTarget(512, 512, true, 000000);
}
...
uintptr_t CSnapShotHelper::CreatePedSnapShot(int iModel, uint32_t dwColor, VECTOR* vecRot, float fZoom)
{
	...
	uintptr_t raster = (uintptr_t)RwRasterCreate(512, 512, 32, rwRASTERFORMAT8888 | rwRASTERTYPECAMERATEXTURE);
	// RwTextureCreate
	...
	if(vecRot->Y != 0.0f)
		RwMatrixRotate(&mat, 0, vecRot->Y);
	if(vecRot->Z != 0.0f)
		RwMatrixRotate(&mat, 0, vecRot->Z);
...
uintptr_t CSnapShotHelper::CreateVehicleSnapShot(int iModel, uint32_t dwColor, VECTOR* vecRot, float fZoom, int dwColor1, int dwColor2)
{
 ...
 //   color_white.G = 255;
  //  color_white.R = 255;
  //  RwTexture* texture = (RwTexture*) bufferTexture;
...
//    m_pSprite2d->m_pRwTexture = texture;
 ...
 //   m_pSprite2d->Draw(0,0,texture->raster->height, texture->raster->width, color_white);
   ...
   // return (uintptr_t)m_pRenderTarget->End();
