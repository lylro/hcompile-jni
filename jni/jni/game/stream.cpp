...
void CStream::ProcessAttached() // todo
{
	...
	if (m_iAttachType == 1) // player
	{
		CPlayerPed* pPed = nullptr;
		if (pNetGame->GetPlayerPool()->GetLocalPlayerID() == m_iAttachedTo)
		{
			pPed = pNetGame->GetPlayerPool()->GetLocalPlayer()->GetPlayerPed();
		}
		else
		{
			if (pNetGame->GetPlayerPool()->GetSlotState(m_iAttachedTo))
				pPed = pNetGame->GetPlayerPool()->GetAt(m_iAttachedTo)->GetPlayerPed();
		}
