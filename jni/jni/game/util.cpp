...
{
	static char* texdb[] = { "samp", "gta3", "gta_int", "player", "txd", "flame" };
...
eWidgetState widgets::ProcessFixed(WIDGET_TYPE* pWidget)
{
	switch (pWidget->HID_ID)
	{
	case HID_ATTACK:
		if(pGame->FindPlayerPed()->IsInVehicle()) return STATE_FIXED;
		break;
	case HID_SPRINT:
		if(pGame->FindPlayerPed()->IsInVehicle() || pNetGame->GetPlayerPool()->GetLocalPlayer()->IsSpectating()) return STATE_FIXED;
		break;
	case HID_ACCELERATE:
		if(!pGame->FindPlayerPed()->IsInVehicle()) return STATE_FIXED;
		break;
	case HID_ENTERCAR:
		...
		if(idPoolVeh != INVALID_VEHICLE_ID) if(pNetGame->GetVehiclePool()->GetAt(idPoolVeh)) if(pNetGame->GetVehiclePool()->GetAt(idPoolVeh)->m_pEntity)
			if( !pGame->FindPlayerPed()->IsInVehicle() && pNetGame->GetVehiclePool()->GetAt(idPoolVeh)->m_pEntity->mat)
				if(GetDistance(&pGame->FindPlayerPed()->m_pEntity->mat->pos, &pNetGame->GetVehiclePool()->GetAt(idPoolVeh)->m_pEntity->mat->pos) > 10.0f) 
					return STATE_FIXED;
