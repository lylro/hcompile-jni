...
extern uintptr_t GetTexture_hook(const char* a1, const char* a2);
uintptr_t LoadTextureFromDB(const char* dbname, const char* texture)
{
	...
	if(!db_handle)
	{
		Log(OBFUSCATE("Error: Database not found! (%s)"), dbname);
		return 0;
	}

	// TextureDatabaseRuntime::Register(db)
	(( void (*)(uintptr_t))(g_libGTASA + 0x1BE898 + 1))(db_handle);
	uintptr_t tex = GetTexture_hook(texture, 0);
...
uintptr_t LoadTexture(const char* texname)
{
	...
	static char *texdb[] = { OBFUSCATE("samp"), OBFUSCATE("mobile"), OBFUSCATE("txd"), OBFUSCATE("menu"), OBFUSCATE("gta3"), OBFUSCATE("gta_int"), OBFUSCATE("player") };
...
{	
	if(!pObject->m_MaterialTexture || rpAtomic->object.object.type != 1) return rpAtomic;

	RpGeometry* pGeometry = rpAtomic->geometry;
	int iTotalEntries = pGeometry->matList.numMaterials;
	if (iTotalEntries > 15) iTotalEntries = 15;
	if (iTotalEntries < 1) return rpAtomic;
	
	RpMaterial** ppMaterialsArr = pGeometry->matList.materials;
	for (int i = 0; i < iTotalEntries; i++)
	{
		if(pObject->m_MaterialTexture[i])
		{
			ppMaterialsArr[i]->texture = (RwTexture*)pObject->m_MaterialTexture[i];
		}
	}
