...
uintptr_t LoadTextureFromDB(const char* dbname, const char* texture)
{
	...
	if (!tex) 
	{
		Log("Error: Texture (%lc) not found in database (%s)", (wchar_t) texture, dbname);
		return 0;
	}
...
uintptr_t LoadTexture(const char* texname)
{
	...
	Log(OBFUSCATE("-> Texture %lc not found!"), (wchar_t) texname);
...
{	
	...
	for (int i = 0; i < iTotalEntries; i++)
	{
		...
		{
			ppMaterialsArr[i]->texture = (RwTexture*)pObject->m_MaterialTextTexture[i];
		}
	}
