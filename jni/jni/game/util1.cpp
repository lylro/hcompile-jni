...
ENTITY_TYPE *GamePool_GetAt_Object(int iID)
{
	return (( ENTITY_TYPE* (*)(int))(g_libGTASA + 0x41DDB4 + 1))(iID);
}

int GamePool_Object_GetIndex(ENTITY_TYPE *pObject)
{
	return (( int (*)(ENTITY_TYPE*))(g_libGTASA + 0x41DD98 + 1))(pObject);
}

eEntityType GetEntityType(ENTITY_TYPE* pEntity)
{
	unsigned char value = *(unsigned char *)((uintptr_t)pEntity + 0x36) & 7;
...
uintptr_t GetTexture(const char* texture) {
	uintptr_t pRwTexture = ((uintptr_t(*)(char const*))(g_libGTASA + 0x001BE990 + 1))(texture);
	if (pRwTexture) {
...
uintptr_t LoadTextureFromDB(const char* dbname, const char* texture)
{
	...
	if(!db_handle)
	{
		Log(OBFUSCATE("Error: Database not found! (%s, %s)"), dbname, texture);
		return 0;
	}
...
std::vector<const char *> crashedTextures;
...
uintptr_t LoadTexture(const char* texname)
{
	...
	// -- If the texture has not been found previously, you do not need to search again
	for (auto & crashedTexture : crashedTextures)
	{
		if (crashedTexture == texname)
			return 0;
	}

	static char *texdb[] = { "samp", "mobile", "txd", "menu", "gta3", "gta_int", "player" };

	for (auto & dbname : texdb)
	{
		uintptr_t texture = LoadTextureFromDB(dbname, texname);
		if (texture != 0)
		{
			Log(OBFUSCATE("%s loaded from %s"), texname, dbname);
	...
	crashedTextures.emplace_back(texname);
	Log(OBFUSCATE("-> Texture %s not found!"), texname);
...
uintptr_t LoadFromTxdSlot(const char* szSlot, const char* szTexture)
{
	...
	if(!tex)
	{
		tex = (RwTexture*)LoadTextureFromDB(szSlot, szTexture);
	}

	Log(OBFUSCATE("%s loaded from %s"), szTexture, szSlot);
...
{
	if(!strcmp("accelerate", name)) SetById(TYPE_ACCELERATE, pWidget);
	if(!strcmp("hud_car", name)) SetById(TYPE_ENTERCAR, pWidget);
}
...
{
	eWidgetIds idWidget = GetId(pWidget);
	switch (idWidget)
	{
		case TYPE_NONE: return STATE_NONE;
		case TYPE_ATTACK:
		{
		...
		case TYPE_ENTERCAR:
		{
			if (!pGame->FindPlayerPed()->IsInVehicle() && !pGame->FindPlayerPed()->IsAPassenger())
			{
				...
				if (pVehiclePool) 
				{
					...
					if (idPoolVeh != INVALID_VEHICLE_ID && idPoolVeh < MAX_VEHICLES && pVehiclePool->GetSlotState(idPoolVeh))
					{
						CVehicle* pVehicle = pVehiclePool->GetAt(idPoolVeh);
						if (pVehicle)
						{
							if(GetDistanceBetween3DPoints(&pGame->FindPlayerPed()->m_pEntity->mat->pos, &pNetGame->GetVehiclePool()->GetAt(idPoolVeh)->m_pEntity->mat->pos) > 10.0f) 
									return STATE_FIXED;
						}
					}
				}
