...
{
	FLog("CVehicle %d %f %f %f %f %d", iType, fX, fY, fZ, fRotation, bSiren);
	RwMatrix matEnt;
	...
	if ((iType != TRAIN_PASSENGER_LOCO) &&
		...
		{
			FLog("veh model %d not found", iType);
            return;
		}
...
void CVehicle::AddComponent(int iComponentID)
{
    ...
    if (!CStreaming::TryLoadModel(iComponentID)) {
			FLog("Component Model not loaded");
			return;
	} 
...
void CVehicle::SetPaintJob(uint8_t bytePaintJobID)
{
	if (m_pVehicle && GamePool_Vehicle_GetAt(m_dwGTAId))
	{
		...
		{
			if (bytePaintJobID >= 0 && bytePaintJobID <= 3) {
...
//	}
    ...
    uintptr_t* dwModelarray = (uintptr_t*)(g_libGTASA + (VER_x32 ? 0x0091DCB8 : 0xB666E0));
    CVehicleModelInfo* pModelInfoStart = CModelInfo::GetVehicleModelInfo(m_pVehicle->m_nModelIndex);
    if (!pModelInfoStart)
    {
        return;
    }
