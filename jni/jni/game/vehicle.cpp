...
CVehicle::CVehicle(int iType, float fPosX, float fPosY, float fPosZ, float fRotation, bool bSiren)
{
		...
		(iType != TRAIN_TRAM)) 
	{
		...
		{
			Log("Warning: car model not preloaded. %d", iType);
			...
			while(!pGame->IsModelLoaded(iType))
			{
				...
				if(iTryCount > 20)
				{
					Log(OBFUSCATE("[CVehicle] Warning: Model %u wouldn't load in time!"), iType);
					return;
				}

				iTryCount++;
			}
			Log("Car model was preloaded. %d", iType);
		}

		m_pVehicle = ((VEHICLE_TYPE *(*)(int, unsigned char))(SA_ADDR(0x2A468C + 1)))(iType, 2); // 2A468C ; CCarCtrl::GetNewVehicleDependingOnCarModel(int, unsigned char)
		dwRetID = GamePool_Vehicle_GetIndex(m_pVehicle);
				
		CVector posn = {fPosX, fPosY, fPosZ};
		((void (*)(const CVector&, ENTITY_TYPE*))(SA_ADDR(0x300B7C + 1)))(posn, (ENTITY_TYPE*)m_pVehicle); // CTheScripts::ClearSpaceForMissionEntity
		((void (*)(ENTITY_TYPE*))(SA_ADDR(0x3C14B0 + 1)))((ENTITY_TYPE*)m_pVehicle); // CWorld::Add
		...
		TeleportTo(fPosX, fPosY, fPosZ);
		m_pEntity->physicalFlags.bExplosionProof = 0;
	...
	m_bStatusBonnet = false;
	m_bStatusBoot = false;
	m_bStatusDoor[0] = false;
	m_bStatusDoor[1] = false;
	m_bStatusDoor[2] = false;
	m_bStatusDoor[3] = false;
	...
	for (size_t i = 0; i < MAX_REPLACED_TEXTURES; i++)
	{
		...
		memset(&(m_szReplacedTextures[i].szNewTexture[0]), 0, MAX_REPLACED_TEXTURE_NAME);
	...
	m_brgbaNeedRendered = false;
	m_bVinylsNeedRendered = false;
	...
	if (pChatWindow) 
	{
		switch (m_pVehicle->entity.nModelIndex)
		{
			case 448:
			case 462:
			case 463:
			...
			case 3383:
			{
				m_pVehicle->entity.vtable = g_libGTASA + 0x5CCB18;
				break;
			}
			case 481:
			case 509:
			case 510:
			{
				m_pVehicle->entity.vtable = g_libGTASA + 0x5CCC30;
				break;
			}
			case 460: 
			case 464: 
			...
			case 3388:
			{
				m_pVehicle->entity.vtable = g_libGTASA + 0x5CD0B0;
				break;
			}
			case 417: 
			case 425: 
			...
			case 3372:
			{
				m_pVehicle->entity.vtable = g_libGTASA + 0x5CCE60;
				break;
			}
			case 406:
			case 444:
			...
			default:
			{
				if (m_pVehicle->m_type == 0 && m_pVehicle->entity.vtable != g_libGTASA + 0x5CC9F0)
				{
					m_pVehicle->entity.vtable = g_libGTASA + 0x5CC9F0;
					break;
				}
			}
		}
...
CVehicle::~CVehicle()
{
	...
	if(m_pVehicle)
	{
		...
		if(m_dwMarkerID != -1)
		{
			((void(*)(int, unsigned int))(SA_ADDR(0x3DC22C + 1)))(m_dwMarkerID, m_dwGTAId); // CRadar::ClearBlipForEntity
		...
		else
		{
			...
			Log("[CVehicle] Destroy vehicle");
			((void (*)(VEHICLE_TYPE*))(SA_ADDR(0x510918 + 1)))(m_pVehicle); // 510918 ; DestroyVehicleAndDriverAndPassengers(CVehicle *)
			m_pVehicle = nullptr;
			Log("[CVehicle] Destroy vehicle sucess");
...
/*
	...
	Чтоб работала замена компонентов через клиент,
	нужно собирать машины с названиями дамми, которые указаны
	в коллбэке GetComponentNameByIDs

*/

void CVehicle::BoolComponentVisible(uint8_t group, uint16_t components, bool bVisible)
{
	std::string szName = GetComponentNameByIDs(group, components);
	if (bVisible == false) SetComponentVisibleInternal(szName.c_str(), false);
	else if (bVisible == true) SetComponentVisibleInternal(szName.c_str(), true);
}
...
void CVehicle::ApplyVinyls(uint8_t bSlot1, uint8_t bSlot2, const char* szTex)
{
	if (bSlot1 == 0)
	{
		RemoveTexture(szTex);
		//RemoveTexture("ret_v_body1");
		return;
	}
	if (bSlot2 == 0)
	{
		RemoveTexture(szTex);
		//RemoveTexture("ret_v_body2");
		return;
	}

	char szText[MAX_REPLACED_TEXTURE_NAME];

	if (bSlot1 != 255)
	{
		sprintf(&szText[0], "v_cust_body_%d", bSlot1);
		if (strstr(szTex, "remap_cbody")) ApplyTexture(szTex, &szText[0]);
		else ApplyTexture(szTex, &szText[0]);
	}

	if (bSlot2 != 255)
	{
		sprintf(&szText[0], "v_cust_body_%d", bSlot2);
		if (strstr(szTex, "remap_cbody")) ApplyTexture(szTex, &szText[0]);
		else ApplyTexture(szTex, &szText[0]);
	}
}

/*void CVehicle::ApplyVinylsGenerate(uint8_t bSlot1, uint8_t bSlot2, const char* szTex, uint32_t dwColor)
{
	if (bSlot1 == 0)
	{
		RemoveTexture(szTex);
	...
	if (bSlot1 != 255)
	{
		...
		{
			if (dwColor != 0x0) ApplyTextureGenerate(&szOld[0], dwColor);
			else ApplyTexture(szTex, &szText[0]);
		}
	}
	...
	if (strstr(szTex, "remap_toner")) sprintf(&szOld[0], "%s%d", szTex, bSlot); // если в .dff будет несколько стёкол
	else sprintf(&szOld[0], szTex, bSlot);
	ApplyTextureGenerate(&szOld[0], dwColor);
}*/

void CVehicle::ApplyToner(uint8_t bSlot, uint8_t bID, const char* szTex)
{
...
void CVehicle::ApplyTonerGenerate(uint8_t bSlot, uint8_t bID, const char* szTex, uint32_t dwColor)
{
	char szOld[MAX_REPLACED_TEXTURE_NAME];
...
void CVehicle::ApplyTextureGenerate(const char* szTexture, uint32_t dwColor)
{
	...
	CTextRasterizer textRasterizer(8, 8);

	CColor colBackground{ CColor::eColorEndianness::COLOR_ENDIAN_ARGB, dwColor };
	textRasterizer.FillBitmapWithColor(colBackground);
	
	uint8_t* pBitmap = textRasterizer.GetBitmap();

	RwImage* pRwImage = RwImageCreate(8, 8, 4 * 8);
	if (!pRwImage) 
	{
	...
	for (int y = 0; y < 8; y++)
	{
		memcpy(pRwImage->cpPixels + pRwImage->stride * y, pBitmap + 8 * 4 * y, 8 * 4);
	}
	...
	m_szReplacedTextures[bID].pTexture = pRwTexture;
...
void CVehicle::ApplyTexture(const char* szTexture, const char* szNew)
{
	...
	m_bReplaceTextureStatus[bID] = true;
	strcpy(&(m_szReplacedTextures[bID].szOld[0]), szTexture);
	strcpy(&(m_szReplacedTextures[bID].szNewTexture[0]), szNew);
	m_szReplacedTextures[bID].pTexture = (RwTexture*)LoadTextureFromDB("gui", szNew);
...
void CVehicle::SetCustomShadow(uint8_t r, uint8_t g, uint8_t b, float fSizeX, float fSizeY, const char* szTex, const char* szStorage)
{
	...
	if (!strcmp(m_Shadow.szOld, szTex)) { }
	else
	{
		m_Shadow.pTexture = (RwTexture*)LoadTextureFromDB(szStorage, szTex);
		strcpy(&(m_Shadow.szOld[0]), szTex);
	}
}
...
void CVehicle::LinkToInterior(int iInterior)
{
	...
	m_pEntity->m_areaCode = iInterior;
}
...
float CVehicle::GetStaticHealth()
{
	return m_IsStaticHealth;
}
...
void CVehicle::ProcessMarkers()
{
	...
	if (m_byteObjectiveVehicle)
	{
		if (!m_bSpecialMarkerEnabled)
		{
			...
			{
				((void(*)(int, unsigned int))(SA_ADDR(0x3DC22C + 1)))(m_dwMarkerID, m_dwGTAId); // CRadar::ClearBlipForEntity
				//((void(*)(int))(SA_ADDR(0x3DC134 + 1)))(m_dwMarkerID); // CRadar::ClearActualBlip
				//ScriptCommand(&disable_marker, m_dwMarkerID);
				m_dwMarkerID = -1;
			}
			...
			m_dwMarkerID = ((int(*)(int, unsigned int, unsigned int, int))(SA_ADDR(0x3DB0A0 + 1)))(1, m_dwGTAId, TranslateColorCodeToRGBA(1006), 2); // CRadar::SetEntityBlip
			((void(*)(int, int))(SA_ADDR(0x3DB290 + 1)))(m_dwMarkerID, 1); // CRadar::ChangeBlipScale
			((void(*)(int, int))(SA_ADDR(0x3DB158 + 1)))(m_dwMarkerID, TranslateColorCodeToRGBA(1006)); // CRadar::ChangeBlipColour
	...
	if (GetDistanceFromLocalPlayerPed() < 200.0f && !IsOccupied())
	{
		if(m_dwMarkerID == -1)
		{
			...
			m_dwMarkerID = ((int(*)(int, unsigned int, unsigned int, int))(SA_ADDR(0x3DB0A0 + 1)))(1, m_dwGTAId, TranslateColorCodeToRGBA(1004), 2); // CRadar::SetEntityBlip
			((void(*)(int, int))(SA_ADDR(0x3DB290 + 1)))(m_dwMarkerID, 1); // CRadar::ChangeBlipScale
			((void(*)(int, int))(SA_ADDR(0x3DB158 + 1)))(m_dwMarkerID, TranslateColorCodeToRGBA(1004)); // CRadar::ChangeBlipColour
		}
	}
...
void CVehicle::SetLightsState(int iState)
{
	...
	m_pVehicle->m_nOverrideLights = iState > 0 ? 2 : 1;
	m_pVehicle->m_nVehicleFlags.bLightsOn = iState;

	m_bLightsState = iState;
...
void CVehicle::RemoveComponent(uint16_t uiComponent)
{
	...
	if (GamePool_Vehicle_GetAt(m_dwGTAId))
	{
		...
		((int32_t(*)(VEHICLE_TYPE*, int32_t))(g_libGTASA + 0x515EB8 + 1))(m_pVehicle, uiComponent);
	}
}

int32_t CVehicle::GetUpgrade(int32_t upgradeId)
{
	return ((int32_t(*)(VEHICLE_TYPE*, int32_t))(g_libGTASA + 0x515BD0 + 1))(m_pVehicle, upgradeId);
}
...
uint8_t CVehicle::getProcessTrailer()
{
	return bInProcessDetachTrailer;
}

void CVehicle::ProcessDamage(VEHICLEID vehId)
{
	if (!m_pVehicle || !m_dwGTAId) return;
	if (!GamePool_Vehicle_GetAt(m_dwGTAId)) return;
	if (pNetGame)
	{
		...
		if (vehId != INVALID_VEHICLE_ID)
		{
			if (HasDamageModel())
			{
				...
				if (IsOccupied()) GetDamageStatusEncoded(&m_byteTyreFlags, &m_byteLightFlags, &m_dwDoorFlags, &m_dwPanelFlags);
				if (m_byteTyreFlags != m_byteTyreStatus || m_byteLightFlags != m_byteLightStatus ||
					m_dwDoorFlags != m_dwDoorStatus || m_dwPanelFlags != m_dwPanelStatus)
				{
					m_byteLightStatus = m_byteLightFlags;
					m_byteTyreStatus = m_byteTyreFlags;
					m_dwDoorStatus = m_dwDoorFlags;
					m_dwPanelStatus = m_dwPanelFlags;

					RakNet::BitStream bsDamage;
						
					bsDamage.Write(vehId);
					bsDamage.Write(m_dwPanelFlags);
					bsDamage.Write(m_dwDoorFlags);
					bsDamage.Write(m_byteLightFlags);
					bsDamage.Write(m_byteTyreFlags);
...
void CVehicle::ProcessStrobs()
{
	...
	else if (m_iStrobsType == -2)
	{
		return;
	}
	else if (m_iStrobsType == 0)
	{
		m_iStrobsType = -2;
		SetLightsState(0);
		return;
	...
	if (m_bStrobsPolice)
	{
		if (m_iStrobsStage == 0 || m_iStrobsStage == 2)
		{
			SetHeadlightsColor(255, 0, 0);
		}
		else
		{
			SetHeadlightsColor(0, 0, 255);
		}
	}
	else
	{
		SetHeadlightsColor(m_bHeadlightsR, m_bHeadlightsG, m_bHeadlightsB);
	}
	...
	if (m_iStrobsType == 1)
	{
		if (m_iStrobsStage == 0)
		{
			SetLightState(1, 0);
			m_iStrobsStage++;
			m_uiLastTickStobs = GetTickCount();
		}
		if (GetTickCount() - m_uiLastTickStobs >= 100 && m_iStrobsStage == 1)
		{
			SetLightState(0, 0);
			SetLightState(1, 1);
			m_iStrobsStage++;
		...
		if (GetTickCount() - m_uiLastTickStobs >= 100 && m_iStrobsStage == 2)
		{
			SetLightState(0, 1);
			SetLightState(1, 0);
			m_iStrobsStage++;
		...
		if (GetTickCount() - m_uiLastTickStobs >= 100 && m_iStrobsStage == 3)
		{
			SetLightState(1, 0);
			SetLightState(1, 1);
			m_iStrobsStage = 0;
	...
	else if (m_iStrobsType == 2)
	{
		...
		if (GetTickCount() - m_uiLastTickStobs >= 80 && m_iStrobsStage == 0)
		{
			SetLightState(1, 0);
			m_iStrobsStage++;
			m_uiLastTickStobs = GetTickCount();
		}
		if (GetTickCount() - m_uiLastTickStobs >= 80 && m_iStrobsStage == 1)
		{
			SetLightState(0, 0);
			m_iStrobsStage++;
			m_uiLastTickStobs = GetTickCount();
		}
		if (GetTickCount() - m_uiLastTickStobs >= 80 && m_iStrobsStage == 2)
		{
			SetLightState(1, 0);
			m_iStrobsStage++;
			m_uiLastTickStobs = GetTickCount();
		}
		if (GetTickCount() - m_uiLastTickStobs >= 80 && m_iStrobsStage == 3)
		{
			SetLightState(0, 1);
			m_iStrobsStage++;
			m_uiLastTickStobs = GetTickCount();
		}
		if (GetTickCount() - m_uiLastTickStobs >= 80 && m_iStrobsStage == 4)
		{
			SetLightState(1, 1);
			m_iStrobsStage++;
			m_uiLastTickStobs = GetTickCount();
		}
		if (GetTickCount() - m_uiLastTickStobs >= 80 && m_iStrobsStage == 5)
		{
			SetLightState(0, 1);
			m_iStrobsStage++;
			m_uiLastTickStobs = GetTickCount();
		}
		if (GetTickCount() - m_uiLastTickStobs >= 80 && m_iStrobsStage == 6)
		{
			SetLightState(1, 1);
			m_iStrobsStage = 0;
			m_uiLastTickStobs = GetTickCount();
...
void CVehicle::SetStrobs(int iType, bool bStrobs)
{
	if(!m_pVehicle) return;
	if (GetVehicleSubtype() != VEHICLE_SUBTYPE_CAR)
	{
	...
	m_iStrobsType = iType;
	m_iStrobsStage = 0;
	m_bStrobsPolice = bStrobs;
	m_uiLastTickStobs = GetTickCount();
	SetLightState(0, 0);
	SetLightState(1, 0);
}

void CVehicle::SetLightState(int iLight, int iState)
{
	if (!m_pVehicle || GetVehicleSubtype() != VEHICLE_SUBTYPE_CAR) return;
	((void(*)(uintptr_t, int, int))(g_libGTASA + 0x4F9380 + 1))((uintptr_t)((uintptr_t)m_pVehicle + 1456), iLight, iState); // CAutomobile::OpenDoor
}
...
void CVehicle::Process()
{
	if (!m_pVehicle || !m_dwGTAId) return;
	if (!GamePool_Vehicle_GetAt(m_dwGTAId)) return;
	if (GetDistanceFromLocalPlayerPed() < 200.0f)
	{
		...
		if (pNetGame)
		{
			if (pNetGame->GetPlayerPool())
			{
				if (pNetGame->GetPlayerPool()->GetLocalPlayer())
				{
					...
					if (pPed && pPed->IsInVehicle() && CFirstPersonCamera::IsEnabled())
					{
						distance = 50.0f;
					}
					else if (pPed && pPed->IsInVehicle() && !CFirstPersonCamera::IsEnabled())
					{
						distance = 25.0f;
					}
				}
		...
		if(rwOutResult.z < 1.0f && GetDistanceFromLocalPlayerPed() > distance)
		{
			if (IsAdded())
			{
				 Remove();
		...
		else
		{
			...
			{
				Add();
				SetColor(m_byteColor1, m_byteColor2);
			}
			...
			{
				EnableEngine(GetEngineState() == 1);
			}
			else
			{
				if (GetEngineState() == -1)
				{
					if (!IsDriverLocalPlayer())
						EnableEngine(false);
								
					else
						EnableEngine(true);
				}
				else if (GetEngineState() != -1)
				{
					if (GetEngineState() == 0)
						EnableEngine(false);
		...
		if (IsOccupied()) SetInvulnerable(false);
		else SetInvulnerable(true);
		
		if (IsInvulnerable() == true)
		{
			SetHealth(GetStaticHealth());
		}
	}
	else 
	{
...
bool showNotyBig = false;
bool lastSendVehicle = false;
int32_t lastVehicleTick = GetTickCount();
void CVehicle::ProcessContact(VEHICLEID vehId)
{
	if (!m_pVehicle || !m_dwGTAId) return;
	if (!GamePool_Vehicle_GetAt(m_dwGTAId)) return;
	
	CPlayerPed *pPlayerPed = pGame->FindPlayerPed();
	if (GetDistanceFromLocalPlayerPed() < 4.0f && !pPlayerPed->IsInVehicle() && !pPlayerPed->IsAPassenger() && pPlayerPed->GetInterior() < 1)
	{
		if (!showNotyBig && !pSettings->GetReadOnly().iButtonVehicle)
		{
			showNotyBig = true;
			CNoty::showNotyBig("Для взаимодействия с транспортом нажмите на него (модель), затем нажмите ещё раз до заполнения шкалы прогресса");
		}
		
		if (GetTickCount() - lastVehicleTick > 1000)
		{
			lastVehicleTick = GetTickCount();
				
			/*RakNet::BitStream bsParams;
			bsParams.Write((uint8_t)ID_CUSTOM_RPC);
			bsParams.Write((uint32_t)RPC_VehicleDistance);

			bsParams.Write((int32_t)vehId);
			pNetGame->GetRakClient()->Send(&bsParams, HIGH_PRIORITY, RELIABLE, 0);*/
			
			lastSendVehicle = true;
		}
	}
	else if (lastSendVehicle)
	{
		if (showNotyBig && !pSettings->GetReadOnly().iButtonVehicle)
		{
			showNotyBig = false;
		...
		lastSendVehicle = false;
	}
}

void CVehicle::ProcessTypes()
{
	if (!m_pVehicle || !m_dwGTAId) return;
	if (!GamePool_Vehicle_GetAt(m_dwGTAId)) return;
	...
	switch (m_pVehicle->entity.nModelIndex)
	{
		...
		case 593:
		{
			if (m_pVehicle->entity.vtable != g_libGTASA + 0x5CD0B0)
			{
				Log("Model %d not vtable accept", m_pVehicle->entity.nModelIndex);
		...
		case 3372:
		{
			if (m_pVehicle->entity.vtable != g_libGTASA + 0x5CCE60)
			{
				Log("Model %d not vtable accept", m_pVehicle->entity.nModelIndex);
				if (pNetGame->GetRakClient())
					pNetGame->GetRakClient()->Disconnect(500, 0);
