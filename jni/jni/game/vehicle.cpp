...
CVehicle::CVehicle(int iType, float fPosX, float fPosY, float fPosZ, float fRotation, bool bSiren)
{
			...
			(iType == TRAIN_TRAM))
	{
		...
		if (iType == TRAIN_PASSENGER_LOCO)
		{
			...
			{
				int modelid = TRAIN_PASSENGER_LOCO;
		...
		else if (iType == TRAIN_FREIGHT_LOCO)
		{
			...
			{
				int modelid = TRAIN_FREIGHT_LOCO;
				pGame->RequestModel(modelid);
				pGame->LoadRequestedModels();
...
void CVehicle::SetWheelOffset(int iWheel, float offset)
{
    ...
    Log("set wheel offset on %d is %f", iWheel, offset);
...
void CVehicle::ProcessWheelsOffset()
{
	...
	if (!m_bWasWheelOffsetProcessedY)
	{
		if (m_bWheelOffsetY)
		{
            ...
            Log("pWheelLF: %f | pWheelRF: %f", m_fWheelOffsetX, m_fWheelOffsetX);
		}
		m_bWasWheelOffsetProcessedY = true;
	}
...
void CVehicle::SetCustomShadow(uint8_t r, uint8_t g, uint8_t b, float fSizeX, float fSizeY, const char* szTex)
{
	...
	m_Shadow.pTexture = (RwTexture*)LoadTextureFromDB(OBFUSCATE("gui"), szTex);
}
...
bool CVehicle::CheckAccessComponent(int component){
    if(component == 1087){
        if(m_pVehicle->entity.nModelIndex == 573){
            return false;
        }
    }
    return true;
