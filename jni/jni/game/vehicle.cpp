...
/*                  m_bIsOnRightTurnLight = toggle;
	...
	CVRect vecFront;
	vecFront.x = m_fWheelOffsetX[0].X + 0.1f;
	vecFront.y = m_fWheelOffsetY[0].Y;

	CVRect vecRear;
	vecRear.x = m_fWheelOffsetX[1].X + 0.1f;
	vecRear.y = m_fWheelOffsetY[1].Y;
	...
	m_pRightFrontTurnLighter = pGame->NewObject(19294, 0.0, 0.0, 0.0, VECTOR, 300.0);
                  m_pRightFrontTurnLighter->AttachToVehicle(getSampId(), &vecFront, &vecFront);

	m_pRightRearTurnLighter = pGame->NewObject(19294, 0.0, 0.0, 0.0, VECTOR, 300.0);
...
/*    m_bIsOnLeftTurnLight = toggle;
    ...
    CVehicleModelInfo* pModelInfoStart = static_cast<CVehicleModelInfo *>(CModelInfo::GetModelInfo(m_pVehicle->nModelIndex));
