...
public:
	...
	void SetLightShadow(int a1, float *a2);
...
void ProcessDamage();
	...
	void SetFuel(int r);
	int GetLightsState();
	void CreateExhaust(VEHICLE_TYPE* a1);
	void SetBootAndBonnetState(int iBoot, int iBonnet);

	void RemoveComponent(uint16_t uiComponent);
	
	uint8_t GetTurnLights();
	uint8_t GetOldLights();
	void TurnLights(uint8_t bSlot);
	void EnableTurnLights();
...
public:
	...
	// поворотники
	RwRGBA tonerColor = {0, 255, 0, 255};
	RwRGBA mainColor = {0, 0, 0, 255};
    RwRGBA wheelColor = {0, 0, 0, 255};
	RwRGBA secondColor = {0, 0, 0, 255};
	RwRGBA lightColor;
	RwRGBA neonColor;
	...
	CObject*		m_pExhaust = nullptr;
	CObject*		m_pExhaust2 = nullptr;
