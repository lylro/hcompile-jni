...
struct SReplacedTexture
{
	...
	char szNewTexture[MAX_REPLACED_TEXTURE_NAME];
	RwTexture* pTexture;
};
...
public:
	...
	void ProcessDamage(VEHICLEID vehId);
	...
	CVehicle* GetTrailer();
	uint8_t getProcessTrailer();

	void RemoveComponent(uint16_t uiComponent);
	int32_t GetUpgrade(int32_t upgradeId);

	void BoolComponentVisible(uint8_t group, uint16_t components, bool bVisible);
	...
	void ApplyVinyls(uint8_t bSlot1, uint8_t bSlot2, const char* szTex);
	void ApplyToner(uint8_t bSlot, uint8_t bID, const char* szTex);
	void ApplyTonerGenerate(uint8_t bSlot, uint8_t bID, const char* szTex, uint32_t dwColor);

	void ApplyTextureGenerate(const char* szTexture, uint32_t dwColor);
	...
	void SetCustomShadow(uint8_t r, uint8_t g, uint8_t b, float fSizeX, float fSizeY, const char* szTex, const char* szStorage);
	...
	void ProcessStrobs();
	void SetStrobs(int iType, bool bPolice);
	void SetLightState(int iLight, int iState);
	
	void Process();
	void ProcessContact(VEHICLEID vehId);
	void ProcessTypes();
...
public:
	...
	bool 			m_bIsLocked;
	CVehicle		*m_pTrailer;
	int32_t			m_pTrailerToVehicle;
	int32_t			m_dwMarkerID;
	...
	uint8_t			m_bLightsState;

	uint8_t			m_byteColor1;
	uint8_t			m_byteColor2;
	bool 			m_bColorChanged;
	
	float			m_IsStaticHealth;
	
	uint8_t 		m_byteTyreFlags;
	uint8_t 		m_byteLightFlags;
	uint32_t 		m_dwDoorFlags; 
	uint32_t 		m_dwPanelFlags;
	...
	uint32_t 		m_uiLastTickStobs;
	int 			m_iStrobsType;
	int 			m_iStrobsStage;
	bool 			m_bStrobsPolice;
	...
	bool m_brgbaNeedRendered;
	uint8_t Rgba;
	uint8_t rGba;
	uint8_t rgBa;
	uint8_t rgbA;
	
	bool m_bVinylsNeedRendered;
	uint8_t VinylsId;

	SCustomCarShadow m_Shadow;
	bool m_bShadow;
	
	bool m_bStatusBonnet;
	bool m_bStatusBoot;
	bool m_bStatusDoor[4];
};