...
void CSkyBox::Process()
{
	...
	if (m_pSkyObject)
	{
		...
		//matrix.pos.Z = aim->pos1z - 10.0f; // ???? ?????? ?? ????? 10 ????
...
CObject* CSkyBox::CreateObjectScaled(int iModel, float fScale)
{
	...
	*(uint8_t*)((uintptr_t)object->m_pEntity + 29) |= 1;
	...
	//RwMatrixScale(&matrix, 1.0f, 1.0f, 0.5f); // ????? ?? ???? Z ??? ??? ???
...
void CSkyBox::ReTexture()
{
	...
	if (pChangeTime != iHours)
	{
		...
		if (iHours >= 0 && iHours <= 5)
			SetTexture("night_user");

		if (iHours >= 6 && iHours <= 10)
			SetTexture("afternoon_user");

		if (iHours >= 11 && iHours <= 15)
			SetTexture("daily_user1");
		
		if (iHours >= 16 && iHours <= 18)
			SetTexture("daily_user");

		if (iHours >= 19 && iHours <= 20)
			SetTexture("evening_user");
		
		if (iHours >= 21 && iHours <= 24)
			SetTexture("night_user");
	}
	// ---
