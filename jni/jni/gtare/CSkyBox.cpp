...
float CSkyBox::m_fRotSpeed = 0.0065f;

uint8_t pChangeTime;
MATRIX4X4* RwMatrixMultiplyByVector(VECTOR* out, MATRIX4X4* a2, VECTOR* in);

// TODO: размытый скайбокс + обычный в одно целое
void CSkyBox::Initialise()
{
	auto* dwModelArray = (uintptr_t*)(SA_ADDR(0x87BF48));
	if (!dwModelArray[18659]) return;

	m_pSkyObject = CreateObjectScaled(18659, 1.855f);
...
//		return;
//
//	m_pSkyObject = CreateObjectScaled(17476, 0.8f);
//	SetTexture("daily_sky_1");
}
...
CObject* CSkyBox::CreateObjectScaled(int iModel, float fScale)
{
	...
	auto *object = pGame->NewObject(iModel, 0.0f, 0.0f, -250.0f, vecRot->Get(), 5.0f);
