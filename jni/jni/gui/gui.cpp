...
extern "C"
{
	...
	{
        //((void(*)())(g_libGTASA + 0x26CC24 + 1))();
        //((void(*)(uintptr_t*))(g_libGTASA + 0x2597C0 + 1))(mapthiz);
        //((void(*)(uintptr_t*))(g_libGTASA + 0x2569D8 + 1))(mapthiz);
        if (pVoice)
			{
				...
				if (pVoice) // 3
				{
					...
					{
						pChatWindow->AddDebugMessage("{FFFF00}| {FFFFFF}Voice status: {00ff00}ON MICRO{FFFFFF} (3)");
                        pVoice->TurnRecording();
					}
					g_pJavaWrapper->VoiceOn();
				}
				else // 5
				{
					pChatWindow->AddDebugMessage("{FFFF00}| {FFFFFF}Voice status: {00ff00}OFF MICRO{FFFFFF} (5)");
                    pVoice->DisableInput();
					g_pJavaWrapper->VoiceOff();
				}
			}
	}
}
...
void CGUI::ShowSpeed(){
	...
	g_pJavaWrapper->UpdateSpeedInfo(i_speed, m_fuel, bHealth, bMeliage, bEngine, bLights, 0, bDoor, 0);
}
