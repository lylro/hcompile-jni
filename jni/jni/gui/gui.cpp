...
void CGUI::Render()
{
	...
	if(this->m_bMemDebugOn) {
		...
		ImGui::SetNextWindowPos(ImVec2(50.0f, io.DisplaySize.y / 2.0 - vecButSize.x / 2));
		...
		ImGui::Text(" "); 
		ImGui::Text("t.me/rkkdev", *(uint32_t*)(g_libGTASA + 0x670680));
		ImGui::Text(" "); //a merixton cocet xa xa

 		if(ImGui::Button("Objects info", ImVec2(ImGui::CalcTextSize("Object info").x + 64, 64))) 
 		{
...
void CGUI::ShowSpeed() {
	if(m_bIsRegistr || pKeyBoard->IsOpen() || pDialogWindow->m_bIsActive) {
		g_pJavaWrapper->HideSpeed();
		return;
	}
	if (!pGame || !pNetGame || !pNetGame->GetGameState() == GAMESTATE_CONNECTED || !pGame->FindPlayerPed()->IsInVehicle()) {
		g_pJavaWrapper->HideSpeed();
		bMeliage =0;
		m_fuel = 0;
		return;
