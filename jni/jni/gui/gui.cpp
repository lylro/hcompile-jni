...
static bool m_bEnableDebug = false;
void CGUI::Render()
{
	PreProcessInput();
    ...
    // .. TODO: full menu, but for what?
    // notsa::ui::DoNestedMenuIL({ "Visualization", "Extra" }, [&] {
    //     ImGui::Checkbox("Enabled", &m_bEnableDebug);
    // });
    notsa::ui::Render();
    ...
    if (!pKeyBoard->IsOpen() && pNetGame && !pDialogWindow->m_bIsActive && pGame->IsToggledHUDElement(HUD_ELEMENT_BUTTONS))
	{
			ImGuiIO& io = ImGui::GetIO();
    ...
    if (pNetGame)
	{
		if (!pKeyBoard->IsOpen() && pVoice && g_IsVoiceServer())
		{
