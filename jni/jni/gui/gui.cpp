...
void CGUI::DrawCustomMenu()
{
    static bool menuOpen = false;

    // Отрисовка кнопки меню
    if (Button("btn_menu", ImVec2(120, 40), &menuOpen))
    {
        // Триггер на нажатие кнопки
        if(menuOpen) {
            CChat::AddDebugMessage(xorstr("Menu button pressed!"));
            // Тут можно вызвать любую логику, которая нужна
        }
    }

    // Если меню открыто, рисуем его содержимое
    if(menuOpen)
    {
        ImGui::Begin("Menu Window", &menuOpen, ImGuiWindowFlags_AlwaysAutoResize);
        ImGui::Text("Hello! Menu is open.");
        ImGui::End();
    }
}
