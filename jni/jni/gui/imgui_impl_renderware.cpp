...
void ImGui_ImplRenderWare_RenderDrawData(ImDrawData* draw_data)
{
    ...
    uintptr_t thizRenderQuere = *(uintptr_t*)(SA_ADDR(0x61B2E8));
    /*if(thizRenderQuere){
        Log("thizRenderQuere: 0x%X", *(int*)(thizRenderQuere+0x264) - ((uintptr_t)*(void**)(thizRenderQuere+0x260)));
        if(*(int*)(thizRenderQuere+0x264) - ((uintptr_t)*(void**)(thizRenderQuere+0x260)) <= 0x300000){
            static int size = 0x5000000;

            void* buff = malloc(size);
            *(void**)(thizRenderQuere+0x260) = buff;

            *(int*)(thizRenderQuere+0x264) = size + (uintptr_t)buff;

            *(void**)(thizRenderQuere+0x268) = buff;
            *(void**)(thizRenderQuere+0x270) = buff;
            *(void**)(thizRenderQuere+0x274) = buff;
        }
    }*/
	if (g_VertexBufferSize < MAX_VERTEXS)
	{
		if (!g_pVB || g_VertexBufferSize < draw_data->TotalVtxCount)
		{
			...
			if (!g_pVB)
			{
				Log(OBFUSCATE("GUI | Error: couldn't allocate vertex buffer (size: %d)"), g_VertexBufferSize);
				return;
			}

			Log(OBFUSCATE("GUI | Vertex buffer reallocated. Size: %d"), g_VertexBufferSize);
		}
	}
...
bool ImGui_ImplRenderWare_Init()
{
	...
	Log(OBFUSCATE("GUI | Display size: %f, %f"), io.DisplaySize.x, io.DisplaySize.y);
...
{
	Log(OBFUSCATE("GUI | CreateDeviceObjects."));
	if(g_FontRaster)
		Log(OBFUSCATE("GUI | Warning: Font raster != 0"));
	...
	Log(OBFUSCATE("GUI | Font atlas width: %d, height: %d, depth: %d"), width, height, bytes_per_pixel*8);
