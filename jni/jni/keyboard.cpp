...
#include "util/interfaces/CRecon.h"
#include "gui/gui.h"
#include "game/game.h"
#include "keyboard.h"
#include "scrollbar.h"
...
extern uint32_t col_all1;
extern uint32_t col_all2;
extern uint32_t col_all3;
...
void CKeyBoard::Render()
{
	...
	if (ImGui::Button(OBFUSCATE("UP"), vecButSize))
	{
		m_pkHistory->PageUp();
	}

	if (ImGui::Button(OBFUSCATE("DOWN"), vecButSize))
	{
	...
	ImGui::GetOverlayDrawList()->AddLine(
		...
		ImVec2(m_Size.x, m_Pos.y + m_fKeySizeY), ImColor(col_all1, col_all2, col_all3, 0xFF));
		...
		for (auto key : m_Rows[m_iLayout][i])
		{
			...
			case KEY_SHIFT:
				ImGui::GetOverlayDrawList()->AddTriangleFilled(
					...
					m_iCase == LOWER_CASE ? ImColor(0xFF8A8886) : ImColor(col_all1, col_all2, col_all3, 0xFF));
				ImGui::GetOverlayDrawList()->AddRectFilled(
			...
			case KEY_BACKSPACE:
				...
				ImGui::GetOverlayDrawList()->AddConvexPolyFilled(points, 5, ImColor(0xFF8A8886));
				break;
			...
			case KEY_SPACE:
				ImGui::GetOverlayDrawList()->AddRectFilled(
					...
					key.id == m_iPushedKey && !IsHidden() ? ImColor(col_all1, col_all2, col_all3, 0xFF) : ImColor(0xFF8A8886));
				break;
...
void CKeyBoard::Open(keyboard_callback* handler, bool bHiden)
{
	...
	m_bEnable = true;
	m_bInputFlag = bHiden;
	CNoty::visibilityNotification(false);
	CRecon::reconHide(false);
...
void CKeyBoard::Close()
{
	...
	m_iPushedKey = -1;
	m_pHandler = nullptr;
	CNoty::visibilityNotification(true);
	CRecon::reconShow(false);
...
void CKeyBoard::EnableNewKeyboard(int i)
{
	m_bNewKeyboard = i;
}
