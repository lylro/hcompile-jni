...
CKeyBoard::CKeyBoard()
{
	...
	//textureID = (RwTexture*)LoadTextureFromDB("mobile", "hud_sprint");
...
void CKeyBoard::Render()
{
	...
	ImGuiIO& io = ImGui::GetIO();
	ImVec2 vecButSize = ImVec2(ImGui::GetFontSize() * 7.5, ImGui::GetFontSize() * 4.0);

	ImGui::SetNextWindowPos(ImVec2(io.DisplaySize.x - vecButSize.x * 3.0, io.DisplaySize.y / 2 - vecButSize.y * 1.45));
	...
	ImVec2 buttonEndPos = ImVec2(buttonPos.x + vecButSize.x, buttonPos.y + vecButSize.y);

	ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0, 0, 0, 0)); // Устанавливаем прозрачный цвет для кнопки
	ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(0, 0, 0, 0)); // Устанавливаем прозрачный цвет для наведения на кнопку

	float triangleSize = vecButSize.y / 3.0f;
	ImVec2 trianglePos(buttonPos.x + vecButSize.x * 0.5f - triangleSize * 0.5f, buttonPos.y + vecButSize.y * 0.5f - triangleSize * 0.5f);
	ImGui::GetOverlayDrawList()->AddRectFilled(buttonPos, buttonEndPos, 0xB0000000);
	...
	drawList->AddTriangleFilled(ImVec2(trianglePos.x, trianglePos.y + triangleSize), ImVec2(trianglePos.x + triangleSize, trianglePos.y + triangleSize), ImVec2(trianglePos.x + triangleSize * 0.5f, trianglePos.y), triangleColor);

	ImVec2 spacingSize(30, 0);

	ImGui::SameLine(0, spacingSize.x);

	buttonPos = ImGui::GetCursorScreenPos();
	buttonEndPos = ImVec2(buttonPos.x + vecButSize.x, buttonPos.y + vecButSize.y);

	trianglePos = ImVec2(buttonPos.x + vecButSize.x * 0.5f - triangleSize * 0.5f, buttonPos.y + vecButSize.y * 0.5f - triangleSize * 0.5f);
	ImGui::GetOverlayDrawList()->AddRectFilled(buttonPos, buttonEndPos, 0xB0000000);
	...
	drawList->AddTriangleFilled(ImVec2(trianglePos.x, trianglePos.y), ImVec2(trianglePos.x + triangleSize, trianglePos.y), ImVec2(trianglePos.x + triangleSize * 0.5f, trianglePos.y + triangleSize), triangleColor);
...
bool CKeyBoard::OnTouchEvent(int type, bool multi, int x, int y)
{
	...
	ImGuiIO& io = ImGui::GetIO();
	ImVec2 vecButSize = ImVec2(ImGui::GetFontSize() * 8, ImGui::GetFontSize() * 4.0);

	if (x >= io.DisplaySize.x / 2 && y >= io.DisplaySize.y / 2 - vecButSize.y * 2 && y <= io.DisplaySize.y / 2) // keys
	{
		return true;
	}
