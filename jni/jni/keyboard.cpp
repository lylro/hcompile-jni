...
void CKeyBoard::Render()
{
    ...
    ImGui::SetWindowSize(ImVec2(1580, 720), ImGuiCond_FirstUseEver); // Example size, adjust as needed
    ImGui::SetNextWindowSize(ImVec2(io.DisplaySize.x, io.DisplaySize.y / 2.25), ImGuiCond_FirstUseEver); // Example size, adjust as needed
    ...
    // background
    ImGui::GetOverlayDrawList()->AddRectFilled(m_Pos, ImVec2(m_Size.x * 1.5, io.DisplaySize.y * 1.5), 0xB0000000);
    ...
    ImGui::GetOverlayDrawList()->AddLine(
            ...
            ImVec2(m_Size.x, m_Pos.y + m_fKeySizeY), 0xFF0000FF);
...
bool CKeyBoard::OnTouchEvent(int type, bool multi, int x, int y)
{
    ...
    ImVec2 rightCorner(m_Size.x, m_Pos.y + m_fKeySizeY);
        ...
        if (type == TOUCH_PUSH && x >= leftCorner.x && y >= leftCorner.y && x <= rightCorner.x && y <= rightCorner.y)
        {
            ...
            {
                std::string msg = "idi naxuy ebanashka";
