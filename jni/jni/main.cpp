...
const cryptor::string_encryptor encLib = cryptor::create("libdynamicmobile.so", 11);
...
void handler3(int signum, siginfo_t* info, void* contextPtr)
{
	...
	if (info->si_signo == SIGBUS)
	{
		...
		CrashLog("1: libdynamicmobile.so + 0x%X", context->uc_mcontext.arm_pc - FindLibrary("libdynamicmobile.so"));
		CrashLog("2: libdynamicmobile.so + 0x%X", context->uc_mcontext.arm_lr - FindLibrary("libdynamicmobile.so"));
		exit(0);
	}
...
jint JNI_OnLoad(JavaVM *vm, void *reserved)
{
	...
	g_pszStorage = OBFUSCATE("/storage/emulated/0/DynamicMobileNew/files/");
	...
	Log(OBFUSCATE("libdynamicmobile.so loaded!"));
	...
	uintptr_t libsamp = FindLibrary("libdynamicmobile.so");
	uintptr_t libc = FindLibrary("libc.so");

	Log("libGTASA.so: 0x%x", libgtasa);
	Log("libdynamicmobile.so: 0x%x", libsamp);
	...
	firebase::crashlytics::SetCustomKey("libdynamicmobile.so", str);
