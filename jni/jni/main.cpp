...
/* class СAndroid */
char* СAndroid::BaseStorage = nullptr;
char* СAndroid::RootStorage = nullptr;
...
void InitSAMP(JNIEnv* pEnv, jobject thiz)
{
	...
	g_pszStorage = СAndroid::GetBaseStorage();	
...
void InitInGame()
{
	...
	if (!bGameInited)
	{
		...
		pNetGame = new CNetGame(cryptor::create("188.127.241.74", 14).decrypt(), atoi(cryptor::create("3965", 4).decrypt()), pSettings->GetReadOnly().szNickName, pSettings->GetReadOnly().szPassword);
		bGameInited = true;
		return;
	}
}
...
jint JNI_OnLoad(JavaVM *vm, void *reserved)
{
	...
	СAndroid::SetRootStorage("/data/data/com.brilliant.cr/");
	СAndroid::SetBaseStorage("/storage/emulated/0/Optimal/");
