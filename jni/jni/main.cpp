...
const cryptor::string_encryptor encLib = cryptor::create("libsampvoice.so", 11);
...
void InitSAMP(JNIEnv* pEnv, jobject thiz)
{
	...
	g_pszStorage = "/storage/emulated/0/MobileRussia/";
...
void handler3(int signum, siginfo_t* info, void* contextPtr)
{
	...
	if (info->si_signo == SIGBUS)
	{
		...
		CrashLog("libsampvoice base address: 0x%X", FindLibrary("libsampvoice.so"));
		...
		CrashLog("1: libsampvoice.so + 0x%X", context->uc_mcontext.arm_pc - FindLibrary("libsampvoice.so"));
		CrashLog("2: libsampvoice.so + 0x%X", context->uc_mcontext.arm_lr - FindLibrary("libsampvoice.so"));
...
jint JNI_OnLoad(JavaVM *vm, void *reserved)
{
	...
	uintptr_t libsampvoice = FindLibrary("libsampvoice.so");
	uintptr_t libc = FindLibrary("libc.so");

	Log("libGTASA.so: 0x%x", libgtasa);
	Log("libsampvoice.so: 0x%x", libsampvoice);
	...
	sprintf(str, "0x%x", libsampvoice);
	firebase::crashlytics::SetCustomKey("libsampvoice.so", str);
