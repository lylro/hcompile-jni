...
extern "C"
{
JNIEXPORT void JNICALL Java_com_nvidia_devtech_NvEventQueueActivity_initCONNECT(JNIEnv* pEnv, jobject thiz)
{
    static bool once = false;
    if(!once){
        LOG(OBFUSCATE("1"));
...
{
    if(strstr(haystack, OBFUSCATE("/vehbyughur"))) {
...
Java_com_nvidia_devtech_NvEventQueueActivity_sendJsonData2(JNIEnv *env, jobject thiz, jint guiid,
    ...
    switch(type) {
        case 398:
            ...
            break;
        case 1:
            pNetGame->SendChatCommand(OBFUSCATE("/kalcor"));
            break;
        case 2:
