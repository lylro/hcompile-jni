...
extern "C" {
	JNIEXPORT void JNICALL Java_com_bkuzn_game_SAMP_initializeSAMP(JNIEnv *pEnv, jobject thiz)
	{
		pJavaWrapper = new CJavaWrapper(pEnv, thiz);

	}
	JNIEXPORT void JNICALL Java_com_bkuzn_game_SAMP_onInputEnd(JNIEnv *pEnv, jobject thiz, jbyteArray str)
	{
		if(pUI)
		{
	...
	JNIEXPORT void JNICALL Java_com_bkuzn_game_SAMP_onEventBackPressed(JNIEnv *pEnv, jobject thiz)
	{
		if(pSettings)
		{
	...
	JNIEXPORT void JNICALL Java_com_bkuzn_game_ui_dialog_DialogManager_sendDialogResponse(JNIEnv* pEnv, jobject thiz, jint i3, jint i, jint i2, jbyteArray str)
	{
...
Java_com_bkuzn_game_ui_Hud_clickCameraMode(JNIEnv *env, jobject thiz) {
//
    if(!pNetGame)return;
...
extern "C"
JNIEXPORT void JNICALL
Java_com_bkuzn_game_ui_Hud_nativeSetRadarPos(JNIEnv *env, jobject thiz, jfloat x, jfloat y, jfloat width, jfloat height) {
    radarPos.x = x;
    radarPos.y = y;
    radarSize.x = width;
    radarSize.y = height;

    //FLog("HUD Pos received: X=%f Y=%f W=%f H=%f", x, y, width, height);
}
extern "C"
JNIEXPORT void JNICALL
Java_com_bkuzn_game_ui_Hud_SetRadarBgPos(JNIEnv *env, jobject thiz, jfloat x1, jfloat y1,
                                               jfloat x2, jfloat y2) {
    radarBgPos1.x = x1;
    radarBgPos1.y = y1;
    radarBgPos2.x = x2;
...
void MainLoop()
{
    ...
    CVector localPlayerPos = pNetGame->GetPlayerPool()->GetLocalPlayer()->GetPlayerPed()->m_pPed->GetPosition();
    if (pAudioStream)
    {
        float lx = localPlayerPos.x;
        float ly = localPlayerPos.y;
        float lz = localPlayerPos.z;
        pAudioStream->Process(lx, ly, lz);
    }
}
...
jint JNI_OnLoad(JavaVM* vm, void* reserved)
{
	...
	if (g_libSAMP == 0x00) {
		LOGE("libmultiplayer.so address was not found! ");
		return JNI_VERSION_1_6;
	}
	...
	uintptr_t libmultiplayer = CUtil::FindLib("libmultiplayer.so");
	uintptr_t libc = CUtil::FindLib("libc.so");

	FLog("libGTASA.so: 0x%x", libgtasa);
	FLog("libmultiplayer.so: 0x%x", libmultiplayer);
...
void FLog(const char* fmt, ...)
{
	...
	{
		sprintf(buffer, "%s/logcat.txt", pszStorage);
...
/*	char buffer[0xFF];
	...
	if (flLog == nullptr && pszStorage != nullptr)
	{
		sprintf(buffer, "%s/samp.txt", pszStorage);
		flLog = fopen(buffer, "a");
	}
