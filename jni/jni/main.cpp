...
#define USE_MULTI_CRASH_HANDLER

#ifdef USE_MULTI_CRASH_HANDLER
	#include <xrtutils/CrashHandler.h>
#endif
...
//CClothesTextureManager *pClothesTextureManager = nullptr;
...
#ifndef USE_MULTI_CRASH_HANDLER
...
void InitSAMP(JNIEnv* pEnv, jobject thiz)
{
	...
	g_pszStorage  = "/storage/emulated/0/FlinOnline/files/";;
	...
	//g_pMasterCheat = new MasterCheat();
...
void InitInMenu()
{
    ...
    //pClothesTextureManager = new CClothesTextureManager();
...
/*		for (int i = 0; i < MAX_SERVERS; i++)
		...
		cp1251_to_utf8(pDialogWindow->m_utf8Title, "Entekhab Server By @CN_USEER");
		cp1251_to_utf8(pDialogWindow->m_utf8Button1, "Connect");
		cp1251_to_utf8(pDialogWindow->m_utf8Button2, "Bastan");
		char szBuff[400];
		...
		g_pJavaWrapper->UpdateLoading(3);
		pNetGame = new CNetGame(cryptor::create("51.75.232.71", 19).decrypt(), atoi(cryptor::create("1249", 4).decrypt()), pSettings->GetReadOnly().szNickName, pSettings->GetReadOnly().szPassword);


		//g_pJavaWrapper->CallLauncherActivity(1555); 
