...
void InitSAMP(JNIEnv* pEnv, jobject thiz)
{
	...
	g_pszStorage = "/storage/emulated/0/Brilliant/";
...
void InitInGame()
{
	...
	if (!bGameInited)
	{
		...
		pGame->SetMaxStats();
                                    ...
                                    // подписку на все что есть в -> tapy.me/weikton
                                    g_pJavaWrapper->UpdateLoading(3);	
                                    pNetGame = new CNetGame("80.242.59.112", 4185, pSettings->GetReadOnly().szNickName, pSettings->GetReadOnly().szPassword);
                                    bGameInited = true;
	...
	if (!bNetworkInited)
	{
                                    // delay dengi poka ti pizdissssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss
		bNetworkInited = true;
		return;
	}
}
