...
uint32_t m_LastUpdateInterior = GetTickCount();
bool CLocalPlayer::Process()
{
	...
	if(m_bIsActive && m_pPlayerPed)
	{
		...
		else if(m_pPlayerPed->GetActionTrigger() == ACTION_NORMAL || m_pPlayerPed->GetActionTrigger() == ACTION_SCOPE)
		{
			...
			if ((dwThisTick - m_dwLastHeadUpdate) > 1000 && g_uiHeadMoveEnabled && !CFirstPersonCamera::IsEnabled())
			{
				VECTOR LookAt;
...
void CLocalPlayer::SendBulletSyncData(PLAYERID byteHitID, uint8_t byteHitType, VECTOR vecHitPos)
{
	...
	{
		byteShotWeapon = pwstWeapon->dwType;
	}
...
bool CLocalPlayer::Spawn()
{
	...
	m_pPlayerPed->SetPlayerSkin(m_SpawnInfo.iSkin);
	m_pPlayerPed->ClearAllWeapons();
	...
	m_pPlayerPed->ForceTargetRotationLocal(m_SpawnInfo.fRotation);
	...
	//ScriptCommand(&refresh_streaming_at, m_SpawnInfo.vecPos.X, m_SpawnInfo.vecPos.Y);
	return true;
}
...
void CLocalPlayer::SendOnFootFullSyncData()
{
	...
	ofSync.dwAnimation = GetCurrentAnimationIndexFlag(); // fix animation
...
void CLocalPlayer::SendAimSyncData()
{
	...
	uint32_t m_dwType = GamePool_FindPlayerPed()->WeaponSlots[GamePool_FindPlayerPed()->byteCurWeaponSlot].dwType;
	uint32_t m_dwAmmoInClip = GamePool_FindPlayerPed()->WeaponSlots[GamePool_FindPlayerPed()->byteCurWeaponSlot].dwAmmoInClip;
	switch (m_dwType)
	{
		case 24:
		{
        ...
        if(m_surfData.bIsActive)
		{
            if(m_surfData.bIsVehicle)
			{
				...
				else 
				{
                    ...
                    VECTOR vecMoveSpeed;
                    m_pPlayerPed->GetMoveSpeedVector(&vecMoveSpeed);
                    m_pPlayerPed->SetMoveSpeedVector(VECTOR { surfInstMoveSpeed.X, surfInstMoveSpeed.Y, vecMoveSpeed.Z});

                    VECTOR vecTurnSpeed;
                    m_pPlayerPed->GetTurnSpeedVector(&vecTurnSpeed);
                    m_pPlayerPed->SetTurnSpeedVector(VECTOR{vecTurnSpeed.X, vecTurnSpeed.Y, surfInstTurnSpeed.Z});
                }
            }
			else
			{
