	...
	if(m_bIsActive && m_pPlayerPed)
	{
		...
		if (m_pPlayerPed->GetStuff() != STUFF_TYPE_NONE)
		{
			...
			if (LocalPlayerKeys.bKeys[ePadKeys::KEY_SECONDARY_ATTACK] || m_pPlayerPed->IsInVehicle())
				m_pPlayerPed->DropStuff() /*DestroyTexture(p_DButtonTexture)*/;
		}

		m_pPlayerPed->ProcessDrunk();

		if (m_pPlayerPed->IsPissing() && (LocalPlayerKeys.bKeys[ePadKeys::KEY_WALK] || m_pPlayerPed->IsInVehicle()))
		{
			m_OnFootData.byteSpecialAction = SPECIAL_ACTION_NONE;
			m_pPlayerPed->StopPissing();
		}
