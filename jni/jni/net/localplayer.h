...
private:
	...
	struct {
		int				iEntityId; // for wSurfInfo?
		uint32_t		dwLastUpdate; // used on samp 0.3.7 (not used)

		union {
			CVehicle 	*pVehicle; // last surfing vehicle?
			CObject		*pObject; // last surfing object?
		};

		bool			bStuck; // used on samp 0.3.7 (not used)
		bool			bIsActive; // used on samp 0.3.7 (not used)
		VECTOR			vecPos; // position of entity?
		int				unk;
		int				iMode; // see eSurfingMode
	} m_surfData;
};