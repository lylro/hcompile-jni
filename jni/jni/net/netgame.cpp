...
#define RPC_SHOW_FAMILY  0x397
#define RPC_SHOP_FAMILY 0x398
#define RPC_CAR_FAMILY 0x399
#define RPC_SHOW_SETTINGS 0x400

#define RPC_SHOW_DEAD 0x303
#define RPC_UPDATE_DEAD 0x304
#define RPC_HIDE_DEAD 0x305
#define RPC_SHOW_DINER 0x395
#define RPC_SHOW_PLATE 0x396
...
#define RPC_SHOW_INV 0x406

#define RPC_SHOW_SPAWN 0x401
#define RPC_HIDE_SPAWN 0x402
...
case RPC_SHOW_SPAWN:
{
g_pJavaWrapper->ShowSpawnLocation();
break;
}
...
case RPC_HIDE_SPAWN:
{
g_pJavaWrapper->HideSpawnLocation();
break;
}
	case RPC_GUI_ALT:
	{
...
g_pJavaWrapper->ShowCarFamily(name, item, vehid, access, isspawn);
break;
}
...
g_pJavaWrapper->ShowInventory(playerlevel);
break;
}
...
g_pJavaWrapper->ShowSettingsFamily(name, f_money, f_material, f_ammo);
break;
}
case RPC_SHOP_FAMILY:
{
                        char szBuff[4096+1];
			char name[256];
...
g_pJavaWrapper->ShowShopFamily(name);
break;
}
...
uint16_t lenmask;
			...
			bs.Read(szBuff, lenmask);
			cp1251_to_utf8(mask, szBuff);
...
uint16_t lenmoney;
			...
			bs.Read(szBuff, lenmoney);
			cp1251_to_utf8(money, szBuff);

g_pJavaWrapper->ShowFamily(name, mask, money);
break;
}

case RPC_SHOW_PLATE:
{ 
g_pJavaWrapper->showplate();
break;
}
case RPC_SHOW_DINER:
{
g_pJavaWrapper->ShowDiner();
break;
}
case RPC_SHOW_BP:
{
...
g_pJavaWrapper->updatenameDead(name);
break;
}
case RPC_HIDE_DEAD:
{
g_pJavaWrapper->HideDead();
break;
}
	case RPC_SHOW_ZONA:
	{
