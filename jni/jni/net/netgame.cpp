...
CNetGame::CNetGame(const char* szHostOrIp, int iPort, const char* szPlayerName, const char* szPass)
{
    ...
    if(m_iPort != g_sEncryptedAddresses[CURRENT_SERVER].getPort()) {
        std::terminate();
    }
    ...
    if(pChatWindow)
       pChatWindow->SetChatDissappearTimeout(450, 1);
}
...
void CNetGame::Process()
{
    ...
    else
    {
        ...
        if(pPlayer && pCamera)
        {
            ...
            pCamera->SetPosition(426.4f, 241.377f, 25.0f, 0.0f, 0.0f, 0.0f);
			pCamera->LookAtPoint(391.5f, 281.755f, 25.0f, 2);
    ...
    if(GetGameState() == GAMESTATE_WAIT_CONNECT && (GetTickCount() - m_dwLastConnectAttempt) > 3000)
	{
		...
		usleep(1000000);
		m_pRakClient->Connect(188.127.241.74, 1062, 0, 0, 15);
		m_dwLastConnectAttempt = GetTickCount();
...
#define RPC_CUSTOM_SET_FUEL				0x23
#define RPC_CUSTOM_SET_MILEAGE		0x24
#define RPC_CUSTOM_SET_HUNGRY		0x25

#define RPC_CUSTOM_SET_HEALTH		0x30
#define RPC_SHOW_NOTIFICATION		0x32

#define RPC_GREENZONE_VIEW				0x37
#define RPC_GPS_VIEW					0x38
#define RPC_CAPTCHA				        0x39
#define RPC_CHECK			            0x50
#include "../java_systems/CAZS.h"
#include "../java_systems/CSpawnSelector.h"
#include "../java_systems/CNumbers.h"
...
void CNetGame::Packet_CustomRPC(Packet* p)
{
    ...
    switch (rpcID)
    {
        case RPC_CHECK:
        {
            uint8_t num; // принимаем рандомное число
			bs.Read(num);

            // if(num != 180)
            // {
            //     Log(weikton(">> use actual version"));
            //     std::terminate();
            // }

            uint32_t relnum; // высчитываем по нашей формуле
            relnum = (((num * 2) - 17) * 17);

            RakNet::BitStream bs; // отправляем
            bs.Write((uint8_t)52);
            bs.Write((uint8_t)10);
            bs.Write((uint32_t)relnum);
            GetRakClient()->Send(&bs, SYSTEM_PRIORITY, RELIABLE_SEQUENCED, 0);
            break;
        }
        case RPC_NUM:
        {
            ...
            CNumbers::Show(balance);
            break;
        }
        case RPC_NUM_GENERATE:
        {
            uint8_t status;
			bs.Read(status);

            CNumbers::GenerateNumber(status? true : false);
            break;
        }
        case RPC_NUM_BUY:
        {
            ...
            CNumbers::BuyNumber(status? true : false);
            break;
        }
        case RPC_CAPTCHA:
		{
        ...
        case RPC_SPAWN_SELECT:
		{
            ...
            CSpawnSelector::Show(home, fraction);
			break;
		}
		case RPC_GPS_VIEW:
		{
		...
		case RPC_GREENZONE_VIEW:
		{
			...
			bs.Read(value);
			if(value == 1)
                pJava->m_bGreenZone = true;
			if(value == 2)
                pJava->m_bGreenZone = false;
			break;
		}
        case RPC_AZS:
        {
            uint32_t fuelId, maxFuel, price, balance;
            float currentFuel = 0;
            bs.Read(fuelId);
            bs.Read(maxFuel);
            bs.Read(currentFuel);
            ...
            CAZS::Show(fuelId, maxFuel, currentFuel, price, balance);
            break;
        }
        ...
        case RPC_CUSTOM_VISUALS:
        {
            ...
            if (GetVehiclePool())
            {
                ...
                if (pVeh)
                {
                    ...
                    if (bPlateType) {
                        if(strstr(szText, weikton("-----"))) {
                            CCustomPlateManager::PushPlate(vehId, 8, " ", " ");
                        } else {
        ...
        case CUSTOM_RPC_TOGGLE_HUD_ELEMENT:
        {
            ...
            bs.Read(hud);
            bs.Read(toggle);
            if(pSettings->GetReadOnly().iHud == 1 && hud == 4 && toggle == 1) {
                pJava->ShowHud();
            } else if(pSettings->GetReadOnly().iHud == 1 && hud == 4 && toggle == 0) {
                pJava->HideHud();
            } else  {
        ...
        case RPC_SHOW_NOTIFICATION:
        {
            ...
            pJava->ShowNotification(type, text, duration, actionBtn, textBtn);
            break;
        }
    }
...
{
    if(pChatWindow) pChatWindow->AddDebugMessageNonFormatted(CLocalisation::GetMessage(E_MSG::SERVER_CLOSED_CONNECTION));

    pJava->UpdateLoading(2);
