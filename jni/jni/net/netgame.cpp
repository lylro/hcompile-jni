...
void CNetGame::Process()
{
	...
	else
	{
		...
		if(pPlayer && pCamera)
		{
			...
			pCamera->SetPosition(426.4, 241.377, 14.3, 0.0f, 0.0f, 0.0f);
...
void CNetGame::Packet_CustomRPC(Packet* p)
{
	...
	case RPC_CUSTOM_ACTOR_PUT_IN_VEH:
		...
		case RPC_ADMIN_SPEC:
		{
			...
			g_pJavaWrapper->ShowSpec(toplayer);
			break;
		}
		case RPC_SPEC_NAME:
		{
			...
			uint16_t lenplayer_name;
			bs.Read(lenplayer_name);

			memset(player_name, 0, sizeof(player_name));
			memset(szBuff, 0, sizeof(szBuff));

			bs.Read(szBuff, lenplayer_name);
			cp1251_to_utf8(player_name, szBuff);

			g_pJavaWrapper->ShowName(player_name);
			return;
		}
		...
		case RPC_SHOW_TAXI:
		{
			...
			g_pJavaWrapper->ShowTaxi(id, num, cash, price, rast, marks);
			break;
		}
...
void CNetGame::SendSpecClick(int id, int specid)
{
			RakNet::BitStream bsSend;
                	bsSend.Write((uint8_t)  0x56);
               	 	bsSend.Write((uint32_t) RPC_ADMIN_SPEC);
			bsSend.Write((int) id);
			bsSend.Write((int) specid);
