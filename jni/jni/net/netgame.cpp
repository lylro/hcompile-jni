...
#include "network/CLocalisation.h"
...
uintptr_t m_byteGTAPlayerNumber;
...
void CNetGame::Process()
{
    ...
    static bool m_bCreatedCar = false;
    bool bProcess = false;
    ...
    else
    {
        ...
        {
            //pGame->DisplayGameText(OBFUSCATE("TAPY.ME/@obfuscati0n"), 3, 0);
            ...
            //m_byteGTAPlayerNumber = *(uint8_t*)(g_libGTASA + 0x8E864C);
            //*(uint8_t*)(g_libGTASA + 0x8E864C) = pPlayer->m_bytePlayerNumber;
...
void CNetGame::UpdateNetwork()
{
    ...
    while(pkt = m_pRakClient->Receive())
    {
        ...
        {
            case ID_AUTH_KEY:
                LOGN("Incoming packet: ID_AUTH_KEY");
                Packet_AuthKey(pkt);
                break;
            ...
            case ID_DISCONNECTION_NOTIFICATION:
                LOGN("Incoming packet: ID_DISCONNECTION_NOTIFICATION");
                Packet_DisconnectionNotification(pkt);
                break;

            case ID_CONNECTION_LOST:
                LOGN("Incoming packet: ID_CONNECTION_LOST");
                Packet_ConnectionLost(pkt);
                break;

            case ID_CONNECTION_REQUEST_ACCEPTED:
                LOGN("Incoming packet: ID_CONNECTION_REQUEST_ACCEPTED");
            ...
            case ID_CONNECTION_BANNED:
                LOGN("Incoming packet: ID_CONNECTION_BANNED");
            ...
            case ID_PLAYER_SYNC:
                LOGN("Incoming packet: ID_PLAYER_SYNC");
                Packet_PlayerSync(pkt);
                break;

            case ID_VEHICLE_SYNC:
                LOGN("Incoming packet: ID_VEHICLE_SYNC");
                Packet_VehicleSync(pkt);
                break;

            case ID_PASSENGER_SYNC:
                LOGN("Incoming packet: ID_PASSENGER_SYNC");
                Packet_PassengerSync(pkt);
                break;

            case ID_MARKERS_SYNC:
                LOGN("Incoming packet: ID_MARKERS_SYNC");
                Packet_MarkersSync(pkt);
                break;

            case ID_AIM_SYNC:
                LOGN("Incoming packet: ID_AIM_SYNC");
                Packet_AimSync(pkt);
                break;

            case ID_BULLET_SYNC:
                LOGN("Incoming packet: ID_BULLET_SYNC");
                Packet_BulletSync(pkt);
                break;

            case ID_TRAILER_SYNC:
                LOGN("Incoming packet: ID_TRAILER_SYNC");
                Packet_TrailerSync(pkt);
                break;

            case ID_CUSTOM_RPC:
                LOGN("Incoming packet: ID_CUSTOM_RPC");
...
void CNetGame::Packet_AuthKey(Packet* pkt)
{
    ...
    //*(uint8_t*)(g_libGTASA + 0x8E864C) = m_byteGTAPlayerNumber;

    LOGN("[AUTH] %s -> %s", szAuth, szAuthKey);
}
