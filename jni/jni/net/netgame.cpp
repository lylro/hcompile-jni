...
CNetGame::CNetGame(const char* szHostOrIp, int iPort, const char* szPlayerName, const char* szPass)
{
	...
	if(pChatWindow) 
                  {
                         // pChatWindow->AddDebugMessage("{007bff}| {ffffff}Created by: {007bff}tapy.me/weikton{ffffff}"); // согласись я лучший ;0
                          pChatWindow->SetChatDissappearTimeout(60, 1); // затухание чата, сука только попробуйте его спиздить и сказать что я лох
                          pChatWindow->AddDebugMessage("Game Started.."); // ну тут крч по старинке на базу.
                  }
}
...
void CNetGame::Process()
{
	...
	{
		if(pChatWindow) {
			g_pJavaWrapper->UpdateLoading(0);
...
{
	//g_pJavaWrapper->UpdateLoading(8);
	Packet* pkt = nullptr;
...
void CNetGame::Packet_DisconnectionNotification(Packet* pkt)
{
	if(pChatWindow) pChatWindow->AddDebugMessageNonFormatted(CLocalisation::GetMessage(E_MSG::SERVER_CLOSED_CONNECTION));
                  g_pJavaWrapper->UpdateLoading(2);
...
void CNetGame::Packet_ConnectionLost(Packet* pkt)
{
	...
	if(pChatWindow)
		pChatWindow->AddDebugMessageNonFormatted(CLocalisation::GetMessage(E_MSG::CONNECTION_LOST));
                  g_pJavaWrapper->UpdateLoading(6);
...
void CNetGame::Packet_ConnectionSucceeded(Packet* pkt)
{
	if(pChatWindow)
		pChatWindow->AddDebugMessageNonFormatted(CLocalisation::GetMessage(E_MSG::CONNECTED));
                  g_pJavaWrapper->UpdateLoading(1);
