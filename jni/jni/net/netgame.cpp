...
CNetGame::CNetGame(const char* szHostOrIp, int iPort, const char* szPlayerName, const char* szPass)
{
	...
	if(pChatWindow) pChatWindow->AddDebugMessage("OUR RUSSIA client version: 0.4");
}
...
#define RPC_CUSTOM_SET_FUEL 0x36
#define RPC_CUSTOM_VEH_NUMBER 0x37
#define RPC_CUSTOM_VEH_HEADLIGHTS 0x38
#define RPC_CUSTOM_VEH_VINYLS 0x39
#define RPC_SHOW_AUTOSALON 0x40
#define RPC_SET_CAR_AUTOSALON 0x41
...
void CNetGame::Packet_CustomRPC(Packet* p)
{
	...
	case RPC_OPEN_SETTINGS:
	{
	    ...
	    case RPC_CUSTOM_VEH_NUMBER:
	    {
			...
			bs.Read(szBuff, len);
			cp1251_to_utf8(link, szBuff);
			
			CCustomPlateManager::PushPlate((uint16_t)vehid, (uint32_t)type, link, link1);
			
			//pChatWindow->AddDebugMessage("vehid: %d, type: %d, region: %s, number: %s", vehid, type, link1, link);
			break;
		}
		case RPC_CUSTOM_SET_FUEL:
		{
		    ...
		    pGUI->m_fuel=fuel;
		    //pChatWindow->AddDebugMessage("fuel: %d", fuel);
		    break;
		}
	    ...
	    case RPC_CUSTOM_VEH_VINYLS:
	    {
			...
			pChatWindow->AddDebugMessage("винил %d", number);
			break;
		}
		case RPC_SHOW_AUTOSALON:
		{
			...
			bs.Read(show);
			pChatWindow->AddDebugMessage("dkdkdkek");
			if(show == 1) {
		...
		case RPC_SET_CAR_AUTOSALON:
		{
			...
			g_pJavaWrapper->SetCarInAutosalon(number, link1);
			break;
         }
		 case RPC_DONATE_SHOW:
		 {
			packetDonateShow(p);
			break;
		 }
		 case RPC_DONATE_UPDATE:
		 {
			packetDonateUpdate(p);
			break;
		 }
...
{	
	if((int)wListBoxItem == 255) return;
	Log("Dialog: selected item %d", (int)wListBoxItem);
	uint8_t respLen = 0;
	if(szInput) if(*szInput != '\0') respLen = strlen(szInput);
