...
{
	//strcpy(m_szHostName, "BLACK RUSSIA");
...
#define RPC_SHOW_FRACTION 0x409

#define RPC_SHOW_SMI 0x405
...
#define RPC_SHOW_DINER	0x397
#define RPC_SHOW_SHOP 0x78
#define RPC_HIDE_SHOP 0x79

#define RPC_SHOW_TEST 0x535
#define RPC_SHOW_TAB  0x277

#define RPC_CALENDAR	0x45


#define RPC_TUNINGSTAGE 0x398
#define RPC_TUNINGMENU 0x399
#define RPC_SHOW_INV 0x693
...
#define RPC_SHOW_BP 0x394
#define RPC_SHOW_CAT 0x421

#define RPC_SHOW_CAPTCHA 0x190
#define RPC_HIDE_CAPTCHA 0x191
...
void CNetGame::Packet_CustomRPC(Packet* p)
{
	...
	case RPC_CUSTOM_ACTOR_PUT_IN_VEH:
		...
		case RPC_SHOW_FRACTION:
		{

			int id;
			bs.Read(id);
			g_pJavaWrapper->ShowFraction(id);
			break;
		}
		case 0x411:
		{
		...
		case RPC_SHOW_SMI:
		{
			...
			char smi_edit[100];
			char smi_player[100];
			char smi_textplayer[100];

			uint16_t lensmi_edit;
			bs.Read(smi_edit);
			memset(smi_edit, 0, sizeof(smi_edit));
			memset(szBuff, 0, sizeof(szBuff));
			bs.Read(szBuff, lensmi_edit);
			cp1251_to_utf8(smi_edit, szBuff);

			uint16_t lensmi_player;
			bs.Read(smi_player);
			memset(smi_player, 0, sizeof(smi_player));
			memset(szBuff, 0, sizeof(szBuff));
			bs.Read(szBuff, lensmi_player);
			cp1251_to_utf8(smi_player, szBuff);


			uint16_t lensmi_textplayer;
			bs.Read(smi_textplayer);
			memset(smi_textplayer, 0, sizeof(smi_textplayer));
			memset(szBuff, 0, sizeof(szBuff));
			bs.Read(szBuff, lensmi_textplayer);
			cp1251_to_utf8(smi_textplayer, szBuff);


			g_pJavaWrapper->ShowSmi(smi_edit, smi_player, smi_textplayer);
			break;
		}
		case RPC_STREAM_POS:
		{
		...
		case RPC_SHOW_CAPTCHA:
		{
			g_pJavaWrapper->ShowCaptcha();
			break;
		}
		case RPC_HIDE_CAPTCHA:
		{
			g_pJavaWrapper->HideCaptcha();
			break;
		}
		...
		case RPC_SHOW_TAXI:
		{
			...
			g_pJavaWrapper->ShowTaxi(id, num, cash, price, rast);
			break;
		}
		...
		case RPC_TUNINGSTAGE:
		{
			int money;
			bs.Read(money);

			g_pJavaWrapper->ShowTuningstage(money);
			return;
		}
		case RPC_TUNINGMENU:
		{
			...
			g_pJavaWrapper->ShowTuningmenu(money);
			return;
		}
		...
		case RPC_SHOW_DINER:
        	{
	            ...
	            if (toggle == 1) {
	                g_pJavaWrapper->ShowBrDiner();
	            }
        	    else if (toggle == 0) {
        	        g_pJavaWrapper->HideBrDiner();
        	    }
            	    break;
        	}  
                case RPC_SHOW_BP:
                {
                        ...
                        g_pJavaWrapper->ShowBp(money, bc);
                        break;
                }
                case RPC_CALENDAR:
	        {
		        ...
		        g_pJavaWrapper->ShowCalendar(calendar);
		        return;
	        }
                ...
                case RPC_SHOW_RECON:
		{
			...
			return;
                ...
                case 0x100:
		{
			g_pJavaWrapper->showInventory();
			break;
		}
		case 0x101:
		{
			g_pJavaWrapper->closeInventory();
			break;
		}
		case 0x102:
		{
			...
			int slot;
			bs.Read(slot);
			g_pJavaWrapper->addItemToInv(id, arg, slot);
			Log("id: %d arg: %d slot: %d", id, arg, slot);
			break;
		}
                case RPC_AUTO_SHOP:
		{
		...
		case RPC_SHOW_TEST:
		{
			g_pJavaWrapper->showtest();
			break;
		}
		case RPC_SHOW_CAT:
		{
			g_pJavaWrapper->showcatcoin();
			break;
		}
		case RPC_UPD_SHOP:
		{
		...
		case RPC_SHOW_FAMILY:
		{
			...
			g_pJavaWrapper->ShowFamily(name, num, num2);
			break;
		}
		...
		case RPC_SHOW_INFOFAM:
		{
			...
			memset(statys, 0, sizeof(statys));
			memset(szBuff, 0, sizeof(szBuff));

			bs.Read(szBuff, lenstatys);
			cp1251_to_utf8(statys, szBuff);
			...
			g_pJavaWrapper->ShowSettingf(name, statys, repa, colivo, lvl1, lvl2, lvl3, cash, mater, masks, fuels, ammos, armors);  // int repa, int colivo, int lvl1, int lvl2, int lvl3, int cash, int mater, int masks, int fuels, int ammos, int armors
			break;
		}
		case RPC_SHOW_SHOP:
		{
		...
		case RPC_SHOW_ULUCFAM:
		{
			...
			g_pJavaWrapper->showuluc(lvl1, lvl2, lvl3, cash, maxcash, mats, mask, fuels, ammos, puples);
			break;
		}
...
void CNetGame::SendShopStoreClick(int buttonid)
{
			RakNet::BitStream bsSend;
                	bsSend.Write((uint8_t)  0x60);
               	 	bsSend.Write((uint32_t) RPC_SHOW_SHOP);
			bsSend.Write((int) buttonid);

                	pNetGame->GetRakClient()->Send(&bsSend, MEDIUM_PRIORITY, RELIABLE, 0);
}
void CNetGame::UseInventory(int id, int slot)
{
	RakNet::BitStream bsSend;
	bsSend.Write((uint8_t)0x23);
	bsSend.Write((uint32_t)0x180);
	bsSend.Write((int)id);
	bsSend.Write((int)slot);
	pNetGame->GetRakClient()->Send(&bsSend, MEDIUM_PRIORITY, RELIABLE, 0);
	Log("id: %d slot: %d", id, slot);
}
void CNetGame::DeleteInventory(int id, int slot)
{
	RakNet::BitStream bsSend;
	bsSend.Write((uint8_t)0x23);
	bsSend.Write((uint32_t)0x181);
	bsSend.Write((int)id);
	bsSend.Write((int)slot);
	pNetGame->GetRakClient()->Send(&bsSend, MEDIUM_PRIORITY, RELIABLE, 0);
	Log("id: %d slot: %d", id, slot);
}
void CNetGame::SendUluchClick(int id)
{
			RakNet::BitStream bsSend;
                	bsSend.Write((uint8_t)  0x68);
               	 	bsSend.Write((uint32_t) RPC_SHOW_INFOFAM);
			bsSend.Write((int) id);

                	pNetGame->GetRakClient()->Send(&bsSend, MEDIUM_PRIORITY, RELIABLE, 0);
}
void CNetGame::SendTaxiClick(int id)
{
		RakNet::BitStream bsSend;
                bsSend.Write((uint8_t)  0x60);
               	bsSend.Write((uint32_t) RPC_SHOW_TAXI);
		bsSend.Write((int) id);

                pNetGame->GetRakClient()->Send(&bsSend, MEDIUM_PRIORITY, RELIABLE, 0);
} 
void CNetGame::SendQuestClick(int qid)
{
			RakNet::BitStream bsSend;
                	bsSend.Write((uint8_t)  0x69);
               	 	bsSend.Write((uint32_t) RPC_SHOW_INFOFAM);
			bsSend.Write((int) qid);

                	pNetGame->GetRakClient()->Send(&bsSend, MEDIUM_PRIORITY, RELIABLE, 0);
}

void CNetGame::SendFamShop(int id, int cost)
{
			RakNet::BitStream bsSend;
                	bsSend.Write((uint8_t)  0x70);
               	 	bsSend.Write((uint32_t) RPC_SHOW_FAMILY);
			bsSend.Write((int) id);
			bsSend.Write((int) cost);

                	pNetGame->GetRakClient()->Send(&bsSend, MEDIUM_PRIORITY, RELIABLE, 0);
}
void CNetGame::ClickInventory(int id)
{
	RakNet::BitStream bsSend;
	bsSend.Write((uint8_t)0x23);
	bsSend.Write((uint32_t)0x182);
	bsSend.Write((int)id);
	pNetGame->GetRakClient()->Send(&bsSend, MEDIUM_PRIORITY, RELIABLE, 0);
	Log("id: %d", id);
}
...
void CNetGame::SendDiner(int price, int eat) {
    RakNet::BitStream bsSend;
    bsSend.Write((uint8_t)  0x422);
    bsSend.Write((uint32_t) RPC_SHOW_DINER);
    bsSend.Write((int) price);
    bsSend.Write((int) eat);
