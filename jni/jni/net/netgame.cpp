...
{
	strcpy(m_szHostName, OBFUSCATE("BLACK RUSSIA"));
	...
	/*if(pChatWindow) {
                  ...
                  //pGame->DisplayGameText(OBFUSCATE("TAPY.ME/WEIKTON"), 2, 0);

                  if (pChatWindow) {
                           pChatWindow->AddDebugMessageNonFormatted(CLocalisation::GetMessage(E_MSG::MODIFIED_FILES));
	         pChatWindow->SetChatDissappearTimeout(75, 1);
	}
...
{	
	...
	else
	{
		...
		if(!CHUD::m_bCreatedCar && pPlayer && pCamera) {
            //pGame->DisplayGameText(OBFUSCATE("TAPY.ME/WEIKTON"), 3, 0);

            pCamera->Restore(); // убираем камеру после инициализации
            pCamera->SetBehindPlayer();

            if (pPlayer->IsInVehicle()) // fix scm
                pPlayer->RemoveFromVehicleAndPutAt(431.8335, 250.1207, 11.9394);
            else
                pPlayer->TeleportTo(431.8335, 250.1207, 11.9394);

            pPlayer->SetInitialState(); // хуй знает но вейк решил поставить :)
            pPlayer->ClearAllWeapons(); // фикс майн сцм
            pGame->DisableTrainTraffic(); // крашфикс

            pPlayer->TogglePlayerControllable(true); // доступ двигаться
            pPlayer->SetModelIndex(122); // даем скин

            pGame->SetWorldWeather(m_byteWeather); // сетаем погоду
            pGame->DisplayWidgets(true); // показываем кнопки
            pGame->ToggleRadar(false); // скрываем радар

            //pPlayer->ApplySpecialAction(SPECIAL_ACTION_USEJETPACK);

            /*int random = rand() % 10; // рандом авто
switch (random)
{
case 0: // скай
{*/

            pVehicle = new CVehicle(402, 433.8335, 250.1207, 11.9394, 0, false);
		...
		(GetTickCount() - m_dwLastConnectAttempt) > 1000)
	{
		if(pChatWindow) pChatWindow->AddDebugMessageNonFormatted(CLocalisation::GetMessage(E_MSG::CONNECTING));
                                    g_pJavaWrapper->ShowNotification(6, OBFUSCATE("Вы находитесь в списке очереди на подключение к серверу.\nЧтобы скоротать время {56D12A}исследуйте карту{ffffff} и {56D12A}новые места."), 0, "", ""); 
...
void CNetGame::UpdateNetwork()
{
	...
	while(pkt = m_pRakClient->Receive())
	{
		...
		switch(packetIdentifier)
		{
			...
			pChatWindow->AddDebugMessage(OBFUSCATE("Неверный пароль для подключения к серверу!"));
			m_pRakClient->Disconnect(0);
			break;
...
#define RPC_SHOW_FAMILY	0x33
#define RPC_FAMILY_CAR	0x34
#define RPC_FAMILY_NOTIF	0x44
#define RPC_FAMILY_RAITING	0x45
#define RPC_FAMILY_INFO	0x46
#define RPC_FAMILY_PLAYERS	0x47
#define RPC_PLAY_MUSIC	0x48
#define RPC_STOP_MUSIC	0x49
#define RPC_SHOW_MUSIC	0x50
#define RPC_PAUSE_MUSIC	0x51
#define RPC_RESUME_MUSIC	0x52
#define RPC_SETSTATE_MUSIC	0x53
...
void CNetGame::Packet_CustomRPC(Packet* p)
{
	...
	case RPC_CUSTOM_ACTOR_PUT_IN_VEH:
        ...
        case RPC_FAMILY_CAR:
        {
            packetFamilyLoadCar(p);
            break;
        }
        case RPC_FAMILY_NOTIF:
        {
            packetFamilyLoadNotif(p);
            break;
        }
        case RPC_FAMILY_RAITING:
        {
            packetFamilyLoadRaiting(p);
            break;
        }
        case RPC_FAMILY_INFO:
        {
            packetFamilyInfoMainMenu(p);
            break;
        }
        case RPC_FAMILY_PLAYERS:
        {
            packetFamilyLoadPlayers(p);
            break;
        }
        case 0x41:
        {
            packetFamilyLoadRangCar(p);
            break;
        }
        case 0x42:
        {
            packetFamilyLoadPlayerInfo(p);
            break;
        }
        case RPC_PLAY_MUSIC:{
            uint8_t byteTextLen;
            uint16_t index, vehid;
            uint8_t state;
            char szURL[1024];
            ...
            {
                pAudioStreamManager->PlayURL(index, state, vehid, szURL, X, Y, Z, fRadius, bUsePos);
            }
            break;
        }
        case RPC_STOP_MUSIC:{
            ...
            if (pAudioStreamManager)
            {
                pAudioStreamManager->Stop(index);
            }
            break;
        }
        case RPC_SHOW_MUSIC:{
            ...
            g_pJavaWrapper->ShowBoomBox(toggle);
            break;
        }
        case RPC_PAUSE_MUSIC:{
            uint16_t index;
            ...
            if (pAudioStreamManager)
            {
                pAudioStreamManager->Pause(index);
            }
            break;
        }
        case RPC_RESUME_MUSIC:{
            uint16_t index;
            ...
            if (pAudioStreamManager)
            {
                pAudioStreamManager->Resume(index);
            }
            break;
        }
        case RPC_SETSTATE_MUSIC:{
            uint16_t index;
            uint8_t state;
            bs.Read(index);
            bs.Read(state);
            ...
            if (pAudioStreamManager)
            {
                pAudioStreamManager->SetPut(index, state);
            }
            break;
        }
        case 0x54:{
        ...
        case 0x55:{
            ...
            g_pJavaWrapper->SetPause(toggle);
            break;
        }
        case 0x56:{
            packetLoadRangName(p);
            break;
        }
        case 0x57:{
            packetFamilyAccessRang(p);
            break;
        }
        case 0x58:{
            packetFamilyInfoStock(p);
            break;
        }
        case 0x59:{
            bool toggle;
            ...
            g_pJavaWrapper->ToggleAutoShop(toggle);
            break;
        }
        case 0x60:{
            char name[256];
            ...
            g_pJavaWrapper->UpdateAutoShop(name, price, count, maxspeed, acceleration, gear);
            break;
        }
        case 0x61:{
            bool toggle;
            ...
            g_pJavaWrapper->TogglePerson(toggle);
            break;
        }
	}
...
void CNetGame::SendPlayMusic(const char url[])
{
    uint16_t urlLen = strlen(url);
    RakNet::BitStream bsSend;

    bsSend.Write((uint8_t)  ID_CUSTOM_RPC);
    bsSend.Write((uint8_t) 0x38);
    bsSend.Write(urlLen);
    bsSend.Write(url, urlLen);
...
void CNetGame::Packet_DisconnectionNotification(Packet* pkt)
{
    ...
    if(pAudioStreamManager) pAudioStreamManager->StopAll();
}
...
{
    CVehiclePool *pVehiclePool = GetVehiclePool();
	if(pChatWindow)
