...
{	
	...
	else
	{
		...
		if(!m_bCreatedCar && pPlayer && pCamera)
		{                                                     
			...
			if(pPlayer->IsInVehicle())
				pPlayer->RemoveFromVehicleAndPutAt(431.8335, 247.1207, 11.9394);
			else
				pPlayer->TeleportTo(431.8335, 247.1207, 11.9394);

	                                                      pPlayer->SetInitialState(); // хуй знает
	                                                      ...
	                                                      pGame->DisableTrainTraffic(); // крашфикс
                                                                        ...
                                                                        pPlayer->SetModelIndex(789); // даем скин
           ...
           // pPlayer->StartJetpack();

                                                      
                                        
									int random = rand() % 11; // рандом авто
	                                    switch (random)
	                                    {
		                               case 0: // BMW M5 F90
		                               {
                                                                                 CVehicle *pVehicleNew = new CVehicle(411, -1800.0f, 775.755f, 35.5f, 0.0f, false);
																				 pPlayer->PutDirectlyInVehicle(pVehicleNew->m_dwGTAId, 0); 
                                                                                 //pPlayer->PutDirectlyInVehicle(pVehicleNew, 0);
			                                                                     break;
		                               }	
                             	                               case 1: // URUS
		                               {
                                                                                 CVehicle *pVehicleNew = new CVehicle(400, -1800.0f, 775.755f, 35.5f, 0.0f, false);
																				 pPlayer->PutDirectlyInVehicle(pVehicleNew->m_dwGTAId, 0); 
                                                                                 //pPlayer->PutDirectlyInVehicle(pVehicleNew, 0);
			                           break;
		                               }
		                               case 2: // LAMBO 
		                               {
                                                                                 CVehicle *pVehicleNew = new CVehicle(415, -1800.0f, 775.755f, 35.5f, 0.0f, false);
																				 pPlayer->PutDirectlyInVehicle(pVehicleNew->m_dwGTAId, 0); 
                                                                                 //pPlayer->PutDirectlyInVehicle(pVehicleNew, 0);
			                           break;
		                               }
		                               case 3: // MERS I63 AMG
		                               {
                                                                                 CVehicle *pVehicleNew = new CVehicle(429, -1800.0f, 775.755f, 35.5f, 0.0f, false);
																				 pPlayer->PutDirectlyInVehicle(pVehicleNew->m_dwGTAId, 0); 
                                                                                 //pPlayer->PutDirectlyInVehicle(pVehicleNew, 0);
			                           break;
		                               }
		                               case 4: // BMW I8
		                               {
                                                                                 CVehicle *pVehicleNew = new CVehicle(494, -1800.0f, 775.755f, 35.5f, 0.0f, false);
																				 pPlayer->PutDirectlyInVehicle(pVehicleNew->m_dwGTAId, 0); 
                                                                                 //pPlayer->PutDirectlyInVehicle(pVehicleNew, 0);
			                           break;
		                               }
		                               case 5: // RANGE ROVER
		                               {
                                                                                 CVehicle *pVehicleNew = new CVehicle(490, -1800.0f, 775.755f, 35.5f, 0.0f, false);
																				 pPlayer->PutDirectlyInVehicle(pVehicleNew->m_dwGTAId, 0); 
                                                                                 //pPlayer->PutDirectlyInVehicle(pVehicleNew, 0);
			                           break;
		                               }
		                               case 6: // NISSAN GTR
		                               {
                                                                                 CVehicle *pVehicleNew = new CVehicle(502, -1800.0f, 775.755f, 35.5f, 0.0f, false);
																				 pPlayer->PutDirectlyInVehicle(pVehicleNew->m_dwGTAId, 0); 
                                                                                 //pPlayer->PutDirectlyInVehicle(pVehicleNew, 0);
			                           break;
		                               }
		                               case 7: // DODGE GTR
		                               {
                                                                                 CVehicle *pVehicleNew = new CVehicle(503,-1800.0f, 775.755f, 35.5f, 0.0f, false);
																				 pPlayer->PutDirectlyInVehicle(pVehicleNew->m_dwGTAId, 0); 
                                                                                 //pPlayer->PutDirectlyInVehicle(pVehicleNew, 0);
			                           break;
		                               }
		                               case 8: // SKYLINE
		                               {
                                                                                 CVehicle *pVehicleNew = new CVehicle(562, -1800.0f, 775.755f, 35.5f, 0.0f, false);
																				 pPlayer->PutDirectlyInVehicle(pVehicleNew->m_dwGTAId, 0); 
                                                                                 //pPlayer->PutDirectlyInVehicle(pVehicleNew, 0);
			                           break;
		                               }
		                               case 9: // BMW ЧЕТЫРЕ
		                               {
                                                                                 CVehicle *pVehicleNew = new CVehicle(558, -1800.0f, 775.755f, 35.5f, 0.0f, false);
																				 pPlayer->PutDirectlyInVehicle(pVehicleNew->m_dwGTAId, 0); 
                                                                                 //pPlayer->PutDirectlyInVehicle(pVehicleNew, 0);
			                           break;
		                               }
		                               case 10: // ALFA ROMEO
		                               {
                                                                                 CVehicle *pVehicleNew = new CVehicle(551, -1800.0f, 775.755f, 35.5f, 0.0f, false);
																				 pPlayer->PutDirectlyInVehicle(pVehicleNew->m_dwGTAId, 0); 
                                                                                 //pPlayer->PutDirectlyInVehicle(pVehicleNew, 0);
			                           break;
		                               }
		                               case 11: // MERS AUTOSCHOOOOOL
		                               {
                                                                                 CVehicle *pVehicleNew = new CVehicle(565, -1800.0f, 775.755f, 35.5f, 0.0f, false);
																				 pPlayer->PutDirectlyInVehicle(pVehicleNew->m_dwGTAId, 0); 
                                                                                 //pPlayer->PutDirectlyInVehicle(pVehicleNew, 0);
			                           break;
		                               }
	...
	if(GetGameState() == GAMESTATE_WAIT_CONNECT && (GetTickCount() - m_dwLastConnectAttempt) > 3000)
	{
        ...
        if(!once_2) {
			g_pJavaWrapper->ShowNotification(5, "Вы находитесь в списке очереди на подключение к серверу.\nЧтобы скоротать время {56D12A}исследуйте карту{ffffff} и {56D12A}новые места.", 0, "", ""); 
...
void CNetGame::Packet_CustomRPC(Packet* p)
{
	...
	case RPC_CUSTOM_ACTOR_PUT_IN_VEH:
		...
		case RPC_SHOW_PERSON: // рядом с кейсами
		{
