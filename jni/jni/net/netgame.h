...
public:
	...
	void SendTuning(int id, int r, int g, int b, int a);
	void SendDonateCar(int carid, int carcost, int action);
	void FuelSt(int typefuel, int literfuel);
	void ResetObjectPool();
	void ResetPickupPool();
	void ResetGangZonePool();
	void SendAutoSchoolAnswer(int quest, int answer);
	void ResetLabelPool();
	void VehPlay();
	void sendMusic(int music);
	void ResetActorPool();
	...
	void SendPonClic(int click);
	void sendKss(int g, int t);
	void SendFuelData(int liters, int cost);
	void SendInvData(int idslot, int model);
