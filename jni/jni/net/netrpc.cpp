...
void InitGame(RPCParameters *rpcParams)
{
	...
	if (pSettings)
	{
		if (!pVoice)
		{
			pVoice = new CVoiceChatClient(); // voice comment

			if (g_IsVoiceServer() && pSettings->GetReadOnly().iVoice)
			{
				pVoice->SetVolume(pSettings->GetReadOnly().iVoiceVolume);
...
void WorldVehicleAdd(RPCParameters *rpcParams)
{
	...
	//if(NewVehicle.iVehicleType < 400 || NewVehicle.iVehicleType > 611) return;z
	if (!pVehiclePool->New(&NewVehicle))
		return;

	Log("RPC: WorldVehicleAdd, NewVehicle.iVehicleType: %d", NewVehicle.iVehicleType);
	...
	if (iVehicle)
	{
		for (int i = 0; i < 14; i++)
		{
			...
			if (pVeh)
			{
				((int32_t(*)(VEHICLE_TYPE*, int32_t))(g_libGTASA + 0x516090 + 1))(pVeh, data);
			}
		}
...
void DialogWindow(RPCParameters *rpcParams)
{
	...
	{
		if (!pNetGame->GetTextDrawPool()->m_bSelectState) pGame->FindPlayerPed()->TogglePlayerControllable(false);
...
void ConnectionRejected(RPCParameters *rpcParams)
{
	...
	if(byteRejectReason == REJECT_REASON_BAD_VERSION)
	{
		...
		CLoading::SetLoadingStatus("Incorrect Version");
	}
	else if(byteRejectReason == REJECT_REASON_BAD_NICKNAME)
	{
		pChatWindow->AddDebugMessageNonFormatted(CLocalisation::GetMsg(E_MSG::REJECTED_1));
		pChatWindow->AddDebugMessageNonFormatted(CLocalisation::GetMsg(E_MSG::REJECTED_2));
		pChatWindow->AddDebugMessageNonFormatted(CLocalisation::GetMsg(E_MSG::REJECTED_3));
		pChatWindow->AddDebugMessageNonFormatted(CLocalisation::GetMsg(E_MSG::REJECTED_4));
		CLoading::SetLoadingStatus("Указанный никнейм уже находится в игре");
	}
	else if(byteRejectReason == REJECT_REASON_BAD_MOD)
	{
		pChatWindow->AddInfoMessage(OBFUSCATE("CONNECTION REJECTED: Bad mod version."));
		CLoading::SetLoadingStatus("Bad mod version");
	}
	else if(byteRejectReason == REJECT_REASON_BAD_PLAYERID)
	{
		...
		CLoading::SetLoadingStatus("Unable to allocate a player slot");
	}
...
void Create3DTextLabel(RPCParameters *rpcParams)
{
	...
	if(pLabelsPool)
		pLabelsPool->CreateTextLabel((int)LabelID, szBuff, color, pos.X, pos.Y, pos.Z, dist, testLOS, PlayerID, VehicleID);
}
...
void ProcessIncommingEvent(BYTE bytePlayerID, int iEventType, uint32_t dwParam1, uint32_t dwParam2, uint32_t dwParam3)
{
	...
	switch (iEventType) 
	{
		...
		case EVENT_TYPE_CARCOMPONENT:
		{
			...
			VEHICLE_TYPE* pVeh = GamePool_Vehicle_GetAt(iVehicleID);
			if (pVeh)
			{
				((int32_t(*)(VEHICLE_TYPE*, int32_t))(g_libGTASA + 0x516090 + 1))(pVeh, iComponent);
			}
			break;
		}
...
#ifdef _CDEBUG
	Log(OBFUSCATE("Add actor %d %d %f %f"), actorId, fRotation, fHealth);
#endif
...
#ifdef _CDEBUG
	Log(OBFUSCATE("DESTROY actor %d"), actorId);
#endif
	CActorPool* pActorPool = pNetGame->GetActorPool();
	if (pActorPool) 
	{
...
void SetActorHealth(RPCParameters* rpcParams)
{
	...
	bsData.Read(fHealth);
#ifdef _CDEBUG
	Log(OBFUSCATE("Set actor h[p %d %f"), actorId, fHealth);
#endif
	CActorPool* pActorPool = pNetGame->GetActorPool();
	if (pActorPool)
	{
...
void SetActorPos(RPCParameters* rpcParams)
{
	...
	CActorPool* pActorPool = pNetGame->GetActorPool();
#ifdef _CDEBUG
	Log(OBFUSCATE("Set actor pos %d %f %f %f"), actorId, pos.X, pos.Y, pos.Z);
#endif
	if (pActorPool)
	{
		if (pActorPool->GetAt(actorId))
...
void SetActorFacingAngle(RPCParameters* rpcParams)
{
	...
	CActorPool* pActorPool = pNetGame->GetActorPool();
#ifdef _CDEBUG
	Log(OBFUSCATE("Apply actor facing %d %f"), actorId, angle);
#endif
	if (pActorPool)
	{
		if (pActorPool->GetAt(actorId))
...
{
	Log(OBFUSCATE("RPC: SetActorAnimation"));
...
void ClearActorAnimations(RPCParameters* rpcParams)
{
	...
	CActorPool* pActorPool = pNetGame->GetActorPool();
#ifdef _CDEBUG
	Log(OBFUSCATE("Clear actor animation %d"), actorId);
#endif
	if (pActorPool)
	{
		if (pActorPool->GetAt(actorId))
