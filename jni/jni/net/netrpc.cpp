...
void ConnectionRejected(RPCParameters *rpcParams)
{
	...
	{
		pChatWindow->AddInfoMessage(OBFUSCATE("CONNECTION REJECTED: Unacceptable NickName"));
		pChatWindow->AddInfoMessage(OBFUSCATE("Please choose another nick between and 3-20 characters"));
		pChatWindow->AddInfoMessage(OBFUSCATE("Please use only a-z, A-Z, 0-9"));
		pChatWindow->AddInfoMessage(OBFUSCATE("Use /quit to exit or press ESC and select Quit Game"));
	}
...
void ProcessIncommingEvent(BYTE bytePlayerID, int iEventType, uint32_t dwParam1, uint32_t dwParam2, uint32_t dwParam3)
{
	...
	case EVENT_TYPE_CARCOMPONENT:
		...
		pChatWindow->AddDebugMessage(OBFUSCATE("Added car component: %d"),iComponent);
		break;
