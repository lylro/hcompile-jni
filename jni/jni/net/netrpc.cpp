...
void InitGame(RPCParameters *rpcParams)
{
	...
	if (!pVoice)
	{
		...
		if (g_IsVoiceServer() && pSettings->GetReadOnly().iVoice)
			...
			Log("Voice %d",pNetGame->m_iPort + 100);
	}
...
void WorldVehicleAdd(RPCParameters *rpcParams)
{
    ...
    CVehicle* pVehicle = pVehiclePool->GetAt((VEHICLEID)NewVehicle.VehicleID);
	if (iVehicle)
	{
		for (int i = 0; i < 14; i++)
		{
...
void ConnectionRejected(RPCParameters *rpcParams)
{
	...
	{
		pChatWindow->AddInfoMessage(OBFUSCATE("С данным ником нельзя зайти на сервер"));
		pChatWindow->AddInfoMessage(OBFUSCATE("Причины: "));
		pChatWindow->AddInfoMessage(OBFUSCATE("1) Под этим ником уже кто-то играет или предыдущая сессия не завершена;"));
		pChatWindow->AddInfoMessage(OBFUSCATE("2) Используется символы или добавлен лишний пробел;"));
		pChatWindow->AddInfoMessage(OBFUSCATE("Для выхода из игры используйте команду /q"));
	}
