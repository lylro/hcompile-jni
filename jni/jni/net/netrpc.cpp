...
void WorldVehicleAdd(RPCParameters* rpcParams)
{
    ...
    Log::addParameter("VehicleInfo.iBodyColor1", VehicleInfo.iBodyColor1);
    Log::addParameter("VehicleInfo.iBodyColor2", VehicleInfo.iBodyColor2);
    Log::addParameter("VehicleInfo.bytePaintJob", VehicleInfo.bytePaintJob);

    if(VehicleInfo.iVehicleType < 400 || VehicleInfo.iVehicleType > 611 && VehicleInfo.iVehicleType != 12596) return;

    if (!pVehiclePool->New(&VehicleInfo)) {
        Log::traceLastFunc("Vehicle creation failed");
        return;
    }

    CVehicle* pVehicle = pVehiclePool->GetAt(VehicleInfo.VehicleID);
    if (!pVehicle) {
        Log::traceLastFunc("Vehicle pointer is null after creation");
        return;
    }
