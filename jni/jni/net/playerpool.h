...
public:
	...
	{
		if (playerId > MAX_PLAYERS || m_pPlayers[playerId] == nullptr) return 0;
	...
	void SetLocalPlayerID(PLAYERID MyPlayerID)
	{
		...
		m_PlayersCount.push_back(MyPlayerID);
	}
	...
	{
		if(playerId >= MAX_PLAYERS || m_pPlayers[playerId] == nullptr) return nullptr;
		return m_pPlayers[playerId];
	}

	bool GetSlotState(PLAYERID playerId)
	{
		if(playerId >= MAX_PLAYERS || m_pPlayers[playerId] == nullptr) return false;
