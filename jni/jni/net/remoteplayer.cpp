...
uint8_t TickCountPlayer = 0;
uint32_t TickRateShowMarker = 0;
...
void CRemotePlayer::Process()
{
	...
	if(IsActive())
	{
		...
		if (m_dwMarkerID == -1 && pNetGame->m_iShowPlayerMarkers)
		{
			int RateTickCount = 300;
			if (TickCountPlayer >= 5)
			{
				RateTickCount = 2000;
				TickCountPlayer = 0;
			}
			if (m_pPlayerPed && m_pPlayerPed->GetDistanceFromLocalPlayerPed() < 300 && GetTickCount() - TickRateShowMarker > RateTickCount)
			{
				TickCountPlayer ++;
				TickRateShowMarker = GetTickCount();
				m_dwMarkerID = m_pPlayerPed->ShowMarker(m_PlayerID);
			}
		}
	...
	uint32_t dwColor, uint8_t byteFightingStyle, bool bVisible)
{
	...
	if(pPlayer)
	{
		...
		{
			//pGame->DisableMarker(m_dwMarkerID);
			m_pPlayerPed->HideMarker();
			m_dwMarkerID = -1;
		}
		
		if (!m_dwMarkerID && pNetGame->m_iShowPlayerMarkers)
		{
			m_dwMarkerID = m_pPlayerPed->ShowMarker(m_PlayerID);
		}*/
...
/*void CRemotePlayer::HandleVehicleEntryExit()
{
	...
	if(pVehiclePool && GetState() == PLAYER_STATE_DRIVER || GetState() == PLAYER_STATE_PASSENGER)
	{
		if(!m_pPlayerPed->IsInVehicle())
		{
...
void CRemotePlayer::StoreBulletSyncData(BULLET_SYNC_DATA* pBulletSync)
{
	...
	if (m_pPlayerPed->IsAdded())
	{
		...
		if (pwstWeapon->dwType != byteWeapon)
		{
...
void CRemotePlayer::ShowGlobalMarker(short sX, short sY, short sZ)
{
	...
	if(!m_pPlayerPed)
	{
		m_dwGlobalMarkerID = pGame->CreateRadarMarkerIcon(0, (float)sX, (float)sY, (float)sZ, m_PlayerID, 4);
	}
}
