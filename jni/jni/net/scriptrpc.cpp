...
void ScrSetCameraLookAt(RPCParameters *rpcParams)
{
	...
	bsData.Read(vecPos.Y);
	bsData.Read(vecPos.Z);
    if(pNetGame->GetPlayerPool()->GetLocalPlayer()->m_byteSpectateMode && pNetGame->GetPlayerPool()->GetLocalPlayer()->m_byteSpectateType != SPECTATE_TYPE_NONE){
        return;
    }
        ...
        for (int i = 0; i < iMaterialCount; i++)
        {
            ...
            if(byteMatType == 1) {
                ...
                if (modelId < 0 || modelId > 20000)
                    modelId = 18631;
				Log(OBFUSCATE("create modelId: %d | steep: 1"), modelId);
                if (!pGame->IsModelLoaded(modelId))
                {
                    int tries = 0;
					Log(OBFUSCATE("create modelId: %d | steep: 2"), modelId);
					//КРАШШШ
					pGame->RequestModel(modelId);
					pGame->LoadRequestedModels();
					Log(OBFUSCATE("create modelId: %d | steep: 3"), modelId);
                    while (!pGame->IsModelLoaded(modelId))
                    {
						...
						Log(OBFUSCATE("create modelId: %d | steep: 4 | tries %d"), modelId, tries);
                        tries++;
                        usleep(100);
                    }
                }
				Log(OBFUSCATE("create modelId: %d | steep: 5"), modelId);
				if (!modelId)
				{
					Log(OBFUSCATE("create modelId: %d | steep: failed 1"), modelId);
					return;
				}
				if (slot > MAX_MATERIALS)
					return;
				Log(OBFUSCATE("create modelId: %d | steep: 6"), modelId);
                // Material Object
...
void ScrTogglePlayerControllable(RPCParameters *rpcParams)
{
    ...
    if(byteControllable){
        pNetGame->GetPlayerPool()->GetLocalPlayer()->GetPlayerPed()->SetLockTogglePlayerControllable(false);
    }
    pNetGame->GetPlayerPool()->GetLocalPlayer()->GetPlayerPed()->TogglePlayerControllable((int)byteControllable);
    if(!byteControllable){
        pNetGame->GetPlayerPool()->GetLocalPlayer()->GetPlayerPed()->SetLockTogglePlayerControllable(true);
    }
}
...
void ScrPlayAudioStream(RPCParameters* rpcParams)
{
	...
	if (pAudioStream)
	{
		...
		if (pChatWindow) {
			pChatWindow->AddInfoMessage(OBFUSCATE("Audio Stream: %s"), szURL);
		}
	}
	else
	{
