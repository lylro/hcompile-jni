...
void ScrSetPlayerFacingAngle(RPCParameters *rpcParams)
{
	...
	RakNet::BitStream bsData((unsigned char*)Data,(iBitLength/8)+1,false);
	bsData.Read(fAngle);
	pGame->FindPlayerPed()->ForceTargetRotationLocal(fAngle);
}
...
void ScrSetPlayerSkin(RPCParameters *rpcParams)
{
	...
	{
		pPlayerPool->GetLocalPlayer()->GetPlayerPed()->SetPlayerSkin(uiSkin);
	}
	else
	{
		if(pPlayerPool->GetSlotState(iPlayerID) && pPlayerPool->GetAt(iPlayerID)->GetPlayerPed())
		{
			pPlayerPool->GetAt(iPlayerID)->GetPlayerPed()->SetPlayerSkin(uiSkin);
		}
	}
}
...
void ScrSetPlayerName(RPCParameters *rpcParams)
{
	...
	if( pPlayerPool->GetLocalPlayerID() == playerId )
	{
		...
		char buff[32];
		snprintf(pSettings->GetWrite().szNickName, 32, "%s", szNewName);
		pSettings->Save();
	}
}
...
void ScrVehicleParamsEx(RPCParameters *rpcParams)
{
	Log(OBFUSCATE("RPC: ScrVehicleParamsEx"));
	...
	if (pVehiclePool)
	{
		...
		if (pVehicle) 
		{
			...
			pVehicle->SetSirenState((alarm == 0xFF) ? 0 : alarm);
			pVehicle->SetDoorState(doors);
			pVehicle->SetBonnetAndBootStatus((bonnet == 0xFF) ? 0 : bonnet, (boot == 0xFF) ? 0 : boot);
		}
	}
}
...
void ScrCreateObject(RPCParameters* rpcParams)
{
	...
	if (!pGame->IsModelLoaded(ModelID)) 
	{
		...
		while(!pGame->IsModelLoaded(ModelID))
		{
			...
			if(iTryCount > 20)
			{
				Log(OBFUSCATE("[ScrCreateObject] Warning: Model %u wouldn't load in time!"), ModelID);
				return;
			}
            ...
            if(byteMatType == 1) 
			{
				...
				if (!pGame->IsModelLoaded(modelId))
                {
                    ...
                    while (!pGame->IsModelLoaded(modelId))
                    {
                        if (tries >= 5)
                        {
                            Log("Error on object %d original %d", modelId, oldmodel);
                            return;
                        }
                ...
                pObject->m_pMaterials[slot].pTex = (RwTexture*)LoadFromTxdSlot(str, tex);//LoadTextureForReTexture(tex);
...
void ScrSetPlayerAttachedObject(RPCParameters* rpcParams)
{
	...
	int iTryCount = 0;
	if (!pGame->IsModelLoaded(info.dwModelId)) 
	{
		pGame->RequestModel(info.dwModelId);
		pGame->LoadRequestedModels();
		while(!pGame->IsModelLoaded(info.dwModelId))
		{
			usleep(1);
			if(iTryCount > 20)
			{
				Log(OBFUSCATE("[ScrSetPlayerAttachedObject] Warning: Model %u wouldn't load in time!"), info.dwModelId);
				return;
			}
...
void ScrSetPlayerObjectMaterial(RPCParameters* rpcParams)
{
	...
	pObj->m_pMaterials[matId].pTex = (RwTexture*)LoadFromTxdSlot(str, tex);//LoadTextureForReTexture(tex);
	pObj->m_pMaterials[matId].dwColor = col;
...
void ScrPlayAudioStream(RPCParameters* rpcParams)
{
	...
	if (pAudioStream)
	{
		...
		Log(OBFUSCATE("Audio Stream: %s"), szURL);
	}
	else
	{
		Log(OBFUSCATE("Failed to play Sound! Audio Stream is not initialized!"));
	}
}
...
void ScrDeathMessage(RPCParameters* rpcParams)
{
   ...
   pKillList->MakeRecord(playername.c_str(), playercolor, playerId, killername.c_str(), killercolor, killerId, reason);
}
...
void ScrDisableRemoteVehicleCollision(RPCParameters *rpcParams)
{
	...
	/*if (pGame->FindPlayerPed()->IsInVehicle())
	{
		...
		if(pVehicle) 
		{
			...
			*(uint8_t*)((uintptr_t)pVehicle->m_pEntity + 29) |= 1;

			pVehicle->AddPhysical();
		}
	}*/
}
