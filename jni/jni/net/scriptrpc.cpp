...
void ScrSetPlayerPosFindZ(RPCParameters* rpcParams)
{
	...
	FLog("4");
	vecPos.z = CWorld::FindGroundZForCoord(vecPos.x, vecPos.y) + 1.5f;
	
	//vecPos.z = pGame->FindGroundZForCoord(vecPos.x, vecPos.y, vecPos.z) + 1.5f;CWorld::FindGroundZForCoord(X, Y)
	FLog("5");
    pLocalPlayer->DisableSurf();
	FLog("6");
	pLocalPlayer->GetPlayerPed()->m_pPed->SetPosn(vecPos.x, vecPos.y, vecPos.z);
