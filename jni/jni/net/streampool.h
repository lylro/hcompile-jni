...
class CStreamPool
{
	...
	std::list<int> m_StreamList;

	bool m_bIsCreated[MAX_STREAMS];
...
public:
	...
	CStream* AddStream(int iID, int iType, int iPlayerID, VECTOR* pPos, int iVirtualWorld, int iInterior, float fDistance, const char* szUrl);
	void AttachedStream(int iID, int iType, int iPlayerID);
	...
	void SetStreamVolumeJava(float fVolume);
