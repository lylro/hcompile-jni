...
void CVehiclePool::Process()
{
	...
	for(VEHICLEID x = 0; x < MAX_VEHICLES; x++)
	{
		if(GetSlotState(x))
		{
			...
			if(m_bIsActive[x])
			{
				if (pVehicle->GetHealth() <= 272.0f)
				{
					pVehicle->SetHealth(270.0f);
				}
				...
				/*if(!pChatWindow->vehp){
					...
					if (GetTickCount() - time >= 600)
					{
						if (pVehicle->bLeftButtonPressed)
						{
							pVehicle->bLeftLEDOn = !pVehicle->bLeftLEDOn;
							pVehicle->bRightLEDOn = false;
							pVehicle->bCenterLEDOn = false;
							pVehicle->TurnLights(pVehicle->bLeftLEDOn ? 1 : 0);
						}
						else if (pVehicle->bRightButtonPressed)
						{
							pVehicle->bRightLEDOn = !pVehicle->bRightLEDOn;
							pVehicle->bLeftLEDOn = false;
							pVehicle->bCenterLEDOn = false;
							pVehicle->TurnLights(pVehicle->bRightLEDOn ? 2 : 0);
						}
						else if (pVehicle->bCenterButtonPressed)
						{
							pVehicle->bLeftLEDOn = !pVehicle->bLeftLEDOn;
							pVehicle->bRightLEDOn = !pVehicle->bRightLEDOn;
							pVehicle->bCenterLEDOn = !pVehicle->bCenterLEDOn;
							pVehicle->TurnLights(pVehicle->bCenterLEDOn ? 3 : 0);
						}
						else
						{
							pVehicle->bLeftLEDOn = false;
				...
				uint8_t bSlot = pVehicle->GetTurnLights();
				if(bSlot != pVehicle->GetOldLights())
				{
					if (bSlot == 0)
					{
						//sprintf(&szOld[0], "remaptoner", bSlot);
						pVehicle->RemoveTexture("v_t_left");
						pVehicle->RemoveTexture("v_t_right");
						pVehicle->RemoveTexture("v_t_leftt");
						pVehicle->RemoveTexture("v_t_rightt");
						//RemoveTexture("vehiclelightson128");/RemoveTexture("vehiclelights128");
						pVehicle->ApplyTexture("v_t_left", "v_t_left");
						pVehicle->ApplyTexture("v_t_right", "v_t_right");
					...
					if(bSlot == 1){
						...
						pVehicle->RemoveTexture("v_t_right");
						pVehicle->RemoveTexture("v_t_rightt");
						pVehicle->ApplyTexture("v_t_left", "v_t_leftt");
						pVehicle->ApplyTexture("v_t_right", "v_t_right");
					}
					if(bSlot == 2){
						pVehicle->RemoveTexture("v_t_right");
						...
						pVehicle->RemoveTexture("v_t_left");
						pVehicle->RemoveTexture("v_t_leftt");
						pVehicle->ApplyTexture("v_t_right", "v_t_rightt");
						pVehicle->ApplyTexture("v_t_left", "v_t_left");
					}
					if(bSlot == 3){
						pVehicle->RemoveTexture("v_t_left");
						pVehicle->RemoveTexture("v_t_leftt");
						pVehicle->RemoveTexture("v_t_right");
						pVehicle->RemoveTexture("v_t_rightt");//RemoveTexture("vehiclelightson128");RemoveTexture("vehiclelights128");
						pVehicle->ApplyTexture("v_t_left", "v_t_leftt");
						//ApplyTexture("vehiclelights128", "vehiclelightson128");
						pVehicle->ApplyTexture("v_t_right", "v_t_rightt");
					}
					pVehicle->oldLights = bSlot;
				}
				*/
...
VEHICLE_TYPE* CVehiclePool::FindAllID()
{
