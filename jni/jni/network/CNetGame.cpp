...
void CNetGame::DbgConnect(char* nickname, int serverChoice = 0) {
    const char* serverIP;
    int serverPort;
    
    if (serverChoice == 0) {
        // Первый сервер
        serverIP = OBFUSCATE("185.189.255.97");
        serverPort = 3005;
    } else {
        // Второй сервер
        serverIP = OBFUSCATE("185.189.255.98"); // Замени на реальный
        serverPort = 3005;
    }
    
    m_rakClientInterface->Connect((char*)serverIP, serverPort, 0, 0, 5);
    m_szHostOrIp = (char*)serverIP;
