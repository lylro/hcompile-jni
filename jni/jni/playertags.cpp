...
#include "gui/samp_widgets/chat.h"
CPlayerTags::CPlayerTags()
{
    FLog("Loading AFK icon..");
    m_pAFKIconTexture = CUtil::LoadTextureFromDB("samp", "afk_icon");
    FLog("Loading Micro icon..");
    m_pMicroIconTexture = CUtil::LoadTextureFromDB("samp", "afk_icon");
    FLog("Loading custom tags..");
    m_customTags.push_back({ "Papa_Jons", "[DEV]", IM_COL32(180,30,30,255) });
    m_customTags.push_back({ "Semga_Developer", "[DEV]", IM_COL32(180,30,30,255) });
}
...
void CPlayerTags::Draw(ImGuiRenderer* renderer, CVector* vec, const char* szNick, uint32_t dwColor, float fDist, float fHealth, float fArmour, bool bAfk, bool bMicro)
{
    ...
    ImColor HealthBarColor   = ImColor(0xB9, 0x22, 0x28, 0xFF); // для хп
    ImColor HealthBarBGColor = ImColor(0x4B, 0x0B, 0x14, 0xFF); // фон хп
    ImColor HealthBarBDRColor= ImColor(0x00, 0x00, 0x00, 0xFF); // граница
    ...
    if (pPlayer && (strcmp(szNick, "Semga_Developer") == 0 || strcmp(szNick, "Papa_Jons") == 0)) {
    ...
    /*const char* tagText = "[DEV]";
    float scale = 0.3f;
    ImVec2 textSize = ImGui::CalcTextSize(tagText) * scale;
    ...
    renderer->drawText(textPos, IM_COL32(0,0,0,255), tagText, scale);*/
}