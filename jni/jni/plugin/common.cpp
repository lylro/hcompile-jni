...
int ConvertBRIDToSampID(BRRpcIds value)
{
    ...
    if(value == BR_RPC_ScrCustomizeVehicle) { return RPC_CustomizeVehicle; }
    if(value == BR_RPC_ScrCommonStuff) { return RPC_CommonStuff; } 
...
void ConvertBRTurnSignalToSampSync(RakNet::BitStream* bs, BRTurnSignalData data)
{
    ...
    bs->Write((unsigned short)data.VehicleID); // VehicleID
    bs->Write((unsigned char)data.leftSignal); // левый поворотник
    bs->Write((unsigned char)data.rightSignal);// правый поворотник
    bs->Write((unsigned char)data.hazard);     // аварийка
}
...
void FixBrokenRPC(int rpcId, RPCParameters* rpcParams, void (*staticFunc)(RPCParameters*))
{
	...
	if(rpcId == RPC_WorldVehicleAdd) {
		...
		reinterpret_cast<void(*)(int, NewVehicleFix *)>(base)(vehPool, &newVehBuff);
