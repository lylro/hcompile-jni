...
bool IsRPCNeedFix(int rpcId)
{
    ...
    if(rpcId == RPC_ScrCreateObject) { return true; }
...
void FixBrokenRPC(int rpcId, RPCParameters* rpcParams, void (*staticFunc)(RPCParameters*))
{
    ...
    if(rpcId == RPC_ScrCreateObject) {
    uint16_t ObjectID;
    int iModelID;
    float vecPos[3], vecRot[3];
    float fDrawDistance = 300.0f;
    ...
    uintptr_t objPoolAddr = CGameAPI::GetBase(xorstr("CNetGame::m_pObjectPool"));
    uintptr_t newFunc = CGameAPI::GetBase(xorstr("CNetObjectPool::New")); 
    if(objPoolAddr && newFunc) {
        uintptr_t objPool = *(uintptr_t*)(objPoolAddr);
        reinterpret_cast<void(*)(uintptr_t, uint16_t, int, CVector, CVector, float)>(
            newFunc
        )(
            objPool,
            ObjectID,
            iModelID,
            CVector{vecPos[0], vecPos[1], vecPos[2]},
            CVector{vecRot[0], vecRot[1], vecRot[2]},
            fDrawDistance
        );
    }
    return;
