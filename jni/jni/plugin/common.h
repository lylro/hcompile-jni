...
{
    BR_ID_GUI_PACKET_ID = 0x68,
    BR_ID_AIM_SYNC = 0x5C,
    BR_ID_WEAPONS_UPDATE = 0x3,
    BR_ID_STATS_UPDATE = 0x4,
    BR_ID_PASSENGER_SYNC = 0x60,
    BR_ID_TRAILER_SYNC = 0x61,
    BR_ID_PLAYER_SYNC = 0x5B,
    BR_ID_UNOCCUPIED_SYNC = 0x98,
    BR_ID_SPECTATOR_SYNC = 0x96,
    BR_ID_BULLET_SYNC = 0x5E,
    BR_ID_VEHICLE_SYNC = 0x5F,
    BR_ID_TURNLIGHTS_SYNC = 0xC,
    BR_ID_USER_PACKET_ENUM = 0xD,
    BR_ID_CONNECTION_ATTEMPT_FAILED = 0xE,
    BR_ID_DISCONNECTION_NOTIFICATION = 0x16,
    BR_ID_NO_FREE_INCOMING_CONNECTIONS = 0x10,
    BR_ID_CONNECTION_REQUEST_ACCEPTED = 0x12,
    BR_ID_TIMESTAMP = 0x35,
    BR_ID_PONG = 0x13,
    BR_ID_CONNECTION_BANNED = 0x14,
    BR_ID_CONNECTION_LOST = 0x17,
    BR_ID_MODIFIED_PACKET = 0x37,
    BR_ID_NEW_INCOMING_CONNECTION = 0x36,
...
struct BRInCarSyncData
{
    ...
    uint16_t EngineStatus; // <-- добавляем сюда
};
#pragma pack(pop);
...
struct BRTurnSignalData {
    uint16_t VehicleID;
    uint8_t leftSignal;
    uint8_t rightSignal;
    uint8_t hazard; // <-- здесь регистр маленький
};
