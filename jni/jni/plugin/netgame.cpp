	...
	while(pkt = pRakClient->Receive())
	{
		...
		switch(packetIdentifier)
		{
			...
			case ID_CONNECTION_REQUEST_ACCEPTED:
				CChat::AddDebugMessage(xorstr("Соединено. Подготовка к игре..."));
...
void CNetGame::Packet_VehicleSync(Packet* pkt)
{
	...
	bsData.ReadBits((unsigned char*)&icsync.playerHealth, 16);
    bsData.ReadBits((unsigned char*)&icsync.playerArmour, 16);
