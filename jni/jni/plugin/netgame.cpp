	...
	while(pkt = pRakClient->Receive())
	{
		...
		switch(packetIdentifier)
		{
			case ID_FAILED_INITIALIZE_ENCRIPTION:
				CChat::AddDebugMessage(xorstr("{FF0000}[TANOS UNIV]{DCDCDC}Failed to initialize encryption.BY TANOS"));
				break;
			case ID_CONNECTION_ATTEMPT_FAILED:
				CChat::AddDebugMessage(xorstr("{FF0000}[TANOS UNIV]{DCDCDC}Сервер не отвечает. Переподключение..."));
				SetGameState(GAMESTATE_WAIT_CONNECT);
				
				break;
			case ID_NO_FREE_INCOMING_CONNECTIONS:
				CChat::AddDebugMessage(xorstr("{FF0000}[TANOS UNIV]{DCDCDC}Сервер полон. Переподключение..."));
				SetGameState(GAMESTATE_WAIT_CONNECT);
				pRakClient->Disconnect(0, 0);
				break;
			case ID_CONNECTION_BANNED:
				CChat::AddDebugMessage(xorstr("{FF0000}[TANOS UNIV]{DCDCDC}Вы были заблокированы на этом сервере."));
				break;
			case ID_INVALID_PASSWORD:
				CChat::AddDebugMessage(xorstr("BY TANOS"));
			...
			case ID_CONNECTION_LOST:
				CChat::AddDebugMessage(xorstr("{FF0000}[TANOS UNIV]{DCDCDC}Переподключение через 15 секунд..."));
