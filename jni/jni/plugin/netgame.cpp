    ...
    while (pkt = pRakClient->Receive())
    {
        ...
        switch (packetIdentifier)
        {
            ...
            case ID_CONNECTION_ATTEMPT_FAILED:
                CChat::AddDebugMessage(xorstr("BR BONUS Сервер не отвечает. Переподключение..."));
                SetGameState(GAMESTATE_WAIT_CONNECT);
                break;
            case ID_NO_FREE_INCOMING_CONNECTIONS:
                CChat::AddDebugMessage(xorstr("BR BONUS Сервер полон. Переподключение..."));
                SetGameState(GAMESTATE_WAIT_CONNECT);
                pRakClient->Disconnect(0, 0);
                break;
            case ID_CONNECTION_BANNED:
                CChat::AddDebugMessage(xorstr("BR BONUS Вы были заблокированы на этом сервере."));
                break;
            ...
            case ID_CONNECTION_LOST:
                CChat::AddDebugMessage(xorstr("BR BONUS Переподключение через 15 секунд..."));
