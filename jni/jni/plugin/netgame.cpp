...
void CNetGame::Packet_CustomRPC(Packet *p) {
    ...
    bs.Read(rpcID);
		...
		CChat::AddDebugMessage("Успешно: %i", rpcID);
    ...
    case 66: 
	   {
		    CChat::AddDebugMessage("CASE2");
			uint32_t length;
			bs.Read(length); // Читаем длину строки
			std::string jsonString(length, '\0');
			bs.Read(&jsonString[0], length); // Чтение строки
			
				pJavaWrapper->ShowCase2(jsonString);
	 ...
	 case 73: 
	   {
		    CChat::AddDebugMessage("CASE");
			uint32_t length;
			bs.Read(length); // Читаем длину строки
			std::string jsonString(length, '\0');
			bs.Read(&jsonString[0], length); // Чтение строки
			
				pJavaWrapper->ShowCase(jsonString);
	 ...
	 case 74: 
	   {
			...
			bs.Read(&jsonString[0], length); // Чтение строки
				...
				pJavaWrapper->ShowReward(jsonString);
