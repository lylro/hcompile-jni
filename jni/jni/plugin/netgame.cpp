    ...
    while (pkt = pRakClient->Receive())
    {
        ...
        switch (packetIdentifier)
        {
            case ID_FAILED_INITIALIZE_ENCRIPTION:
				CChat::AddDebugMessage(xorstr("Не удалось инициализировать шифрование."));
				break;
			...
			case ID_INVALID_PASSWORD:
				CChat::AddDebugMessage(xorstr("Сервер закрыт на работы."));
			...
			case ID_CONNECTION_LOST:
				CChat::AddDebugMessage(xorstr("Потеряно соединение с сервером, переподключение через 15 секунд..."));
				Packet_ConnectionLost(pkt);
				break;
			case ID_DISCONNECTION_NOTIFICATION:
				CChat::AddDebugMessage(xorstr("Вы были отключены от сервера, можете выйти."));
				pRakClient->Disconnect(0);
				break;
            case ID_AIM_SYNC:
                Packet_AimSync(pkt);
