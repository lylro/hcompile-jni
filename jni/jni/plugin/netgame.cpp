...
void FLog1(const char* fmt, ...)
{
	...
	const char* pszStorage = "/storage/emulated/0/Android/data/com.vise.russia/";


	if (flLog == nullptr && pszStorage != nullptr)
	{
		sprintf(buffer, "%s/vise_log.txt", pszStorage);
...
void CNetGame::Packet_PassengerSync(Packet* pkt)
{
    ...
    uint16_t dataSize = bsData.GetNumberOfUnreadBits() / 8;
    if(dataSize > 0 && dataSize <= sizeof(passengerSync)) {
