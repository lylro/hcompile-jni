...
void FLog1(const char* fmt, ...)
{
	...
	const char* pszStorage = "/storage/emulated/0/Android/data/com.tanos.client/";
    ...
    while(pkt = pRakClient->Receive())
    {
        ...
        switch(packetIdentifier)
        {
            ...
            case ID_CONNECTION_ATTEMPT_FAILED:
                CChat::AddDebugMessage(xorstr("Сервер не отвечает. Переподключение...(UNIV TANOS)"));
                SetGameState(GAMESTATE_WAIT_CONNECT);
                break;

            case ID_NO_FREE_INCOMING_CONNECTIONS:
                CChat::AddDebugMessage(xorstr("Сервер полон. Переподключение...(UNIV TANOS)"));
                SetGameState(GAMESTATE_WAIT_CONNECT);
                pRakClient->Disconnect(0, 0);
                break;

            case ID_CONNECTION_BANNED:
                CChat::AddDebugMessage(xorstr("Вы были заблокированы на этом сервере.(UNIV TANOS)"));
                break;
            ...
            case ID_CONNECTION_LOST:
                CChat::AddDebugMessage(xorstr("Переподключение через 15 секунд...(UNIV TANOS)"));
