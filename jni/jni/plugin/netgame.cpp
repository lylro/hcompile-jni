...
void CNetGame::ProcessNetwork()
{
	...
	while(pkt = pRakClient->Receive())
	{
		...
		switch(packetIdentifier)
		{
			...
			case ID_AUTH_KEY:
			{
				CChat::AddDebugMessage(xorstr("process authkey"));
				Packet_AuthKey(pkt);
				break;
			}
			case ID_CONNECTION_REQUEST_ACCEPTED:
			{
				CChat::AddDebugMessage(xorstr("process connect succed"));
...
void CNetGame::SendChatCommand()
{
	reinterpret_cast<void(*)()>(CGameAPI::GetBase(xorstr("CNetGame::SendChatCommand")))();
}