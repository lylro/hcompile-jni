    ...
    while(pkt = pRakClient->Receive())
    {
		packetIdentifier = GetPacketID(pkt);
			...
			case 252: 
			{
				...
				bsData.Read(guiId); 
				bsData.Read(jsonLength);
				
				char* jsonData = new char[jsonLength + 1];
				bsData.Read(jsonData, jsonLength);
...
void CNetGame::Packet_TurnLightsSync(Packet* pkt)
{
...
// Закрываем предыдущий код, если нужно
// }  <-- добавь, если выше открывался блок без закрытия

/*if (localPlayerVeh == pVehicle)
{
    // Проверяем состояние поворотников
    if (turnSignalState == 1) // левый
    {
        PlaySound(0x20); // звук тика влево
    }
    else if (turnSignalState == 2) // правый
    {
        PlaySound(0x21); // звук тика вправо
    }
    else if (turnSignalState == 3) // аварийка
    {
        PlaySound(0x22); // звук аварийки
    }

    // Автоотключение поворотников
    static uint32_t lastTick = 0;
    uint32_t currentTick = CTimer::m_snTimeInMilliseconds;

    if (turnSignalState != 0 && (currentTick - lastTick) > 4500)
    {
        turnSignalState = 0;
        PlaySound(0x23); // звук выключения
    }
