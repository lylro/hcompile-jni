...
void CScoreBoard::Toggle()
{
    ...
    //    pNetGame->GetPlayerPool()->GetLocalPlayer()->GetPlayerPed()->TogglePlayerControllableWithoutLock(false);
        ...
        PLAYERID i = 1, x;
		g_pJavaWrapper->setStat(m_pPlayers[0].dwID, m_pPlayers[0].szName, m_pPlayers[0].iScore, m_pPlayers[0].dwPing);
        for (x = 0; x < MAX_PLAYERS; x++)
        {
			...
			g_pJavaWrapper->setStat(m_pPlayers[i].dwID, m_pPlayers[i].szName, m_pPlayers[i].iScore, m_pPlayers[i].dwPing);
