...
bool CScrollbar::OnTouchEvent(int type, bool multi, int x, int y)
{
	...
	case TOUCH_MOVE:
	{
		...
		m_iValue = (int)((m_fValue * ((float)m_iMaxValue / (float)m_iMaxValue)) * (float)m_iMaxValue);
		return false;
	}
...
void CScrollbar::Draw()
{
	...
	ImGui::GetOverlayDrawList()->AddRectFilled(a, b, ImColor(col_all1, col_all2, col_all3, 0xFF), 0.0f);
}
