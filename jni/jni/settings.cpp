...
void CSettings::Save(int iIgnoreCategory)
{
	...
	ini_table_create_entry(config, "bymerixton", "merixton", m_Settings.merixton); // host
	ini_table_create_entry_as_int(config, "bymerixton", "topchik", m_Settings.topchik); // port
...
void CSettings::LoadSettings(const char* szNickName, int iChatLines)
{
	...
	// load host and port
	const char* host = ini_table_get_entry(config, "bymerixton", "merixton"); // host
	if (host) strcpy(m_Settings.merixton, host);
	m_Settings.topchik = ini_table_get_entry_as_int(config, "bymerixton", "topchik", -1); // port
