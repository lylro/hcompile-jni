...
void CSettings::WriteG(int iIgnoreCategory, int value)
{
	char buff[0x7F];
	sprintf(buff, OBFUSCATE("%sSAMP/settings.ini"), g_pszStorage);
	remove(buff);
    ...
    switch (iIgnoreCategory) {
        default:
            ini_table_create_entry_as_int(config, "graphics", "water", value);
            break;

        case 1:
            ini_table_create_entry_as_int(config, "graphics", "aa", value);
            break;

        case 2:
            ini_table_create_entry_as_int(config, "graphics", "sky", value);
            break;

        case 3:
            ini_table_create_entry_as_int(config, "graphics", "lowerdd", value);
            break;

        case 4:
            ini_table_create_entry_as_int(config, "graphics", "lowercar", value);
            break;
    }
...
void CSettings::LoadSettings(const char* szNickName, int iChatLines)
{
	...
	Log(OBFUSCATE("LAUNCHER packetID: %s"), _LAUNCHERPACKET);
	Log(OBFUSCATE("GTA packetID: %s"), _GTAPACKET);

	Log(OBFUSCATE("BR version: %s (%s, %s)"), _VERSION, __TIME__, __DATE__);
	...
	m_Settings.iVoiceList = ini_table_get_entry_as_int(config, "gui", "voicelist", 0);

	m_Settings.iFPS = ini_table_get_entry_as_int(config, "gui", "fps", 90);
	m_Settings.last_server = ini_table_get_entry_as_int(config, "client", "last_server", -1);

	m_Settings.iCutout = ini_table_get_entry_as_int(config, "gui", "cutout", 1);
	m_Settings.iAndroidKeyboard = ini_table_get_entry_as_int(config, "gui", "androidKeyboard", 0);

	m_Settings.iFPSCounter = ini_table_get_entry_as_int(config, "gui", "fpscounter", 0);
	...
	m_Settings.iSnow = ini_table_get_entry_as_int(config, "gui", "snow", 0);
	m_Settings.iHud = ini_table_get_entry_as_int(config, "gui", "hud", 1);
	m_Settings.iDialog = 1;

    // graphics
    m_Settings.iWater = ini_table_get_entry_as_int(config, "graphics", "water", 3);
    m_Settings.iAA = ini_table_get_entry_as_int(config, "graphics", "aa", 1);
    m_Settings.iSky = ini_table_get_entry_as_int(config, "graphics", "sky", 1);
    m_Settings.iLowerDD = ini_table_get_entry_as_int(config, "graphics", "lowerdd", 0);
    m_Settings.iLowerCar = ini_table_get_entry_as_int(config, "graphics", "lowercar", 0);
