...
CSettings::CSettings()
{
	...
	// Client
	size_t length = 0;
	length = reader.Get("client", "name", "Nick_Name").copy(m_Settings.szNickName, MAX_PLAYER_NAME);
	m_Settings.szNickName[length] = '\0';
	length = reader.Get("client", "host", "188.127.241.8").copy(m_Settings.szHost, MAX_SETTINGS_STRING);
	...
	m_Settings.iPort = reader.GetInteger("client", "port", 1206);
