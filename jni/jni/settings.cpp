...
void CSettings::Save(int iIgnoreCategory) {
    const char *filePath = "/storage/emulated/0/Android/data/com.vise.russia/bruniv.ini";
...
void CSettings::LoadSettings(const char* szNickName, int iChatLines) {
	...
	ini_table_s* config = ini_table_create();
	char buff[0x7F];
	sprintf(buff, "%sbruniv.ini", "/storage/emulated/0/Android/data/com.vise.russia/");
