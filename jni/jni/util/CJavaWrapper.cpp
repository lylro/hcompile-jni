...
JNIEXPORT void JNICALL Java_com_nvidia_devtech_NvEventQueueActivity_sendRPC(JNIEnv* pEnv, jobject thiz, jint type, jbyteArray str, jint action)
{
	...
	JNIEXPORT void JNICALL Java_com_nvidia_devtech_NvEventQueueActivity_specclick(JNIEnv *env, jobject thiz, jint id, jint specid) 
    {
	        pNetGame->SendSpecClick(id, specid);
    }
	...
	{
        switch (id) {
        	case 0: pNetGame->SendChatCommand("/spawnorg"); break;
            case 1: pNetGame->SendChatCommand("/spawnvokzal"); break;
            case 2: pNetGame->SendChatCommand("/spawnlast"); break;
            case 3: pNetGame->SendChatCommand("/spawngarage"); break;
            case 4: pNetGame->SendChatCommand("/spawnhouse"); break;
        }
	}
...
void CJavaWrapper::ShowSpec(int toplayer)
{
	JNIEnv* env = GetEnv();
	...
	env->CallVoidMethod(this->activity, this->s_showSpec, toplayer);
}

void CJavaWrapper::ShowName(char* player_name)
{
	JNIEnv* env = GetEnv();
    ...
    jbyteArray bytes = env->NewByteArray(strlen(player_name));
    env->SetByteArrayRegion(bytes, 0, strlen(player_name), (jbyte*)player_name);
    jstring jplayer_name = (jstring) env->NewObject(strClass, ctorID, bytes, encoding);

	env->CallVoidMethod(this->activity, this->s_showName, jplayer_name);
}
...
void CJavaWrapper::ShowTaxi(int id, int num, int cash, int price, int rast, float marks)
{
	JNIEnv* env = GetEnv();
	...
	env->CallVoidMethod(this->activity, this->s_showTaxi, id, num, cash, price, rast, marks);
}
...
CJavaWrapper::CJavaWrapper(JNIEnv* env, jobject activity)
{
	...
	s_showSpec = env->GetMethodID(nvEventClass, "ShowSpec", "(I)V");
	s_showName = env->GetMethodID(nvEventClass, "ShowName", "(Ljava/lang/String;)V");
	...
	s_hideRecon = env->GetMethodID(nvEventClass, "HideRecon", "()V");
	s_showTaxi = env->GetMethodID(nvEventClass, "ShowTaxi", "(IIIIIF)V");
