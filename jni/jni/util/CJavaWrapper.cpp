...
JNIEXPORT void JNICALL Java_com_nvidia_devtech_NvEventQueueActivity_sendRPC(JNIEnv* pEnv, jobject thiz, jint type, jbyteArray str, jint action)
{
	...
	JNIEXPORT void JNICALL Java_com_nvidia_devtech_NvEventQueueActivity_SetRadarPos(JNIEnv* env, jobject thiz, jfloat x1, jfloat y1, jfloat size) {
	...
	JNIEXPORT void JNICALL Java_com_nvidia_devtech_NvEventQueueActivity_InitMap(JNIEnv* pEnv, jobject thiz, jboolean b)
	{
		*(uint8_t*)(g_libGTASA + 0X26AA40) = 1;
		return g_pJavaWrapper->ShowNotification(2, "By Hyonte", 10, "", "");
	}
...
void CJavaWrapper::InitPause()
{
	...
	env->CallVoidMethod(this->activity, this->s_InitPause);
}
...
CJavaWrapper::CJavaWrapper(JNIEnv* env, jobject activity)
{
	...
	s_InitPause = env->GetMethodID(nvEventClass, "InitPause", "()V");
