...
void CJavaWrapper::ShowChoose()
{
    ...
    env->CallVoidMethod(this->activity, this->show_Choose);
}
...
void CJavaWrapper::ShowMenu()
{
    ...
    env->CallVoidMethod(this->activity, this->show_Menu);
}
...
extern "C"
{
	...
	JNIEXPORT void JNICALL Java_com_nvidia_devtech_NvEventQueueActivity_onNativeHeightChanged(JNIEnv* pEnv, jobject thiz, jint orientation, jint height)
	{
		...
		JNIEXPORT void JNICALL Java_com_nvidia_devtech_NvEventQueueActivity_OtpravkaServer(JNIEnv* pEnv, jobject thiz, jint server)
	{
		if(server == 1)
		{
			pNetGame = new CNetGame("141.94.184.105",1989,pSettings->GetReadOnly().szNickName,pSettings->GetReadOnly().szPassword);
		}
		else if(server == 2)
		{
			pNetGame = new CNetGame("141.94.184.105",1989,pSettings->GetReadOnly().szNickName,pSettings->GetReadOnly().szPassword);
	...
	JNIEXPORT void JNICALL Java_com_nvidia_devtech_NvEventQueueActivity_OtpravkaServer1(JNIEnv* pEnv, jobject thiz)
		{
	...
	JNIEXPORT jbyteArray JNICALL Java_com_nvidia_devtech_NvEventQueueActivity_getNativeHudElementColor(JNIEnv* pEnv, jobject thiz, jint id)
	{
		...
		JNIEXPORT void JNICALL Java_com_nvidia_devtech_NvEventQueueActivity_sendMenuClick(JNIEnv* pEnv, jobject thiz, jint id)
	{
        switch (id) {
        	case 0: pNetGame->SendChatCommand("/mm"); break;
            case 1: pNetGame->SendChatCommand("/cars"); break;
            case 2: pNetGame->SendChatCommand("/family"); break;
            case 3: pNetGame->SendChatCommand("/anim"); break;
            case 4: pNetGame->SendChatCommand("/gps"); break;
            case 5: pNetGame->SendChatCommand("/inv"); break;
	    case 6: pNetGame->SendChatCommand("/promo"); break;
	    case 7: pNetGame->SendChatCommand("/donate"); break;
	    case 8: pNetGame->SendChatCommand("/report"); break;
	    case 9: pNetGame->SendChatCommand("/mm"); break;
	    case 10: g_pJavaWrapper->ShowMenu(); break;
...
CJavaWrapper::CJavaWrapper(JNIEnv* env, jobject activity)
{
    ...
    show_Choose = env->GetMethodID(nvEventClass, "ShowChoose", "()V");
    show_Menu = env->GetMethodID(nvEventClass, "ShowMenu", "()V");
