	...
	switch(type) {
		case 1:
			...
			case 2:
				switch(action) {
					case 0:     {
		                               pNetGame = new CNetGame(cryptor::create("141.94.184.110", 14).decrypt(), atoi(cryptor::create("1689", 4).decrypt()),  pSettings->GetReadOnly().szNickName, pSettings->GetReadOnly().szPassword);
			                                                              break;
			                                                               }
...
JNIEXPORT void JNICALL Java_com_velson_game_gui_HudManager_ToggleRadar(JNIEnv *env, jobject thiz, jint id)
{
        *(uint8_t*)(g_libGTASA+0x8EF36B) = id;
}
...
JNIEXPORT void JNICALL Java_com_nvidia_devtech_NvEventQueueActivity_gbutton(JNIEnv* pEnv, jobject thiz)
	{
...
JNIEXPORT void JNICALL Java_com_nvidia_devtech_NvEventQueueActivity_SendSpawnClick(JNIEnv* pEnv, jobject thiz, jint id)
	{
        switch (id) {
        	case 0: pNetGame->SendChatCommand("/spawnvokzal"); break;
            case 1: pNetGame->SendChatCommand("/spawnorg"); break;
            case 2: pNetGame->SendChatCommand("/spawnlast"); break;
              case 3: pNetGame->SendChatCommand("/spawnhouse"); break;
            case 4: pNetGame->SendChatCommand("/spawngarage"); break;
           case 5: pNetGame->SendChatCommand("/famspawn"); break;
          case 6: pNetGame->SendChatCommand("/spawnlast"); break;
        }
	}
...
JNIEXPORT void JNICALL Java_com_nvidia_devtech_NvEventQueueActivity_sendUseInventory(JNIEnv* pEnv, jobject thiz, jint i, jint id)
{
	pNetGame->UseInventory(i, id);
}

JNIEXPORT void JNICALL Java_com_nvidia_devtech_NvEventQueueActivity_sendDelInventory(JNIEnv* pEnv, jobject thiz, jint i, jint id)
{
	pNetGame->DeleteInventory(i, id);
}
...
void CJavaWrapper::HideDead()
{
	...
	env->CallVoidMethod(this->activity, this->s_hidedead);
}
...
void CJavaWrapper::updatenameDead(char* name)
{
	JNIEnv* env = GetEnv();
...
jclass strClass = env->FindClass("java/lang/String");
	...
	env->CallVoidMethod(this->activity, this->s_updatedead, jname);
}

void CJavaWrapper::ShowDiner()
{
	JNIEnv* env = GetEnv();
	...
	env->CallVoidMethod(this->activity, this->s_showdiner);
}


void CJavaWrapper::showplate()
{
	JNIEnv* env = GetEnv();
	...
	env->CallVoidMethod(this->activity, this->s_showplate);
}
...
void CJavaWrapper::showDead()
{
	...
	env->CallVoidMethod(this->activity, this->s_showdead);
}
...
void CJavaWrapper::ShowTabWindowww()
{
	JNIEnv* env = GetEnv();
	...
	env->CallVoidMethod(this->activity, this->s_showTabWindoww);
}
void CJavaWrapper::SetTabStattt(int id, char* name, int score, int ping) {
	...
	env->CallVoidMethod(this->activity, this->s_setTabStatt, id, jname, score, ping);
}
...
void CJavaWrapper::ShowFamily(char* name, char* mask, char* money) 
{
	JNIEnv* env = GetEnv();
 ...
 bytes = env->NewByteArray(strlen(mask));
                  env->SetByteArrayRegion(bytes, 0, strlen(mask), (jbyte*)mask);
                  jstring jmask = (jstring) env->NewObject(strClass, ctorID, bytes, encoding);

 bytes = env->NewByteArray(strlen(money));
                  env->SetByteArrayRegion(bytes, 0, strlen(money), (jbyte*)money);
                  jstring jmoney = (jstring) env->NewObject(strClass, ctorID, bytes, encoding);

	env->CallVoidMethod(this->activity, this->s_showfamily, jname, jmask, jmoney);
}

void CJavaWrapper::ShowShopFamily(char* name) 
{
	JNIEnv* env = GetEnv();
	...
	env->CallVoidMethod(this->activity, this->s_showshopfamily, jname);
}

void CJavaWrapper::ShowCarFamily(char* name, char* item, int vehid, int access, bool isspawn) 
{
	JNIEnv* env = GetEnv();
 ...
 bytes = env->NewByteArray(strlen(item));
                  env->SetByteArrayRegion(bytes, 0, strlen(item), (jbyte*)item);
                  jstring jitem = (jstring) env->NewObject(strClass, ctorID, bytes, encoding);

	env->CallVoidMethod(this->activity, this->s_showcarfamily, jname, jitem, vehid, access, isspawn);
}

void CJavaWrapper::ShowSettingsFamily(char* name, int f_money, int f_material, int f_ammo) 
{
	JNIEnv* env = GetEnv();
	...
	env->CallVoidMethod(this->activity, this->s_showsettingsfamily, jname, f_money, f_material, f_ammo);
}
...
void CJavaWrapper::ShowSpawnLocation()
{
    JNIEnv* env = GetEnv();
...
void CJavaWrapper::HideSpawnLocation()
{
    JNIEnv* env = GetEnv();
...
void CJavaWrapper::ShowInventory(int playerlevel)
{
    JNIEnv* env = GetEnv();
    ...
    env->CallVoidMethod(this->activity, this->s_showinv, playerlevel);
}
...
CJavaWrapper::CJavaWrapper(JNIEnv* env, jobject activity)
{
	...
	s_hideAuto = env->GetMethodID(nvEventClass, "HideAuto", "()V");
        ...
        s_showTabWindoww = env->GetMethodID(nvEventClass, "showTabWindowww", "()V");	
	s_setTabStatt = env->GetMethodID(nvEventClass, "setTabStattt", "(ILjava/lang/String;II)V");
...
s_showinv = env->GetMethodID(nvEventClass, "ShowInventory", "(I)V");

s_showsmi = env->GetMethodID(nvEventClass, "ShowSmi", "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)V");

s_showspawn = env->GetMethodID(nvEventClass, "ShowSpawnLocation", "()V");
s_hidespawn = env->GetMethodID(nvEventClass, "HideSpawnLocation", "()V");
...
s_showfamily = env->GetMethodID(nvEventClass, "ShowFamily", "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)V");
s_showshopfamily = env->GetMethodID(nvEventClass, "ShowShopFamily", "(Ljava/lang/String;)V");
s_showcarfamily = env->GetMethodID(nvEventClass, "ShowCarFamily", "(Ljava/lang/String;Ljava/lang/String;IIZ)V");
s_showsettingsfamily = env->GetMethodID(nvEventClass, "ShowSettingsFamily", "(Ljava/lang/String;III)V");

s_showdiner = env->GetMethodID(nvEventClass, "ShowDiner", "()V");
s_showplate = env->GetMethodID(nvEventClass, "showplate", "()V");
s_showdead = env->GetMethodID(nvEventClass, "showDead", "()V");
s_updatedead = env->GetMethodID(nvEventClass, "updatenameDead", "(Ljava/lang/String;)V");
s_hidedead = env->GetMethodID(nvEventClass, "HideDead", "()V");
