...
extern CGame* pGame;
extern CAudioStreamManager* pAudioStreamManager;
...
//		CHUD::radar1.y1 = y1;
                  ...
                  JNIEXPORT void JNICALL Java_com_nvidia_devtech_NvEventQueueActivity_SetRadarPos(JNIEnv *env, jobject thiz, jfloat x1, jfloat y1, jfloat x2, jfloat y2, jfloat size)
                  {
		CHUD::radarPos.X = x1;
		CHUD::radarPos.Y  = y1;
		...
		case 1:
			...
			case 2:
				switch(action) {
					...
					case 1:     
		                                                                       pNetGame = new CNetGame(
			                                                                g_sEncryptedAddresses[1].decrypt(),
				                                              g_sEncryptedAddresses[1].getPort(),
			                                                                pSettings->GetReadOnly().szNickName,
			                                                                pSettings->GetReadOnly().szPassword);
...
JNIEXPORT jbyteArray JNICALL Java_com_nvidia_devtech_NvEventQueueActivity_nativeEntitySnapAddToQueue(JNIEnv* pEnv, jobject thiz)
{
    VECTOR vecOffset;
	                  vecOffset.X = 0.0f;
	                  vecOffset.Y = 0.0f;
	                  vecOffset.Z = 0.0f;
        uintptr_t snapshot = pSnapShotHelper->CreateVehicleSnapShot(400, 0xFFFFFFFF, &vecOffset, 5.0f, 3, 1);

        //jbyteArray buf = (jbyteArray)(pEnv->NewGlobalRef(pEnv->NewByteArray((jsize)snapshot)));
        jbyteArray jByteArray = (jbyteArray)(pEnv->NewGlobalRef(pEnv->NewByteArray(sizeof(snapshot))));
        //pEnv->SetByteArrayRegion(jByteArray, 0, sizeof(snapshot), (jbyte*)snapshot);

    //pChatWindow->AddDebugMessage("%d", jByteArray);
    return jByteArray;
}
...
void CJavaWrapper::UpdateSpeedInfo(int speed, int fuel, int hp, int mileage, int engine, int light, int belt, int lock, int turnlight)
{
	JNIEnv* env = GetEnv();
	...
	env->CallVoidMethod(this->activity, this->s_updateSpeedInfo, speed, fuel, hp, mileage, engine, light, belt, lock, turnlight);
}
...
void CJavaWrapper::HideHelper()
{
    ...
    env->CallVoidMethod(this->activity, this->s_hideHelper);
}
...
void CJavaWrapper::VoiceOn()
{
	...
	env->CallVoidMethod(this->activity, this->s_voiceOn);
}


void CJavaWrapper::VoiceOff()
{
	JNIEnv* env = GetEnv();
	...
	env->CallVoidMethod(this->activity, this->s_voiceOff);
}
...
void CJavaWrapper::ShowBoomBox(bool toggle)
{
	JNIEnv* env = GetEnv();
	...
	env->CallVoidMethod(this->activity, this->s_showboom, toggle);
}

void CJavaWrapper::AddTestServer(char* name)
{
    JNIEnv* env = GetEnv();
    if (!env)
    {
    ...
    jstring str1name = (jstring)env->NewObject(strClass3, ctorID3, bytes3, encoding3);

    env->CallVoidMethod(activity, s_parseTestServerJson, str1name);
}
void CJavaWrapper::SetPut(bool toggle){
    JNIEnv* env = GetEnv();
    ...
    env->CallVoidMethod(this->activity, this->s_setput, toggle);
}
void CJavaWrapper::SetPause(bool toggle){
    JNIEnv* env = GetEnv();
    ...
    env->CallVoidMethod(this->activity, this->s_setpause, toggle);
}
void CJavaWrapper::Log(char log[]) {
    ...
    env->CallVoidMethod(this->activity, this->s_log, jlog);
...
CJavaWrapper::CJavaWrapper(JNIEnv* env, jobject activity)
{
    ...
    s_log = env->GetMethodID(nvEventClass, "LogClient", "(Ljava/lang/String;)V");
	...
	s_voiceOn = env->GetMethodID(nvEventClass, "voiceOn", "()V");
	s_voiceOff = env->GetMethodID(nvEventClass, "voiceOff", "()V");
	...
	s_parseTestServerJson = env->GetMethodID(nvEventClass, "parseTestServerJson", "(Ljava/lang/String;)V");
	...
	s_updateSpeedInfo = env->GetMethodID(nvEventClass, "updateSpeedInfo", "(IIIIIIIII)V");
    ...
    s_showboom = env->GetMethodID(nvEventClass, "showboombox", "(Z)V");
    s_setput = env->GetMethodID(nvEventClass, "setput", "(Z)V");
    s_setpause = env->GetMethodID(nvEventClass, "setpause", "(Z)V");
    j_toggleAutoShop = env->GetMethodID(nvEventClass, "toggleauto", "(Z)V");
    j_togglehud = env->GetMethodID(nvEventClass, "toggleHud", "(Z)V");
    j_toggleperson = env->GetMethodID(nvEventClass, "toggleperson", "(Z)V");

    j_updateAutoShop = env->GetMethodID(nvEventClass, "updateauto",
                                        "(Ljava/lang/String;IIFFI)V");
...
Java_com_rockstargames_gtacr_gui_BoomBox_PlayBoomBox(JNIEnv *env, jobject thiz, jstring url) {
    const char *_url = env->GetStringUTFChars(url, nullptr);

    if(pNetGame){
        pNetGame->SendPlayMusic(_url);
    }
...
void CJavaWrapper::ToggleAutoShop(bool toggle) {
    ...
    CHUD::m_isAutoShop = toggle;

    env->CallVoidMethod(this->activity, this->j_toggleAutoShop, toggle);
}
void CJavaWrapper::ToggleHud(bool toggle){
    JNIEnv *env = GetEnv();
    ...
    env->CallVoidMethod(this->activity, this->j_togglehud, toggle);
}

void CJavaWrapper::TogglePerson(bool toggle){
    JNIEnv *env = GetEnv();
    ...
    CHUD::m_bPerson = toggle;

    env->CallVoidMethod(this->activity, this->j_toggleperson, toggle);
}

void CJavaWrapper::UpdateAutoShop(const char name[], int price, int count, float maxspeed,
                                  float acceleration, int gear) {
    JNIEnv *env = GetEnv();
    ...
    env->CallVoidMethod(this->activity, this->j_updateAutoShop, j_name, price, count, maxspeed,
                        acceleration, gear);
...
extern "C"
JNIEXPORT void JNICALL
Java_com_rockstargames_gtacr_gui_BoomBox_ClickBoomBox(JNIEnv *env, jobject thiz, jint button) {
...
extern "C"
JNIEXPORT void JNICALL
Java_com_nvidia_devtech_NvEventQueueActivity_sendServer(JNIEnv *env, jobject thiz, jint type) {
    // TODO: implement sendServer()
    pNetGame = new CNetGame(
            g_sEncryptedAddresses[type].decrypt(),
            g_sEncryptedAddresses[type].getPort(),
            pSettings->GetReadOnly().szNickName,
            pSettings->GetReadOnly().szPassword);
    pSettings->GetWrite().last_server = type;
    pSettings->Save();
}
extern "C"
JNIEXPORT void JNICALL
Java_com_nvidia_devtech_NvEventQueueActivity_setVoulmeMusic(JNIEnv *env, jobject thiz, jfloat volume) {
    // TODO: implement setVoulmeMusic()
    if(volume > 1.0f)
        volume = 1.0f;
    pSettings->GetWrite().fjblVolume = volume;
    if(pAudioStreamManager){
        pAudioStreamManager->SetVoulme(pSettings->GetReadOnly().fjblVolume);
    }
    pSettings->Save();
}
extern "C"
JNIEXPORT jfloat JNICALL
Java_com_nvidia_devtech_NvEventQueueActivity_getVolumeMusic(JNIEnv *env, jobject thiz) {
    // TODO: implement getVolumeMusic()
    return (jfloat)pSettings->GetReadOnly().fjblVolume;
}
extern "C"
JNIEXPORT void JNICALL
Java_com_nvidia_devtech_NvEventQueueActivity_sendAutoShopButton(JNIEnv *env, jobject thiz,
                                                                jint button) {
    // TODO: implement sendAutoShopButton()
    RakNet::BitStream bsSend;
    bsSend.Write((uint8_t) 23);
    bsSend.Write((uint8_t) 0x01);
    bsSend.Write((uint8_t) button);
...
extern "C"
JNIEXPORT void JNICALL
Java_com_nvidia_devtech_NvEventQueueActivity_personclick(JNIEnv *env, jobject thiz, jint button) {
    // TODO: implement personclick()
    RakNet::BitStream bsSend;
    bsSend.Write((uint8_t) 24);
    bsSend.Write((uint8_t) 0x01);
    bsSend.Write((uint8_t) button);

    pNetGame->GetRakClient()->Send(&bsSend, HIGH_PRIORITY, RELIABLE, 0);
}
void CJavaWrapper::toggleDrivingSchool(bool toggle) {
    JNIEnv *env = GetEnv();
    ...
    jmethodID jmethodId = env->GetMethodID(env->GetObjectClass(this->activity), "toggleDrivingSchool","(Z)V");
    env->CallVoidMethod(this->activity, jmethodId, toggle);
