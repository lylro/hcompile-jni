...
void CJavaWrapper::AddMessageToJavaChat(const char msg[])
{
	JNIEnv* env = GetEnv();
	char msg_utf[1024];
	...
	env->CallVoidMethod(this->activity, this->s_addJavaChatMessage, jmsg);
}
...
void CJavaWrapper::ShowPaus()
{
	...
	env->CallVoidMethod(this->activity, this->s_showPaus);
}
...
CJavaWrapper::CJavaWrapper(JNIEnv* env, jobject activity)
{
	...
	s_addJavaChatMessage = env->GetMethodID(nvEventClass, "addMessageToChat", "(Ljava/lang/String;)V");
	s_showPaus = env->GetMethodID(nvEventClass, "ShowPaus", "()V");
