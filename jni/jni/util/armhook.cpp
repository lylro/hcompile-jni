...
void InitHookStuff()
{
	...
	memlib_start = SA_ADDR(0x179FE0);
	memlib_end = memlib_start + 0x2A36;
	...
	WriteMemory(SA_ADDR(0x1BDD4A), (uintptr_t)"\x10\x46\xA2\xF1\x04\x0B", 6); // TextureDatabaseRuntime::LoadFullTexure
	WriteMemory(SA_ADDR(0x3E1A2C), (uintptr_t)"\x67\xE0", 2); // Interior_c::Interior
}
...
#pragma optimize( "", off )
#include "..///..//tea-gtasa/encryption/common.h"
