...
void CPatch::installJMPHook(uintptr_t addr, uintptr_t func)
{
	Log("installJMPHook: 0x%X -> 0x%X", addr, func);

	if(memlib_end < (memlib_start + 0x10) || mmap_end < (mmap_start + 0x20))
	{
		Log("installJMPHook: space limit reached");
		std::terminate();
	}
