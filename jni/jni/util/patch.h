...
public:
    ...
    static void installJMPHook(uintptr_t addr, uintptr_t func);
    ...
    static void SetUpHook(Addr addr, Func func, Orig orig)
    {
        ...
        *(uintptr_t*)orig = mmap_start+1;
        mmap_start += 32;

        JMPCode(addr, memlib_start);
        WriteHookProc(memlib_start, (uintptr_t)func);
        memlib_start += 16;
    }
    ...
    {   
        //CPatch::SetUpHook(lib + (uintptr_t)addr, (uintptr_t)func, (uintptr_t*)orig);
       // return;
        *orig = NULL;
        addr += CRYPT_MASK;
