...
auto packetst = cryptor::create(OBFUSCATE("ru.stepdev.crimemobile"), 23);
auto packetbh = cryptor::create(OBFUSCATE("ru.stepdev.crimemobile"), 23);
...
bool DumpLibraries(std::vector<std::string>& buff)
{
	...
	{
		//Log("buffer: %s", buffer);
		if (strstr(&buffer[0], CMain::GetCryptStorage()))
		{
...
int DumpProcessLibraries(char* findedLibrary)
{
	...
	while (fgets(buffer, sizeof(buffer), fp))
	{
		if (strstr(&buffer[0], findedLibrary))
		{
			fclose(fp);
			return 1;
		}
...
char* return_DumpProcessLibraries(char* findedLibrary)
{
	char filename[0xFF] = { 0 },
		buffer[2048] = { 0 };
	...
	while (fgets(buffer, sizeof(buffer), fp))
	{
		if (strstr(&buffer[0], findedLibrary))
		{
			char buffer_return[2048] = { 0 };
			sprintf(buffer_return, "%s", buffer[0]);
			fclose(fp);
