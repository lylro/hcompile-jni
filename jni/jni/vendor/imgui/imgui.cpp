...
/*
 ...
 EXHIBIT 1: USING THE EXAMPLE BINDINGS (imgui_impl_XXX.cpp files from the examples/ folder)
     ...
     // Initialise helper Platform and Renderer bindings (here we are using imgui_impl_win32 and imgui_impl_dx11)
...
#if 0
    if (g.NavScoringCount > 0) printf("[%05d] NavScoringCount %d for '%s' layer %d (Initialise:%d, Move:%d)\n", g.FrameCount, g.NavScoringCount, g.NavWindow ? g.NavWindow->Name : "NULL", g.NavLayer, g.NavInitRequest || g.NavInitResultId != 0, g.NavMoveRequest);
#endif
...
ImVec2 ImGui::CalcTextSizeByFontSize(float size, const char* text, const char* text_end, bool hide_text_after_double_hash, float wrap_width)
{
    ImGuiContext& g = *GImGui;
...
ImVec2 ImGui::CalcTextSize(const float font_size, const char* text, const char* text_end, bool hide_text_after_double_hash, float wrap_width)
{
ImGuiContext& g = *GImGui;
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        else
        {
            ...
            // Borders
            if (window_border_size > 0.0f && !(window->Flags & ImGuiWindowFlags_NoBackground))
...
static bool ImGui::SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min, const TYPE v_max, const char* format, float power, ImGuiSliderFlags flags)
{
	...
	const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab);
    RenderFrame(grab_bb.Min, grab_bb.Max, frame_col, true, g.Style.FrameRounding);
