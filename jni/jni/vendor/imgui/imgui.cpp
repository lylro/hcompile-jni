...
ImGuiStyle::ImGuiStyle()
{
    ...
    WindowRounding          = 4.0f;             // Radius of window corners rounding. Set to 0.0f to have rectangular windows
    ...
    ChildBorderSize         = 3.0f;             // Thickness of border around child windows. Generally set to 0.0f or 1.0f. Other values not well tested.
    PopupRounding           = 3.0f;             // Radius of popup window corners rounding. Set to 0.0f to have rectangular child windows
    PopupBorderSize         = 3.0f;             // Thickness of border around popup or tooltip windows. Generally set to 0.0f or 1.0f. Other values not well tested.
    ...
    FrameBorderSize         = 3.0f;             // Thickness of border around frames. Generally set to 0.0f or 1.0f. Other values not well tested.
...
void ImGui::SetHoveredID(ImGuiID id)
{
    //ImGuiContext& g = *GImGui;
    GImGui->HoveredId = id;
    GImGui->HoveredIdAllowOverlap = false;
    if (id != 0 && GImGui->HoveredIdPreviousFrame != id)
...
void ImGui::Render()
{
    ...
    if (g.IO.MouseDrawCursor && g.IO.Fonts->GetMouseCursorTexData(g.MouseCursor, &offset, &size, &uv[0], &uv[2]))
    {
        ...
        g.OverlayDrawList.AddImage(tex_id, pos + ImVec2(1,0)*sc, pos+ImVec2(1,0)*sc + size*sc, uv[2], uv[3], IM_COL32(37, 37, 37, 0.9));        // Shadow
        g.OverlayDrawList.AddImage(tex_id, pos + ImVec2(2,0)*sc, pos+ImVec2(2,0)*sc + size*sc, uv[2], uv[3], IM_COL32(37, 37, 37, 0.9));        // Shadow
        g.OverlayDrawList.AddImage(tex_id, pos,                  pos + size*sc,                uv[2], uv[3], IM_COL32(37, 37, 37,255));       // Black border
...
void ImGui::Scrollbar(ImGuiLayoutType direction)
{
    ...
    if (held && grab_h_norm < 1.0f)
    {
        ...
        if (!previously_held)
        {
            ...
            {
              if(horizontal) g.ScrollbarClickDeltaToGrabCenter.x = clicked_v_norm - grab_v_norm - grab_h_norm*0.5f;
              else g.ScrollbarClickDeltaToGrabCenter.y = clicked_v_norm - grab_v_norm - grab_h_norm*0.5f;
            }
            else
            {
                seek_absolute = true;
                if(horizontal) g.ScrollbarClickDeltaToGrabCenter.x = 0.0f;
                else g.ScrollbarClickDeltaToGrabCenter.y = 0.0f;
            }
        }

        // Apply scroll
        // It is ok to modify Scroll here because we are being called in Begin() after the calculation of SizeContents and before setting up our starting position
        const float scroll_v_norm = ImSaturate((clicked_v_norm -  
        (horizontal ? g.ScrollbarClickDeltaToGrabCenter.x : g.ScrollbarClickDeltaToGrabCenter.y) 
        /**click_delta_to_grab_center_v*/ - grab_h_norm*0.5f) / (1.0f - grab_h_norm));
