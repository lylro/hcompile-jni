...
CSoundOutput::CSoundOutput(char* deviceName, int sampleRate, int sourcesCount) : _sampleRate(sampleRate)
{
	...
	if (!device)
		Log(OBFUSCATE("Cannot open device"));
	...
	if (ctx == nullptr || alcMakeContextCurrent(ctx) == ALC_FALSE)
	{
		...
		Log(OBFUSCATE("Cannot create context"));
		return;
	}
	...
	Log(OBFUSCATE("Max sources: %d"), maxMonoSources);
	sources = new ALuint[sourcesCount];
	alGenSources(sourcesCount, sources);
	if (alGetError() != AL_NO_ERROR)
	{
		Log(OBFUSCATE("Sources error"));
		return;
	}
...
{
	auto* nextStreamPlayer = new CStreamPlayer();
