...
void CVoiceChatClient::OnPacketIncoming(ENetEvent& event)
{
	...
	switch (id)
	{
		...
		case 246:
		{
			...
			pChatWindow->AddDebugMessageNonFormatted(CLocalisation::GetMsg(E_MSG::VOICE_MUTED));
...
CVoiceChatClient::CVoiceChatClient()
{
	...
	{
		Log(OBFUSCATE("Could not initialize network"));
		return;
	}
	...
	if (!m_pPlayer)
	{
		Log(OBFUSCATE("Cannot open microphone, allow this in permissions"));
...
void CVoiceChatClient::UnMutePlayer(int iPlayerID)
{
	...
	m_StatusMute[iPlayerID] = 1;

	bufferedLocal.WriteUnlock();
}

int CVoiceChatClient::StatusMutePlayer(int iPlayerID)
{
	return m_StatusMute[iPlayerID];
}
...
int CVoiceChatClient::GetVolumePlayer(int iPlayerID)
{
	return m_bVolume[iPlayerID];
}
...
void CVoiceChatClient::SetVolume(int iVolume)
{
	...
	m_GlobalVolume = iVolume;
}

int CVoiceChatClient::GetVolume()
{
	return m_GlobalVolume;
}

void CVoiceChatClient::SetPlayerVoiceStatus(int iPlayerID, bool status)
{
	m_PlayerVoiceStatus[iPlayerID] = status;
}

bool CVoiceChatClient::GetPlayerVoiceStatus(int iPlayerID)
{
	return m_PlayerVoiceStatus[iPlayerID];
}
...
#define AUTH_BS OBFUSCATE("15121F6F18550C00AC4B4F8A167D0379BB0ACA99043")
...
{
	//if (pChatWindow) pChatWindow->AddDebugMessage(OBFUSCATE("Вы отключены от войс чата"));
	Log("Full Disconnect Voice Chat");
		...
		while (true)
		{
			...
			
			{
				...
				while ((bcs = bufferedNetwork.ReadLock()) != 0)
				{
					...
					{
						int state = *(int*)(SA_ADDR(0x8ED86D)); // CMenuManager::m_bMenuActive
						int state1 = *(int*)(SA_ADDR(0x63E06C)); // gMobileMenu
						int state2 = *(int*)(SA_ADDR(0x63E074));
						int state3 = 0;

						if (GetTickCount() - bProcessedRender2dstuff >= 500)
							state3 = 1;

						uint8_t isPaused = *(uint8_t*)(SA_ADDR(0x5DDD38));
