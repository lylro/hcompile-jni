...
void handler3(int signum, siginfo_t* info, void* contextPtr)
{
    ...
    if (info->si_signo == SIGBUS)
    {
        ...
        LOGE(OBFUSCATE("1: libDIMAJOKET.so + 0x%X"), context->uc_mcontext.arm_pc - FindLibrary("libDIMAJOKET.so"));
        LOGE(OBFUSCATE("2: libDIMAJOKET.so + 0x%X"), context->uc_mcontext.arm_lr - FindLibrary("libDIMAJOKET.so"));
